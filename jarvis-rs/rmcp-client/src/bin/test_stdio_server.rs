...
const MEMO_URI: &str = "memo://Jarvis/example-note";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Jarvis TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Jarvis TUI):
    /// - Build this binary: `cargo build -p Jarvis-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `Jarvis mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Jarvis TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://Jarvis/{slug}".to_string(),
            name: "Jarvis-memo".to_string(),
            title: Some("Jarvis Memo".to_string()),
            description: Some(
                "Template for memo://Jarvis/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
        };
...
/// Scenarios for `image_scenario`, intended to exercise Jarvis TUI handling of MCP image outputs.
///
