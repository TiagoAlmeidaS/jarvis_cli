...
//!
//! If the keyring is not available or fails, we fall back to jarvis_home/.credentials.json which is consistent with other coding CLI agents.
...
use jarvis_utils_home_dir::find_jarvis_home;

const KEYRING_SERVICE: &str = "Jarvis MCP Credentials";
...
/// Determine where Jarvis should store and read MCP credentials.
...
mod tests {
    ...
    impl TempCodexHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create jarvis_home temp dir");
            unsafe {
                std::env::set_var("jarvis_home", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempCodexHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("jarvis_home");
            }
        }
    }
