...
#[derive(Debug, Parser)]
#[command(name = "Jarvis-state-logs")]
#[command(about = "Tail Jarvis logs from the state SQLite DB with simple filters")]
struct Args {
    /// Path to jarvis_home. Defaults to $jarvis_home or ~/.jarvis.
    #[arg(long, env = "jarvis_home")]
    jarvis_home: Option<PathBuf>,

    /// Direct path to the SQLite database. Overrides --Jarvis-home.
    #[arg(long)]
    db: Option<PathBuf>,
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let jarvis_home = args.jarvis_home.clone().unwrap_or_else(default_codex_home);
    Ok(jarvis_state::state_db_path(jarvis_home.as_path()))
}

fn default_codex_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".jarvis");
    }
    PathBuf::from(".jarvis")
}
