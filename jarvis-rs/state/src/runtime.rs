...
const METRIC_DB_INIT: &str = "Jarvis.db.init";
...
impl StateRuntime {
    /// Initialize the state runtime using the provided Jarvis home and default provider.
    ///
    /// This opens (and migrates) the SQLite database at `jarvis_home/state.sqlite`.
    pub async fn init(
        jarvis_home: PathBuf,
        default_provider: String,
        ...
        let state_path = state_db_path(jarvis_home.as_path());
    ...
    /// Return the configured Jarvis home directory for this runtime.
    pub fn jarvis_home(&self) -> &Path {
        self.jarvis_home.as_path()
    }
...
FROM threads
    ...
    /// Insert a tool operation into the tool_operations table.
    pub async fn insert_tool_operation(&self, operation: &ToolOperation) -> anyhow::Result<()> {
        let (
            thread_id,
            call_id,
            tool_name,
...
INSERT INTO tool_operations (
    ...
    /// Query tool operations for a specific thread.
    pub async fn query_tool_operations(
        &self,
        thread_id: ThreadId,
        limit: Option<usize>,
    ) -> anyhow::Result<Vec<ToolOperation>> {
...
WHERE thread_id = ?
ORDER BY created_at DESC, created_at_nanos DESC, id DESC
            "#,
        );
        builder.push_bind(thread_id_str);
        ...
        for row in rows {
            operations.push(ToolOperation::from_row(&row)?);
        }
        Ok(operations)
    }

    /// Query tool operations by tool name across all threads.
    pub async fn query_tool_operations_by_name(
        &self,
        tool_name: &str,
        limit: Option<usize>,
    ) -> anyhow::Result<Vec<ToolOperation>> {
...
pub fn state_db_path(jarvis_home: &Path) -> PathBuf {
    jarvis_home.join(state_db_filename())
}

async fn remove_legacy_state_files(jarvis_home: &Path) {
    let current_name = state_db_filename();
    let mut entries = match tokio::fs::read_dir(jarvis_home).await {
        Ok(entries) => entries,
        Err(err) => {
            warn!(
                "failed to read jarvis_home for state db cleanup {}: {err}",
                jarvis_home.display()
            );
            return;
        }
...
mod tests {
    ...
    fn unique_temp_dir() -> PathBuf {
        ...
        std::env::temp_dir().join(format!("Jarvis-state-runtime-test-{nanos}"))
    }

    #[tokio::test]
    async fn init_removes_legacy_state_db_files() {
        let jarvis_home = unique_temp_dir();
        tokio::fs::create_dir_all(&jarvis_home)
            .await
            .expect("create jarvis_home");

        let current_name = state_db_filename();
        let previous_version = STATE_DB_VERSION.saturating_sub(1);
        let unversioned_name = format!("{STATE_DB_FILENAME}.sqlite");
        for suffix in ["", "-wal", "-shm", "-journal"] {
            let path = jarvis_home.join(format!("{unversioned_name}{suffix}"));
            tokio::fs::write(path, b"legacy")
                .await
                .expect("write legacy");
            let old_version_path = jarvis_home.join(format!(
        ...
        let unrelated_path = jarvis_home.join("state.sqlite_backup");
        tokio::fs::write(&unrelated_path, b"keep")
            .await
            .expect("write unrelated");
        let numeric_path = jarvis_home.join("123");
        tokio::fs::write(&numeric_path, b"keep")
            .await
            .expect("write numeric");

        let _runtime = StateRuntime::init(jarvis_home.clone(), "test-provider".to_string(), None)
            .await
            .expect("initialize runtime");

        for suffix in ["", "-wal", "-shm", "-journal"] {
            let legacy_path = jarvis_home.join(format!("{unversioned_name}{suffix}"));
            assert_eq!(
        ...
        assert_eq!(
            tokio::fs::try_exists(jarvis_home.join(current_name))
                .await
        ...
        let _ = tokio::fs::remove_dir_all(jarvis_home).await;
    }

    #[tokio::test]
    async fn insert_and_query_tool_operations() {
        let jarvis_home = temp_dir();
        let runtime = StateRuntime::init(jarvis_home.clone(), "test-provider".to_string(), None)
            .await
            .expect("init runtime");

        let thread_id = jarvis_protocol::ThreadId::new_v4();
        ...
        runtime
            .insert_tool_operation(&operation)
            .await
            .expect("insert operation");

        let operations = runtime
            .query_tool_operations(thread_id, None)
            .await
    ...
    async fn tool_success_rates() {
        ...
        let success_rates = runtime
            .tool_success_rates(Some(thread_id))
            .await
            .expect("get success rates");

        let shell_rate = success_rates.get("shell").expect("shell rate");
