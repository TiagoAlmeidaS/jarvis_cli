...
//! orchestration and rollout scanning live in `Jarvis-core`.
...
pub use model::ToolOperation;
pub use model::ToolOperationBuilder;
...
pub const DB_ERROR_METRIC: &str = "Jarvis.db.error";
/// Metrics on backfill process during first init of the db. Tags: [status]
pub const DB_METRIC_BACKFILL: &str = "Jarvis.db.backfill";
/// Metrics on backfill duration during first init of the db. Tags: [status]
pub const DB_METRIC_BACKFILL_DURATION_MS: &str = "Jarvis.db.backfill.duration_ms";
/// Metrics on errors during comparison between DB and rollout file. Tags: [stage]
pub const DB_METRIC_COMPARE_ERROR: &str = "Jarvis.db.compare_error";
