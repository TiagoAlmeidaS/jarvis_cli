...
fn execpolicy_check_matches_expected_json() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let policy_path = jarvis_home.path().join("rules").join("policy.rules");
    fs::create_dir_all(
        policy_path
            .parent()
...
"#,
    ...
    let output = Command::new(jarvis_utils_cargo_bin::cargo_bin("Jarvis")?)
        .env("jarvis_home", jarvis_home.path())
        .args([
            "execpolicy",
            "check",
            "--rules",
