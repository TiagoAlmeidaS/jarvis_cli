...
use jarvis_rmcp_client::delete_oauth_tokens;
use jarvis_rmcp_client::perform_oauth_login;

/// Subcommands:
/// - `list`   — list configured servers (with `--json`)
/// - `get`    — show a single server (with `--json`)
/// - `add`    — add a server launcher entry to `~/.jarvis/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "Jarvis mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    ConfigEditsBuilder::new(&jarvis_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", jarvis_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `Jarvis mcp login {name}` to login."
        ),
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `Jarvis mcp add my-tool -- my-command`.");
        return Ok(());
    }
