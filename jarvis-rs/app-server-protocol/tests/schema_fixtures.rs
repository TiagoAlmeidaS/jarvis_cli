...
fn schema_root() -> Result<std::path::PathBuf> {
    ...
    let typescript_index = jarvis_utils_cargo_bin::find_resource!("schema/typescript/index.ts")
    ...
    let json_root = json_bundle
        .parent()
        .and_then(|p| p.parent())
        .context("derive schema root from schema/json/jarvis_app_server_protocol.schemas.json")?;
    anyhow::ensure!(
