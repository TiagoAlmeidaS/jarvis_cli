...
use jarvis_experimental_api_macros::ExperimentalApi;
use jarvis_protocol::account::PlanType;
use jarvis_protocol::approvals::ExecPolicyAmendment as CoreExecPolicyAmendment;
...
use jarvis_protocol::config_types::SandboxMode as CoreSandboxMode;
use jarvis_protocol::config_types::Verbosity;
use jarvis_protocol::config_types::WebSearchMode;
use jarvis_protocol::items::AgentMessageContent as CoreAgentMessageContent;
use jarvis_protocol::items::TurnItem as CoreTurnItem;
use jarvis_protocol::mcp::Resource as McpResource;
use jarvis_protocol::mcp::ResourceTemplate as McpResourceTemplate;
use jarvis_protocol::mcp::Tool as McpTool;
...
use jarvis_protocol::parse_command::ParsedCommand as CoreParsedCommand;
use jarvis_protocol::plan_tool::PlanItemArg as CorePlanItemArg;
...
use jarvis_protocol::protocol::SessionSource as CoreSessionSource;
use jarvis_protocol::protocol::SkillDependencies as CoreSkillDependencies;
use jarvis_protocol::protocol::SkillErrorInfo as CoreSkillErrorInfo;
use jarvis_protocol::protocol::SkillInterface as CoreSkillInterface;
use jarvis_protocol::protocol::SkillMetadata as CoreSkillMetadata;
use jarvis_protocol::protocol::SkillScope as CoreSkillScope;
use jarvis_protocol::protocol::SkillToolDependency as CoreSkillToolDependency;
use jarvis_protocol::protocol::SubAgentSource as CoreSubAgentSource;
use jarvis_protocol::protocol::TokenUsage as CoreTokenUsage;
use jarvis_protocol::protocol::TokenUsageInfo as CoreTokenUsageInfo;
use jarvis_protocol::user_input::ByteRange as CoreByteRange;
use jarvis_protocol::user_input::TextElement as CoreTextElement;
use jarvis_protocol::user_input::UserInput as CoreUserInput;
...
/// This translation layer make sure that we expose Jarvis error code in camel case.
///
/// When an upstream HTTP status is available (for example, from the Responses API or a provider),
/// it is forwarded in `httpStatusCode` on the relevant `JarvisErrorInfo` variant.
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, Eq, JsonSchema, TS)]
#[serde(rename_all = "camelCase")]
#[ts(export_to = "v2/")]
pub enum JarvisErrorInfo {
...
impl From<CoreCodexErrorInfo> for JarvisErrorInfo {
    fn from(value: CoreCodexErrorInfo) -> Self {
        match value {
            CoreCodexErrorInfo::ContextWindowExceeded => JarvisErrorInfo::ContextWindowExceeded,
            CoreCodexErrorInfo::UsageLimitExceeded => JarvisErrorInfo::UsageLimitExceeded,
            ...
            CoreCodexErrorInfo::HttpConnectionFailed { http_status_code } => {
                JarvisErrorInfo::HttpConnectionFailed { http_status_code }
            }
            CoreCodexErrorInfo::ResponseStreamConnectionFailed { http_status_code } => {
                JarvisErrorInfo::ResponseStreamConnectionFailed { http_status_code }
            }
            CoreCodexErrorInfo::InternalServerError => JarvisErrorInfo::InternalServerError,
            CoreCodexErrorInfo::Unauthorized => JarvisErrorInfo::Unauthorized,
            CoreCodexErrorInfo::BadRequest => JarvisErrorInfo::BadRequest,
            CoreCodexErrorInfo::ThreadRollbackFailed => JarvisErrorInfo::ThreadRollbackFailed,
            CoreCodexErrorInfo::SandboxError => JarvisErrorInfo::SandboxError,
            CoreCodexErrorInfo::ResponseStreamDisconnected { http_status_code } => {
                JarvisErrorInfo::ResponseStreamDisconnected { http_status_code }
            }
            CoreCodexErrorInfo::ResponseTooManyFailedAttempts { http_status_code } => {
                JarvisErrorInfo::ResponseTooManyFailedAttempts { http_status_code }
            }
            CoreCodexErrorInfo::Other => JarvisErrorInfo::Other,
        }
    }
}
...
v2_enum_from_core!(
    pub enum ReviewDelivery from jarvis_protocol::protocol::ReviewDelivery {
        Inline, Detached
    }
);

v2_enum_from_core!(
    pub enum McpAuthStatus from jarvis_protocol::protocol::McpAuthStatus {
        Unsupported,
        NotLoggedIn,
        BearerToken,
        OAuth
...
pub enum ConfigLayerSource {
    ...
    /// User config layer from $jarvis_home/config.toml. This layer is special
    ...
    /// Path to a .jarvis/ folder within a project. There could be multiple of
...
impl SandboxPolicy {
    pub fn to_core(&self) -> jarvis_protocol::protocol::SandboxPolicy {
        match self {
            SandboxPolicy::DangerFullAccess => {
                jarvis_protocol::protocol::SandboxPolicy::DangerFullAccess
            }
            SandboxPolicy::ReadOnly => jarvis_protocol::protocol::SandboxPolicy::ReadOnly,
            SandboxPolicy::ExternalSandbox { network_access } => {
                jarvis_protocol::protocol::SandboxPolicy::ExternalSandbox {
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                exclude_slash_tmp,
            } => jarvis_protocol::protocol::SandboxPolicy::WorkspaceWrite {
...
impl From<jarvis_protocol::protocol::SandboxPolicy> for SandboxPolicy {
    fn from(value: jarvis_protocol::protocol::SandboxPolicy) -> Self {
        match value {
            jarvis_protocol::protocol::SandboxPolicy::DangerFullAccess => {
                SandboxPolicy::DangerFullAccess
            }
            jarvis_protocol::protocol::SandboxPolicy::ReadOnly => SandboxPolicy::ReadOnly,
            jarvis_protocol::protocol::SandboxPolicy::ExternalSandbox { network_access } => {
...
pub enum LoginAccountParams {
    ...
    /// The access token must contain the same scopes that Jarvis-managed ChatGPT auth tokens have.
...
pub enum ChatgptAuthTokensRefreshReason {
    /// Jarvis attempted a backend request and received `401 Unauthorized`.
    Unauthorized,
}
...
pub struct ChatgptAuthTokensRefreshParams {
    ...
    /// Workspace/account identifier that Jarvis was previously using.
    ///
...
pub struct ThreadStartParams {
    ...
    ///
    /// This is for internal use only (e.g. Jarvis Cloud).
...
pub struct ThreadResumeParams {
    ...
    /// [UNSTABLE] FOR Jarvis CLOUD - DO NOT USE.
...
pub struct Thread {
    ...
    /// Origin of the thread (CLI, VSCode, Jarvis exec, Jarvis app-server, etc.).
...
pub struct TurnError {
    pub message: String,
    pub jarvis_error_info: Option<JarvisErrorInfo>,
    #[serde(default)]
...
impl From<jarvis_protocol::models::WebSearchAction> for WebSearchAction {
    fn from(value: jarvis_protocol::models::WebSearchAction) -> Self {
        match value {
            jarvis_protocol::models::WebSearchAction::Search { query, queries } => {
                WebSearchAction::Search { query, queries }
            }
            jarvis_protocol::models::WebSearchAction::OpenPage { url } => {
                WebSearchAction::OpenPage { url }
            }
            jarvis_protocol::models::WebSearchAction::FindInPage { url, pattern } => {
                WebSearchAction::FindInPage { url, pattern }
            }
            jarvis_protocol::models::WebSearchAction::Other => WebSearchAction::Other,
        }
    }
}
...
mod tests {
    use super::*;
    use jarvis_protocol::items::AgentMessageContent;
    use jarvis_protocol::items::AgentMessageItem;
    use jarvis_protocol::items::ReasoningItem;
    use jarvis_protocol::items::TurnItem;
    use jarvis_protocol::items::UserMessageItem;
    use jarvis_protocol::items::WebSearchItem;
    use jarvis_protocol::models::WebSearchAction as CoreWebSearchAction;
    use jarvis_protocol::protocol::NetworkAccess as CoreNetworkAccess;
    use jarvis_protocol::user_input::UserInput as CoreUserInput;
        ...
        let user_item = TurnItem::UserMessage(UserMessageItem {
            ...
            content: vec![
                ...
                CoreUserInput::Skill {
                    ...
                    path: PathBuf::from("/repo/.jarvis/skills/skill-creator/SKILL.md"),
                },
    ...
    #[test]
    fn jarvis_error_info_serializes_http_status_code_in_camel_case() {
        let value = JarvisErrorInfo::ResponseTooManyFailedAttempts {
