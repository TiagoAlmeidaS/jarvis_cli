...
pub enum AuthMode {
    /// OpenAI API key provided by the caller and stored by Jarvis.
    ApiKey,
    /// ChatGPT OAuth managed by Jarvis (tokens persisted and refreshed by Jarvis).
    Chatgpt,
...
client_request_definitions! {
    ...
    /// List recorded Jarvis conversations (rollouts) with optional pagination and search.
    ListConversations {
        params: v1::ListConversationsParams,
        response: v1::ListConversationsResponse,
    },
    /// Resume a recorded Jarvis conversation from a rollout file.
    ResumeConversation {
        params: v1::ResumeConversationParams,
        response: v1::ResumeConversationResponse,
    },
    /// Fork a recorded Jarvis conversation into a new session.
    ForkConversation {
...
/// Superset of [`jarvis_file_search::FileMatch`]
#[derive(Serialize, Deserialize, Debug, Clone, PartialEq, JsonSchema, TS)]
...
server_notification_definitions! {
    ...
    /// This event is internal-only. Used by Jarvis Cloud.
