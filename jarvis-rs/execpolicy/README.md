...
- The legacy rule matcher lives in `jarvis-execpolicy-legacy`.
...
- From the jarvis CLI, run `jarvis execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
jarvis execpolicy check --rules path/to/policy.rules git status
```
...
```bash
cargo run -p jarvis-execpolicy -- check --rules path/to/policy.rules git status
```
