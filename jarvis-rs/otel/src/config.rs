...
pub enum OtelExporter {
    None,
    /// Statsig metrics ingestion exporter using Jarvis-internal defaults.
    ///
    /// This is intended for metrics only.
    Statsig,
    /// Prometheus exporter that exposes metrics via HTTP endpoint.
    ///
    /// This exporter collects OpenTelemetry metrics and exposes them
    /// in Prometheus format. The endpoint should be configured separately
    /// (e.g., via HTTP server exposing /metrics).
    Prometheus,
    OtlpGrpc {
        endpoint: String,
