...
use jarvis_api::ApiError;
use jarvis_api::ResponseEvent;
...
impl OtelManager {
    ...
    pub fn new(
        ...
        Self {
            ...
            // Temporariamente comentado para resolver dependência circular
            // state_runtime: None,
        }
    }

    /// Set the state runtime for persistence of tool operations.
    // Temporariamente comentado para resolver dependência circular
    // pub fn with_state_runtime(
    //     mut self,
    //     state_runtime: std::sync::Arc<jarvis_state::StateRuntime>,
    // ) -> Self {
    //     self.state_runtime = Some(state_runtime);
    //     self
    // }
    ...
    pub fn conversation_starts(
        ...
        tracing::event!(
            tracing::Level::INFO,
            event.name = "Jarvis.conversation_starts",
    ...
    pub fn record_websocket_request(&self, duration: Duration, error: Option<&str>) {
        ...
        tracing::event!(
            tracing::Level::INFO,
            event.name = "Jarvis.websocket_request",
            event.timestamp = %timestamp(),
            conversation.id = %self.metadata.conversation_id,
            app.version = %self.metadata.app_version,
            auth_mode = self.metadata.auth_mode,
    ...
    pub fn record_websocket_event(
        ...
        tracing::event!(
            tracing::Level::INFO,
            event.name = "Jarvis.websocket_event",
            event.timestamp = %timestamp(),
            event.kind = %kind_str,
            conversation.id = %self.metadata.conversation_id,
            app.version = %self.metadata.app_version,
    ...
    pub fn tool_decision(
        ...
        tracing::event!(
            tracing::Level::INFO,
            event.name = "Jarvis.tool_decision",
            event.timestamp = %timestamp(),
            conversation.id = %self.metadata.conversation_id,
            app.version = %self.metadata.app_version,
            auth_mode = self.metadata.auth_mode,
    ...
    pub fn tool_result(
        ...
        // Temporariamente comentado para resolver dependência circular
        // Persist to database if state runtime is available
        // if let Some(state_runtime) = &self.state_runtime {
        //     let state_runtime = std::sync::Arc::clone(state_runtime);
        //     let thread_id = self.metadata.conversation_id;
        //     let call_id = call_id.to_string();
        //     let tool_name = tool_name.to_string();
        //     let arguments = arguments.to_string();
        //     let result = Some(output.to_string());
        //     let duration_ms = duration.as_millis().min(i64::MAX as u128) as i64;
        //
        //     tokio::spawn(async move {
        //         let operation = jarvis_state::ToolOperationBuilder::new(
        //             thread_id,
        //             call_id,
        //             tool_name,
        //             arguments,
        ...
        //         .build();
        //
        //         if let Err(err) = state_runtime.insert_tool_operation(&operation).await {
        //             tracing::warn!("failed to persist tool operation: {err}");
        //         }
        //     });
        // }
    }
