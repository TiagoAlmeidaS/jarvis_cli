...
/// OpenTelemetry metrics client used by Jarvis.
#[derive(Clone, Debug)]
    ...
    pub fn new(config: MetricsConfig) -> Result<Self> {
        ...
        let (meter_provider, meter) = match exporter {
            ...
            MetricsExporter::Prometheus => {
                // For Prometheus, we use OTLP exporter pointing to a collector
                // In the future, this could be a direct Prometheus exporter
                return Err(MetricsError::InvalidConfig {
                    message: "Prometheus exporter requires OTLP configuration. Use Otlp exporter with Prometheus collector.".to_string(),
                });
            }
...
fn build_otlp_metric_exporter(
    ...
    match exporter {
        ...
        OtelExporter::Prometheus => {
            // Prometheus exporter is handled separately via PrometheusMetricsClient
