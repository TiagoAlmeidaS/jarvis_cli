...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    let (metrics, exporter) =
        build_metrics_with_defaults(&[("service", "Jarvis-cli"), ("env", "prod")])?;

    metrics.counter("Jarvis.turns", 1, &[("model", "gpt-5.1"), ("env", "dev")])?;
    metrics.histogram("Jarvis.tool_latency", 25, &[("tool", "shell")])?;
    metrics.shutdown()?;

    let resource_metrics = latest_metrics(&exporter);

    let counter = find_metric(&resource_metrics, "Jarvis.turns").expect("counter metric missing");
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "Jarvis.tool_latency");
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    metrics.counter("Jarvis.alpha", 1, &[("env", "dev"), ("component", "alpha")])?;
    metrics.counter(
        "Jarvis.beta",
        2,
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "Jarvis.alpha").expect("Jarvis.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "Jarvis.beta").expect("Jarvis.beta metric missing");
...
fn shutdown_flushes_in_memory_exporter() -> Result<()> {
    ...
    metrics.counter("Jarvis.turns", 1, &[])?;
    metrics.shutdown()?;
