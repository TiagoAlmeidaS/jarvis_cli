...
This repository uses Bazel to build the Rust workspace under `jarvis-rs`.
...
- `rules_rs` imports third-party crates from `jarvis-rs/Cargo.toml` and
  `jarvis-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
...
- Each crate in `jarvis-rs/*/BUILD.bazel` typically uses `codex_rust_crate` and
