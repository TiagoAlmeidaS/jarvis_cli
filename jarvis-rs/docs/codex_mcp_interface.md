# jarvis MCP Server Interface [experimental]

This document describes jarvis’s experimental MCP server interface: a JSON‑RPC API that runs over the Model Context Protocol (MCP) transport to control a local jarvis engine.

- Status: experimental and subject to change without notice
- Server binary: `jarvis mcp-server` (or `jarvis-mcp-server`)
- Transport: standard MCP over stdio (JSON‑RPC 2.0, line‑delimited)

## Overview

jarvis exposes a small set of MCP‑compatible methods to create and manage conversations, send user input, receive live events, and handle approval prompts. The types are defined in `protocol/src/mcp_protocol.rs` and re‑used by the MCP server implementation in `mcp-server/`.

At a glance:

- Conversations
  - `newConversation` → start a jarvis session
...
- Notifications (server → client)
  ...
  - `jarvis/event` stream with agent events

See code for full type definitions and exact shapes: `protocol/src/mcp_protocol.rs`.

## Starting the server

Run jarvis as an MCP server and connect an MCP client:

```bash
jarvis mcp-server | your_mcp_client
```
...
Use the separate `jarvis mcp` subcommand to manage configured MCP server launchers in `config.toml`.
...
- `model`: string model id (e.g. "o3", "gpt-5.1", "gpt-5.1-jarvis")
...
Fetch the catalog of models available in the current jarvis build with `model/list`. The request accepts optional pagination inputs:
...
- `jarvis/event` with the serialized jarvis event payload. The shape matches `core/src/protocol.rs`’s `Event` and `EventMsg` types. Some notifications include a `_meta.requestId` to correlate with the originating request.
...
The `jarvis` and `jarvis-reply` tools return standard MCP `CallToolResult` payloads. For
compatibility with MCP clients that prefer `structuredContent`, jarvis mirrors the
...
```json
{
  "content": [{ "type": "text", "text": "Hello from jarvis" }],
  "structuredContent": {
...
When jarvis needs approval to apply changes or run commands, the server issues JSON‑RPC requests to the client:
...
```json
{ "jsonrpc": "2.0", "id": 2, "method": "sendUserMessage", "params": { "conversationId": "c7b0…", "items": [{ "type": "text", "text": "Hello jarvis" }] } }
```

While processing, the server emits `jarvis/event` notifications containing agent output, approvals, and status updates.
