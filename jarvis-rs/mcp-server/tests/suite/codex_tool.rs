...
use jarvis_mcp_server::CodexToolCallParam;
use jarvis_mcp_server::ExecApprovalElicitRequestParams;
use jarvis_mcp_server::ExecApprovalResponse;
use jarvis_mcp_server::PatchApprovalElicitRequestParams;
use jarvis_mcp_server::PatchApprovalResponse;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(jarvis_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Jarvis sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "Jarvis" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let jarvis_request_id = mcp_process
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            jarvis_request_id.to_string(),
            params.jarvis_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `Jarvis` tool call completes and that the file was created.
    let jarvis_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(jarvis_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(jarvis_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    jarvis_mcp_tool_call_id: String,
    jarvis_event_id: String,
    thread_id: jarvis_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Jarvis to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let jarvis_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        ...
        jarvis_mcp_tool_call_id,
        jarvis_event_id,
        jarvis_command: command,
        jarvis_cwd: workdir.to_path_buf(),
        jarvis_call_id: "call1234".to_string(),
        jarvis_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "Jarvis" tool request that will trigger the apply_patch command
    let jarvis_request_id = mcp_process
        .send_codex_tool_call(CodexToolCallParam {
    ...
    // Verify the original `Jarvis` tool call completes
    let jarvis_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(jarvis_request_id)),
    )
...
async fn test_codex_tool_passes_base_instructions() {
    ...
    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = jarvis_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn jarvis_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `Jarvis mcp` with a specific config.toml.
    let jarvis_home = TempDir::new()?;
    create_config_toml(jarvis_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(jarvis_home.path()).await?;
    ...
    .await??;
    assert_eq!(jarvis_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(jarvis_response.id, RequestId::Number(jarvis_request_id));
    assert_eq!(
        ...
        json!({
            ...
            "structuredContent": {
                "threadId": jarvis_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("Jarvis tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
fn create_expected_patch_approval_elicitation_request_params(
    ...
    message_lines.push("Allow Jarvis to apply proposed code changes?".to_string());
    let params_json = serde_json::to_value(PatchApprovalElicitRequestParams {
        message: message_lines.join("\n"),
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        jarvis_elicitation: "patch-approval".to_string(),
        jarvis_mcp_tool_call_id,
        jarvis_event_id,
        jarvis_reason: reason,
        jarvis_grant_root: grant_root,
        jarvis_changes: changes,
        jarvis_call_id: "call1234".to_string(),
    })?;
...
/// Create a Jarvis config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(jarvis_home: &Path, server_uri: &str) -> std::io::Result<()> {
