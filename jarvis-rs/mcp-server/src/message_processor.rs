...
use jarvis_core::protocol::Submission;
...
use crate::jarvis_tool_config::CodexToolCallParam;
use crate::jarvis_tool_config::CodexToolCallReplyParam;
use crate::jarvis_tool_config::create_tool_for_codex_tool_call_param;
use crate::jarvis_tool_config::create_tool_for_codex_tool_call_reply_param;
...
impl MessageProcessor {
    ...
    async fn handle_initialize(
        ...
        let server_info = Implementation {
            name: "Jarvis-mcp-server".to_string(),
            title: Some("Jarvis".to_string()),
            version: env!("CARGO_PKG_VERSION").to_string(),
            icons: None,
            website_url: None,
        };

        // Preserve Jarvis's existing non-spec `serverInfo.user_agent` field.
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParam) {
        ...
        match name.as_ref() {
            "Jarvis" => self.handle_tool_call_codex(id, arguments).await,
            "Jarvis-reply" => {
    ...
    async fn handle_tool_call_codex(
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg
                    .into_config(self.jarvis_linux_sandbox_exe.clone())
                    .await
                {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![rmcp::model::Content::text(format!(
                                "Failed to load Jarvis configuration from overrides: {e}"
                            ))],
                            structured_content: None,
                            is_error: Some(true),
                            meta: None,
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![rmcp::model::Content::text(format!(
                            "Failed to parse configuration for Jarvis tool: {e}"
                        ))],
                        structured_content: None,
                        is_error: Some(true),
                        meta: None,
            ...
            None => {
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for Jarvis tool-call; the `prompt` field is required.",
                    )],
                    structured_content: None,
                    is_error: Some(true),
                    meta: None,
        ...
        // Spawn an async task to handle the Jarvis session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Jarvis session and stream events back to the client.
            crate::jarvis_tool_runner::run_codex_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
    ...
    async fn handle_tool_call_codex_session_reply(
        ...
        // parse arguments
        let jarvis_tool_call_reply_param: CodexToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<CodexToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Jarvis tool call reply parameters: {e}");
                    let result = CallToolResult {
                        content: vec![rmcp::model::Content::text(format!(
                            "Failed to parse configuration for Jarvis tool: {e}"
                        ))],
            ...
            None => {
                tracing::error!(
                    "Missing arguments for Jarvis-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for Jarvis-reply tool-call; the `thread_id` and `prompt` fields are required.",
        ...
        let thread_id = match jarvis_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let Jarvis = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::jarvis_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = jarvis_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_codex_uuid = running_requests_id_to_codex_uuid.clone();

            async move {
                crate::jarvis_tool_runner::run_codex_tool_session_reply(
                    thread_id,
                    Jarvis,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        // Obtain the Jarvis thread from the server.
        let jarvis_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Jarvis.
        if let Err(e) = jarvis_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: jarvis_core::protocol::Op::Interrupt,
            })
            .await
        {
            tracing::error!("Failed to submit interrupt to Jarvis: {e}");
            return;
        }
