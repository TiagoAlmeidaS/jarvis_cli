...
/// Client-supplied configuration for a `Jarvis` tool-call.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, Default)]
#[serde(rename_all = "kebab-case")]
pub struct CodexToolCallParam {
    /// The *initial user prompt* to start the Jarvis conversation.
    pub prompt: String,

    /// Optional override for the model name (e.g. 'gpt-5.2', 'gpt-5.2-Jarvis').
...
/// Builds a `Tool` definition (JSON schema etc.) for the Jarvis tool-call.
pub(crate) fn create_tool_for_codex_tool_call_param() -> Tool {
    ...
    let input_schema = create_tool_input_schema(schema, "Jarvis tool schema should serialize");

    Tool {
        name: "Jarvis".into(),
        title: Some("Jarvis".to_string()),
        input_schema,
        output_schema: Some(jarvis_tool_output_schema()),
        description: Some(
            "Run a Jarvis session. Accepts configuration parameters matching the Jarvis Config struct."
                .into(),
        ),
        annotations: None,
        icons: None,
...
fn jarvis_tool_output_schema() -> Arc<JsonObject> {
...
impl CodexToolCallParam {
    /// Returns the initial user prompt to start the Jarvis conversation and the
    ...
    pub async fn into_config(
        ...
        // Build the `ConfigOverrides` recognized by Jarvis-core.
...
pub struct CodexToolCallReplyParam {
    ...
    /// The thread id for this Jarvis session.
    ...
    /// The *next user prompt* to continue the Jarvis conversation.
    pub prompt: String,
}
...
/// Builds a `Tool` definition for the `Jarvis-reply` tool-call.
pub(crate) fn create_tool_for_codex_tool_call_reply_param() -> Tool {
    ...
    Tool {
        ...
        title: Some("Jarvis Reply".to_string()),
        input_schema,
        output_schema: Some(jarvis_tool_output_schema()),
        description: Some(
            "Continue a Jarvis conversation by providing the thread id and prompt.".into(),
        ),
        annotations: None,
        icons: None,
        meta: None,
...
mod tests {
    ...
    fn verify_codex_tool_json_schema() {
        ...
        let expected_tool_json = serde_json::json!({
          "description": "Run a Jarvis session. Accepts configuration parameters matching the Jarvis Config struct.",
          "inputSchema": {
            "properties": {
              "approval-policy": {
              ...
              "config": {
                ...
                "description": "Individual config settings that will override what is in jarvis_home/config.toml.",
                "type": "object"
              },
              "cwd": {
              ...
              "model": {
                "description": "Optional override for the model name (e.g. 'gpt-5.2', 'gpt-5.2-Jarvis').",
                "type": "string"
              },
              "profile": {
              ...
              "prompt": {
                "description": "The *initial user prompt* to start the Jarvis conversation.",
                "type": "string"
              },
              "sandbox": {
    ...
    fn verify_codex_tool_reply_json_schema() {
        ...
        let tool_json = serde_json::to_value(&tool).expect("tool serializes");
        let expected_tool_json = serde_json::json!({
          "description": "Continue a Jarvis conversation by providing the thread id and prompt.",
          "inputSchema": {
            "properties": {
              "conversationId": {
              ...
              "prompt": {
                "description": "The *next user prompt* to continue the Jarvis conversation.",
                "type": "string"
              },
              "threadId": {
                "description": "The thread id for this Jarvis session. This field is required, but we keep it optional here for backward compatibility for clients that still use conversationId.",
                "type": "string"
              }
            },
            "required": [
