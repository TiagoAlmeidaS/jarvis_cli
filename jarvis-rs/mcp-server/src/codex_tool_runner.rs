...
use jarvis_core::config::Config as CodexConfig;
...
/// To adhere to MCP `tools/call` response format, include the Jarvis
...
/// Run a complete Jarvis session and stream events back to the client.
///
...
pub async fn run_codex_tool_session(
    ...
    // Use the original MCP request ID as the `sub_id` for the Jarvis submission so that
            ...
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(jarvis_event_to_notification(&event)) above has
