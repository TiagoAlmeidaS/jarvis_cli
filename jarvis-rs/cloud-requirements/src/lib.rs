...
//! customers.
//!
//! Today, fetching is best-effort: on error or timeout, Jarvis continues without cloud requirements.
//! We expect to tighten this so that Enterprise ChatGPT customers must successfully fetch these
//! requirements before Jarvis will run.
...
/// This blocks Jarvis startup, so must be short.
...
trait RequirementsFetcher: Send + Sync {
    ...
    /// Result.
    async fn fetch_requirements(&self, auth: &JarvisAuth) -> Option<String>;
}
...
impl CloudRequirementsService {
    ...
    async fn fetch_with_timeout(&self) -> Option<ConfigRequirementsToml> {
        let _timer =
            jarvis_otel::start_global_timer("Jarvis.cloud_requirements.fetch.duration_ms", &[]);
...
mod tests {
    ...
    impl RequirementsFetcher for StaticFetcher {
        async fn fetch_requirements(&self, _auth: &JarvisAuth) -> Option<String> {
            self.contents.clone()
        }
    }
