...
pub(crate) fn subagent_header(source: &Option<SessionSource>) -> Option<String> {
    ...
    match sub {
        jarvis_protocol::protocol::SubAgentSource::Review => Some("review".to_string()),
        jarvis_protocol::protocol::SubAgentSource::Compact => Some("compact".to_string()),
        jarvis_protocol::protocol::SubAgentSource::ThreadSpawn { .. } => {
            Some("collab_spawn".to_string())
        }
        jarvis_protocol::protocol::SubAgentSource::Other(label) => Some(label.clone()),
    }
}
