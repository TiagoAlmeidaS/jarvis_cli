...
async fn accept_elicitation_for_prompt_rule() -> Result<()> {
    ...
    // $jarvis_home with an execpolicy that prompts for `git init` commands.
...
"#,
    ...
    let transport = create_transport(jarvis_home.as_ref(), dotslash_cache).await?;
    ...
    let jarvis_linux_sandbox_exe = if cfg!(target_os = "linux") {
        let jarvis_linux_sandbox_exe = linux_sandbox_exe_folder.path().join("Jarvis-linux-sandbox");
        let jarvis_cli = ensure_codex_cli()?;
        symlink(&jarvis_cli, &jarvis_linux_sandbox_exe)?;
        Some(jarvis_linux_sandbox_exe)
    } else {
        None
    };
    let response =
        notify_readable_sandbox(&project_root_path, jarvis_linux_sandbox_exe, &service).await?;
...
fn ensure_codex_cli() -> Result<PathBuf> {
    let jarvis_cli = jarvis_utils_cargo_bin::cargo_bin("Jarvis")?;

    let metadata = jarvis_cli.metadata().with_context(|| {
        format!(
            "failed to read metadata for Jarvis binary at {}",
            jarvis_cli.display()
        )
    })?;
    ensure!(
        metadata.is_file(),
        "expected Jarvis binary at {} to be a file; run `cargo build -p Jarvis-cli --bin Jarvis` before this test",
        jarvis_cli.display()
    );
    ...
    let mode = metadata.permissions().mode();
        ...
        mode & 0o111 != 0,
        "Jarvis binary at {} is not executable (mode {mode:o}); run `cargo build -p Jarvis-cli --bin Jarvis` before this test",
        jarvis_cli.display()
    );
