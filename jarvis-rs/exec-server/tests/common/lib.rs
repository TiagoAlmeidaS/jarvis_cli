use jarvis_core::MCP_SANDBOX_STATE_METHOD;
use jarvis_core::SandboxState;
...
where
    P: AsRef<Path>,
{
    let mcp_executable = jarvis_utils_cargo_bin::cargo_bin("Jarvis-exec-mcp-server")?;
    let execve_wrapper = jarvis_utils_cargo_bin::cargo_bin("Jarvis-execve-wrapper")?;
    ...
    let transport = TokioChildProcess::new(Command::new(&mcp_executable).configure(|cmd| {
        ...
        cmd.env("jarvis_home", jarvis_home.as_ref());
...
pub async fn write_default_execpolicy<P>(policy: &str, jarvis_home: P) -> anyhow::Result<()>
where
    P: AsRef<Path>,
{
    let policy_dir = jarvis_home.as_ref().join("rules");
