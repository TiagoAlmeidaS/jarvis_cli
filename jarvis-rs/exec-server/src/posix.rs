...
//! from us, and we give its fd number in the jarvis_ESCALATE_SOCKET environment variable.
//!
...
use jarvis_core::is_dangerous_command::command_might_be_dangerous;
...
const jarvis_EXECVE_WRAPPER_EXE_NAME: &str = "Jarvis-execve-wrapper";
...
pub async fn main_mcp_server() -> anyhow::Result<()> {
    ...
    let execve_wrapper = match cli.execve_wrapper {
        ...
        None => {
            ...
            cwd.parent()
                .map(|p| p.join(jarvis_EXECVE_WRAPPER_EXE_NAME))
                .ok_or_else(|| {
...
async fn load_exec_policy() -> anyhow::Result<Policy> {
    let jarvis_home = find_jarvis_home().context("failed to resolve jarvis_home for execpolicy")?;

    // TODO(mbolin): At a minimum, `cwd` should be configurable via
    // `Jarvis/sandbox-state/update` or some other custom MCP call.
    let cwd = None;
    let cli_overrides = Vec::new();
    let overrides = jarvis_core::config_loader::LoaderOverrides::default();
    let config_layer_stack = jarvis_core::config_loader::load_config_layers_state(
        &jarvis_home,
        cwd,
        &cli_overrides,
        overrides,
        jarvis_core::config_loader::CloudRequirementsLoader::default(),
    )
    .await?;

    jarvis_core::load_exec_policy(&config_layer_stack)
        .await
