...
use anyhow::Result;
use jarvis_core::MCP_SANDBOX_STATE_CAPABILITY;
...
const jarvis_BASH_PATH_ENV_VAR: &str = "jarvis_BASH_PATH";

const SANDBOX_STATE_CAPABILITY_VERSION: &str = "1.0.0";

pub(crate) fn get_bash_path() -> Result<PathBuf> {
    std::env::var(jarvis_BASH_PATH_ENV_VAR)
        .map(PathBuf::from)
        .context(format!("{jarvis_BASH_PATH_ENV_VAR} must be set"))
}
...
impl ExecTool {
    ...
    async fn shell(
        ...
        let effective_timeout = Duration::from_millis(
            params
                .timeout_ms
                .unwrap_or(jarvis_core::exec::DEFAULT_EXEC_COMMAND_TIMEOUT_MS),
        );
