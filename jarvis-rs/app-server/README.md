...
`jarvis app-server` is the interface jarvis uses to power rich interfaces such as the [jarvis VS Code extension](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt).
...
Similar to [MCP](https://modelcontextprotocol.io/), `jarvis app-server` supports bidirectional communication, streaming JSONL over stdio. The protocol is JSON-RPC 2.0, though the `"jsonrpc":"2.0"` header is omitted.

## Message Schema

Currently, you can dump a TypeScript version of the schema using `jarvis app-server generate-ts`, or a JSON Schema bundle via `jarvis app-server generate-json-schema`. Each output is specific to the version of jarvis you used to run the command, so the generated artifacts are guaranteed to match that version.

```
jarvis app-server generate-ts --out DIR
jarvis app-server generate-json-schema --out DIR
```

## Core Primitives

The API exposes three top level primitives representing an interaction between a user and jarvis:

- **Thread**: A conversation between a user and the jarvis agent. Each thread contains multiple turns.
...
- Initialize once: Immediately after launching the jarvis app-server process, send an `initialize` request with your client metadata, then emit an `initialized` notification. Any other request before this handshake gets rejected.
...
Applications building on top of `jarvis app-server` should identify themselves via the `clientInfo` parameter.

**Important**: `clientInfo.name` is used to identify the client for the OpenAI Compliance Logs Platform. If
you are developing a new jarvis integration that is intended for enterprise use, please contact us to get it
added to a known clients list. For more context: https://chatgpt.com/admin/api-reference#tag/Logs:-jarvis
...
```json
{
  ...
  "params": {
    "clientInfo": {
      "name": "codex_vscode",
      "title": "jarvis VS Code Extension",
      "version": "0.1.0"
    }
  }
}
...
- `turn/start` — add user input to a thread and begin jarvis generation; responds with the initial `turn` object and streams `turn/started`, `item/*`, and `turn/completed` notifications.
- `turn/interrupt` — request cancellation of an in-flight turn by `(thread_id, turn_id)`; success is an empty `{}` response and the turn finishes with `status: "interrupted"`.
- `review/start` — kick off jarvis’s automated reviewer for a thread; responds like `turn/start` and emits `item/started`/`item/completed` notifications with `enteredReviewMode` and `exitedReviewMode` items, plus a final assistant `agentMessage` containing the review.
...
- `skills/remote/write` — download a public remote skill by `hazelnutId`; `isPreload=true` writes to `.jarvis/vendor_imports/skills` under `codex_home` (**under development; do not call from production clients yet**).
...
Start a fresh thread when you need a new jarvis conversation.
...
Turns attach user input (text or images) to a thread and trigger jarvis generation. The `input` field is a list of discriminated unions:
...
{ "method": "turn/start", "id": 33, "params": {
    ...
    "input": [
        ...
        { "type": "skill", "name": "skill-creator", "path": "/Users/me/.jarvis/skills/skill-creator/SKILL.md" }
    ]
} }
{ "id": 33, "result": { "turn": {
    "id": "turn_457",
...
The server requests cancellations for running subprocesses, then emits a `turn/completed` event with `status: "interrupted"`. Rely on the `turn/completed` to know when jarvis-side cleanup is done.

### Example: Request a code review

Use `review/start` to run jarvis’s reviewer on the currently checked-out project. The request takes the thread id plus a `target` describing what should be reviewed:

- `{"type":"uncommittedChanges"}` — staged, unstaged, and untracked files.
- `{"type":"baseBranch","branch":"main"}` — diff against the provided branch’s upstream (see prompt for the exact `git merge-base`/`git diff` instructions jarvis will run).
...
jarvis streams the usual `turn/started` notification followed by an `item/started`
...
- For clients that are already sandboxed externally, set `sandboxPolicy` to `{"type":"externalSandbox","networkAccess":"enabled"}` (or omit `networkAccess` to keep it restricted). jarvis will not enforce its own sandbox in this mode; it tells the model it has full file-system access and passes the `networkAccess` state through `environment_context`.
...
- `contextCompaction` — `{id}` emitted when jarvis compacts the conversation history. This can happen automatically.
- `compacted` - `{threadId, turnId}` when jarvis compacts the conversation history. This can happen automatically. **Deprecated:** Use `contextCompaction` instead.
...
Certain actions (shell commands or modifying files) may require explicit user approval depending on the user's config. When `turn/start` is used, the app-server drives an approval flow by sending a server-initiated JSON-RPC request to the client. The client must respond to tell jarvis whether to proceed. UIs should present these requests inline with the active turn so users can review the proposed command or diff before choosing.
  ...
  "params": {
    ...
    "input": [
      ...
      },
      {
        "type": "skill",
        "name": "skill-creator",
        "path": "/Users/me/.jarvis/skills/skill-creator/SKILL.md"
      }
    ]
  }
}
...
{ "id": 25, "result": {
    "data": [{
        ...
        "skills": [
            {
              "name": "skill-creator",
              "description": "Create or update a jarvis skill",
              "enabled": true,
              "interface": {
                "displayName": "Skill Creator",
                "shortDescription": "Create or update a jarvis skill",
                "iconSmall": "icon.svg",
...
jarvis supports these authentication modes. The current mode is surfaced in `account/updated` (`authMode`) and can be inferred from `account/read`.

- **API key (`apiKey`)**: Caller supplies an OpenAI API key via `account/login/start` with `type: "apiKey"`. The API key is saved and used for API requests.
- **ChatGPT managed (`chatgpt`)** (recommended): jarvis owns the ChatGPT OAuth flow and refresh tokens. Start via `account/login/start` with `type: "chatgpt"`; jarvis persists tokens to disk and refreshes them automatically.
...
- `requiresOpenaiAuth` reflects the active provider; when `false`, jarvis can run without OpenAI credentials.
...
5. Verify the protocol crate:
   ...
   ```bash
   cargo test -p jarvis-app-server-protocol
   ```
