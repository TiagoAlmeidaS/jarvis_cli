...
const jarvis_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Jarvis, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_returns_original_thread() -> Result<()> {
    ...
    let start_id = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("gpt-5.1-Jarvis-max".to_string()),
            ..Default::default()
        })
        .await?;
...
async fn thread_resume_without_overrides_does_not_change_updated_at_or_mtime() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let jarvis_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(jarvis_home.path(), &server.uri())?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(jarvis_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
fn create_config_toml(jarvis_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = jarvis_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
model = "gpt-5.2-Jarvis"
...
fn setup_rollout_fixture(jarvis_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(jarvis_home, server_uri)?;
    ...
    let rollout_file_path = rollout_path(jarvis_home, filename_ts, &conversation_id);
