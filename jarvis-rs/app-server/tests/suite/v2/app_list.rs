...
async fn list_apps_returns_connectors_with_accessible_flags() -> Result<()> {
    ...
    let jarvis_home = TempDir::new()?;
    write_connectors_config(jarvis_home.path(), &server_url)?;
    write_chatgpt_auth(
        jarvis_home.path(),
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/Jarvis/apps", mcp_service);
...
fn write_connectors_config(jarvis_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
