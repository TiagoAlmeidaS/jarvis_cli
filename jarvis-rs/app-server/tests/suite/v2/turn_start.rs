...
use jarvis_app_server_protocol::ByteRange;
...
use jarvis_app_server_protocol::PatchChangeKind;
use jarvis_app_server_protocol::RequestId;
use jarvis_app_server_protocol::ServerRequest;
use jarvis_app_server_protocol::TextElement;
...
use jarvis_core::protocol_config_types::ReasoningSummary;
...
const TEST_ORIGINATOR: &str = "jarvis_vscode";
...
async fn turn_start_sends_originator_header() -> Result<()> {
    ...
    let mut mcp = McpProcess::new(jarvis_home.path()).await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: TEST_ORIGINATOR.to_string(),
            title: Some("Jarvis VS Code Extension".to_string()),
...
async fn turn_start_emits_notifications_and_accepts_model_override() -> Result<()> {
    ...
    // Three Jarvis turns hit the mock model (session start + two turn/start calls).
    let responses = vec![
    ...
    assert_eq!(
        started.turn.status,
        jarvis_app_server_protocol::TurnStatus::InProgress
    );
...
async fn turn_start_accepts_collaboration_mode_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("gpt-5.2-Jarvis".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_accepts_personality_override_v2() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(ThreadStartParams {
            model: Some("exp-Jarvis-personality".to_string()),
            ..Default::default()
        })
        .await?;
    let thread_resp: JSONRPCResponse = timeout(
...
async fn turn_start_accepts_local_image_input() -> Result<()> {
    // Two Jarvis turns hit the mock model (session start + turn/start).
    let responses = vec![
        create_final_assistant_message_sse_response("Done")?,
        create_final_assistant_message_sse_response("Done")?,
    ];
    ...
    let image_path = jarvis_home.path().join("image.png");
...
async fn turn_start_exec_approval_toggle_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let jarvis_home = tmp.path().to_path_buf();
    ...
    let mut mcp = McpProcess::new(jarvis_home.as_path()).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    mcp.send_response(
        request_id,
        serde_json::json!({ "decision": jarvis_core::protocol::ReviewDecision::Approved }),
    )
    .await?;
    timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("Jarvis/event/task_complete"),
    )
    .await??;
    timeout(
        DEFAULT_READ_TIMEOUT,
    ...
    let second_turn_id = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(jarvis_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(jarvis_app_server_protocol::SandboxPolicy::DangerFullAccess),
            model: Some("mock-model".to_string()),
...
async fn turn_start_updates_sandbox_and_cwd_between_turns_v2() -> Result<()> {
    ...
    let tmp = TempDir::new()?;
    let jarvis_home = tmp.path().join("jarvis_home");
    std::fs::create_dir(&jarvis_home)?;
    ...
    let mut mcp = McpProcess::new(&jarvis_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, mcp.initialize()).await??;
    ...
    let first_turn = mcp
        .send_turn_start_request(TurnStartParams {
            ...
            approval_policy: Some(jarvis_app_server_protocol::AskForApproval::Never),
            sandbox_policy: Some(jarvis_app_server_protocol::SandboxPolicy::WorkspaceWrite {
...
"#;
    ...
    pretty_assertions::assert_eq!(
        started_changes,
        vec![jarvis_app_server_protocol::FileUpdateChange {
...
fn create_config_toml(
    ...
    let feature_entries = features
        ...
        .collect::<Vec<_>>()
        .join("\n");
    let config_toml = jarvis_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
