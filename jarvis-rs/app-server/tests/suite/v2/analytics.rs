...
use jarvis_core::config::types::OtelExporterKind;
use jarvis_core::config::types::OtelHttpProtocol;
...
fn set_metrics_exporter(config: &mut jarvis_core::config::Config) {
...
async fn app_server_default_analytics_disabled_without_flag() -> Result<()> {
    ...
    let provider = jarvis_core::otel_init::build_provider(
        &config,
        SERVICE_VERSION,
        Some("jarvis_app_server"),
        false,
    )
