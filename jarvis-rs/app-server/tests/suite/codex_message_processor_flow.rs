...
use jarvis_core::spawn::jarvisSANDBOX_NETWORK_DISABLED_ENV_VAR;
...
async fn test_codex_jsonrpc_conversation_flow() -> Result<()> {
    if env::var(jarvisSANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
    ...
    let tmp = TempDir::new()?;
    // Temporary Jarvis home with config pointing at the mock server.
    ...
    create_config_toml(&jarvis_home, &server.uri())?;
    ...
    let send_user_id = mcp
        .send_send_user_message_request(SendUserMessageParams {
            conversation_id,
            items: vec![jarvis_app_server_protocol::InputItem::Text {
                text: "text".to_string(),
    ...
    let task_started_notification: JSONRPCNotification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("Jarvis/event/task_started"),
    )
    .await??;
...
async fn test_send_user_turn_updates_sandbox_and_cwd_between_turns() -> Result<()> {
    ...
    let exec_begin_notification = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.read_stream_until_notification_message("Jarvis/event/exec_command_begin"),
    )
    .await??;
