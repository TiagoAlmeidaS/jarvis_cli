...
use jarvis_app_server_protocol::CancelLoginChatGptParams;
use jarvis_app_server_protocol::ClientInfo;
use jarvis_app_server_protocol::ClientNotification;
use jarvis_app_server_protocol::CollaborationModeListParams;
use jarvis_app_server_protocol::ConfigBatchWriteParams;
use jarvis_app_server_protocol::ConfigReadParams;
use jarvis_app_server_protocol::ConfigValueWriteParams;
...
use jarvis_app_server_protocol::JSONRPCError;
...
use jarvis_core::default_client::jarvis_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
pub const DEFAULT_CLIENT_NAME: &str = "Jarvis-app-server-tests";

impl McpProcess {
    pub async fn new(jarvis_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(jarvis_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = jarvis_utils_cargo_bin::cargo_bin("Jarvis-app-server")
            .context("should find binary for Jarvis-app-server")?;
        ...
        cmd.env_remove(jarvis_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR);
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("Jarvis-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
