...
use jarvis_app_server_protocol::TextPosition as AppTextPosition;
use jarvis_app_server_protocol::TextRange as AppTextRange;
use jarvis_core::ExecPolicyError;
use jarvis_core::check_execpolicy_for_warnings;
use jarvis_core::config_loader::ConfigLoadError;
use jarvis_core::config_loader::TextRange as CoreTextRange;
...
// Re-export with jarvis_ prefix for compatibility
pub use codex_message_processor as jarvismessage_processor;
mod config_api;
mod dynamic_tools;
mod error_code;
mod filters;
...
pub async fn run_main(
        ...
        .await
    {
        Ok(config) => {
            ...
            match effective_toml.try_into() {
                Ok(config_toml) => {
                    if let Err(err) = jarvis_core::personality_migration::maybe_migrate_personality(
                        &config.jarvis_home,
                        &config_toml,
                    )
                    .await
    ...
    let otel = jarvis_core::otel_init::build_provider(
        &config,
        env!("CARGO_PKG_VERSION"),
        Some("jarvisapp_server"),
