...
use jarvis_app_server_protocol::ConfigRequirements;
use jarvis_app_server_protocol::ConfigRequirementsReadResponse;
use jarvis_app_server_protocol::ConfigValueWriteParams;
use jarvis_app_server_protocol::ConfigWriteErrorCode;
...
use jarvis_core::config::ConfigServiceError;
use jarvis_core::config_loader::CloudRequirementsLoader;
use jarvis_core::config_loader::ConfigRequirementsToml;
use jarvis_core::config_loader::LoaderOverrides;
use jarvis_core::config_loader::ResidencyRequirement as CoreResidencyRequirement;
use jarvis_core::config_loader::SandboxModeRequirement as CoreSandboxModeRequirement;
use serde_json::json;
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(jarvis_app_server_protocol::AskForApproval::from)
                .collect()
        }),
...
) -> jarvis_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => jarvis_app_server_protocol::ResidencyRequirement::Us,
    }
}
...
mod tests {
    ...
    fn map_requirements_toml_to_api_converts_core_enums() {
        ...
        assert_eq!(
            ...
            Some(vec![
                jarvis_app_server_protocol::AskForApproval::Never,
                jarvis_app_server_protocol::AskForApproval::OnRequest,
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(jarvis_app_server_protocol::ResidencyRequirement::Us),
        );
    }
}
