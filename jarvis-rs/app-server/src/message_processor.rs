...
use jarvis_app_server_protocol::ChatgptAuthTokensRefreshReason;
use jarvis_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
use jarvis_app_server_protocol::ConfigWarningNotification;
use jarvis_app_server_protocol::ExperimentalApi;
...
use jarvis_app_server_protocol::experimental_required_message;
use jarvis_core::AuthManager;
use jarvis_core::ThreadManager;
use jarvis_core::auth::ExternalAuthRefreshContext;
use jarvis_core::auth::ExternalAuthRefreshReason;
use jarvis_core::auth::ExternalAuthRefresher;
use jarvis_core::auth::ExternalAuthTokens;
...
use jarvis_core::default_client::SetOriginatorError;
use jarvis_core::default_client::USER_AGENT_SUFFIX;
...
use jarvis_core::default_client::set_default_originator;
...
pub(crate) struct MessageProcessor {
    outgoing: Arc<OutgoingMessageSender>,
    jarvismessage_processor: CodexMessageProcessor,
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let jarvismessage_processor = CodexMessageProcessor::new(CodexMessageProcessorArgs {
            auth_manager,
            thread_manager,
            outgoing: outgoing.clone(),
            jarvislinux_sandbox_exe,
    ...
    pub(crate) async fn process_request(&mut self, request: JSONRPCRequest) {
        ...
        let jarvisrequest = match serde_json::from_value::<ClientRequest>(request_json) {
            Ok(jarvisrequest) => jarvisrequest,
            Err(err) => {
                let error = JSONRPCErrorError {
                    code: INVALID_REQUEST_ERROR_CODE,
                    message: format!("Invalid request: {err}"),
                    ...
                    if let Err(error) = set_default_originator(name.clone()) {
                        match error {
                            ...
                            SetOriginatorError::AlreadyInitialized => {
                                ...
                                // TODO(owen): Once we remove support for jarvisINTERNAL_ORIGINATOR_OVERRIDE,
        ...
        if let Some(reason) = jarvisrequest.experimental_reason()
        ...
        match jarvisrequest {
            ...
            other => {
                self.jarvismessage_processor.process_request(other).await;
            }
        }
    }
    ...
    pub(crate) fn thread_created_receiver(&self) -> broadcast::Receiver<ThreadId> {
        self.jarvismessage_processor.thread_created_receiver()
    }
