...
/// Regression test for https://github.com/openai/Jarvis/issues/8803.
#[tokio::test]
...
"#,
    ...
    std::fs::write(jarvis_home.join("config.toml"), config_contents)?;

    let CodexCliOutput { exit_code, output } = run_codex_cli(jarvis_home, cwd).await?;
    assert_ne!(0, exit_code, "Jarvis CLI should exit nonzero.");
    assert!(
        output.contains("ERROR: Failed to initialize Jarvis:"),
...
async fn run_codex_cli(
    jarvis_home: impl AsRef<Path>,
    cwd: impl AsRef<Path>,
) -> anyhow::Result<CodexCliOutput> {
    let jarvis_cli = jarvis_utils_cargo_bin::cargo_bin("Jarvis")?;
    let mut env = HashMap::new();
    env.insert(
        "jarvis_home".to_string(),
        jarvis_home.as_ref().display().to_string(),
    );

    let args = vec!["-c".to_string(), "analytics.enabled=false".to_string()];
    let spawned = jarvis_utils_pty::spawn_pty_process(
        jarvis_cli.to_string_lossy().as_ref(),
        &args,
        cwd.as_ref(),
        &env,
        &None,
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            ...
            anyhow::bail!("timed out waiting for Jarvis CLI to exit");
        }
    };
