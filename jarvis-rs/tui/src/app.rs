...
use jarvis_core::config_loader::ConfigLayerStackOrdering;
use jarvis_core::features::Feature;
use jarvis_core::models_manager::manager::RefreshStrategy;
use jarvis_core::models_manager::model_presets::HIDE_GPT_5_1_CODEX_MAX_MIGRATION_PROMPT_CONFIG;
use jarvis_core::models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
use jarvis_core::protocol::AskForApproval;
use jarvis_core::protocol::Event;
use jarvis_core::protocol::EventMsg;
use jarvis_core::protocol::FinalOutput;
...
use jarvis_core::protocol::SkillErrorInfo;
...
use jarvis_protocol::openai_models::ModelUpgrade;
...
fn session_summary(
    ...
    let resume_command = jarvis_core::util::resume_command(thread_name.as_deref(), thread_id);
    Some(SessionSummary {
        usage_line,
        resume_command,
    })
...
impl App {
    ...
    async fn rebuild_config_for_cwd(&self, cwd: PathBuf) -> Result<Config> {
        ...
        ConfigBuilder::default()
            .jarvis_home(self.config.jarvis_home.clone())
    ...
    async fn select_agent_thread(&mut self, tui: &mut tui::Tui, thread_id: ThreadId) -> Result<()> {
        ...
        let jarvis_op_tx = crate::chatwidget::spawn_op_forwarder(thread);
        self.chat_widget = ChatWidget::new_with_op_sender(init, jarvis_op_tx);
    ...
    pub async fn run(
        ...
        let feedback_audience = if auth_ref
            .and_then(JarvisAuth::get_account_email)
        ...
        let otel_manager = OtelManager::new(
            ...
            model.as_str(),
            model.as_str(),
            auth_ref.and_then(JarvisAuth::get_account_id),
            auth_ref.and_then(JarvisAuth::get_account_email),
            auth_ref.map(JarvisAuth::api_auth_mode),
            config.otel.log_user_prompt,
            jarvis_core::terminal::user_agent(),
            SessionSource::Cli,
        );
        ...
        #[cfg(target_os = "windows")]
        {
            let should_check = WindowsSandboxLevel::from_config(&app.config)
                ...
                && matches!(
                    ...
                    jarvis_core::protocol::SandboxPolicy::WorkspaceWrite { .. }
                        | jarvis_core::protocol::SandboxPolicy::ReadOnly
                )
                && !app
                    .config
                    .notices
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.jarvis_home.clone();
        ...
        match event {
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let jarvis_home = self.config.jarvis_home.clone();
                    let tx = self.app_event_tx.clone();
                    ...
                    tokio::task::spawn_blocking(move || {
                        let result = jarvis_core::windows_sandbox::run_elevated_setup(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
                        ...
                        let event = match result {
                            Ok(()) => {
                                otel_manager.counter(
                                    "Jarvis.windows_sandbox.elevated_setup_success",
                                    1,
                                    &[],
                                );
                            ...
                            Err(err) => {
                                ...
                                if let Some((code, message)) =
                                    jarvis_core::windows_sandbox::elevated_setup_failure_details(
                                        &err,
                                    )
                                {
                                    code_tag = Some(code);
                                ...
                                otel_manager.counter(
                                    jarvis_core::windows_sandbox::elevated_setup_failure_metric_name(
                                        &err,
                                    ),
                                    1,
                                    &tags,
            ...
            AppEvent::EnableWindowsSandboxForAgentMode { preset, mode } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let mut builder =
                        ConfigEditsBuilder::new(&self.config.jarvis_home).with_profile(profile);
                    if elevated_enabled {
            ...
            AppEvent::UpdateSandboxPolicy(policy) => {
                ...
                #[cfg(target_os = "windows")]
                if !matches!(&policy, jarvis_core::protocol::SandboxPolicy::ReadOnly)
                    || WindowsSandboxLevel::from_config(&self.config)
                        != WindowsSandboxLevel::Disabled
                {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.jarvis_home.clone();
            ...
            AppEvent::UpdateFeatureFlags { updates } => {
                ...
                let mut builder = ConfigEditsBuilder::new(&self.config.jarvis_home)
            ...
            AppEvent::PersistFullAccessWarningAcknowledged => {
                if let Err(err) = ConfigEditsBuilder::new(&self.config.jarvis_home)
    ...
    fn reasoning_label_for(
        ...
        (!model.starts_with("Jarvis-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }

    pub(crate) fn token_usage(&self) -> jarvis_core::protocol::TokenUsage {
        self.chat_widget.token_usage()
    }
        ...
        let editor_cmd = match external_editor::resolve_editor_command() {
            ...
            Err(external_editor::EditorError::MissingEditor) => {
                self.chat_widget
                    ...
                    "Cannot open external editor: set $VISUAL or $EDITOR before starting Jarvis."
                        .to_string(),
                ));
    ...
    fn spawn_world_writable_scan(
        ...
        sandbox_policy: jarvis_core::protocol::SandboxPolicy,
        tx: AppEventSender,
    ) {
        tokio::task::spawn_blocking(move || {
            let result = jarvis_windows_sandbox::apply_world_writable_scan_and_denies(
                &logs_base_dir,
                &cwd,
                &env_map,
                &sandbox_policy,
...
mod tests {
    ...
    async fn make_test_app() -> App {
        ...
        let server = Arc::new(ThreadManager::with_models_provider(
            JarvisAuth::from_api_key("Test API Key"),
    ...
    fn all_model_presets() -> Vec<ModelPreset> {
        jarvis_core::models_manager::model_presets::all_model_presets().clone()
    }
    ...
    async fn model_migration_prompt_skips_when_target_missing() {
        ...
        let mut current = available
            .iter()
            .find(|preset| preset.model == "gpt-5-Jarvis")
            .cloned()
        ...
        available.retain(|preset| preset.model != "gpt-5-Jarvis");
    ...
    async fn model_migration_prompt_shows_for_hidden_model() {
        let jarvis_home = tempdir().expect("temp Jarvis home");
        ...
        let current = available_models
            .iter()
            .find(|preset| preset.model == "gpt-5.1-Jarvis")
            .cloned()
            .expect("gpt-5.1-Jarvis preset present");
        assert!(
            !current.show_in_picker,
            "expected gpt-5.1-Jarvis to be hidden from picker for this test"
        );
    ...
    async fn session_summary_includes_resume_hint() {
        ...
        assert_eq!(
            ...
            Some("Jarvis resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
    ...
    async fn session_summary_prefers_name_over_id() {
        ...
        assert_eq!(
            summary.resume_command,
            Some("Jarvis resume my-session".to_string())
        );
    }
}
