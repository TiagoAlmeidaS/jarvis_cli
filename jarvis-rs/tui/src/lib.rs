...
use jarvis_core::config::load_config_as_toml_with_cli_overrides;
use jarvis_core::config::resolve_oss_provider;
use jarvis_core::config_loader::CloudRequirementsLoader;
use jarvis_core::config_loader::ConfigLoadError;
use jarvis_core::config_loader::format_config_error_with_source;
use jarvis_core::default_client::set_default_client_residency_requirement;
use jarvis_core::find_thread_path_by_id_str;
use jarvis_core::find_thread_path_by_name_str;
use jarvis_core::path_utils;
use jarvis_core::protocol::AskForApproval;
use jarvis_core::read_session_meta_line;
use jarvis_core::terminal::Multiplexer;
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = jarvis_common::CliConfigOverrides { raw_overrides };
    ...
    #[allow(clippy::print_stderr)]
    let jarvis_home = match find_jarvis_home() {
        Ok(jarvis_home) => jarvis_home.to_path_buf(),
        Err(err) => {
            eprintln!("Error finding Jarvis home: {err}");
            std::process::exit(1);
        }
    };
    ...
    if let Err(err) =
        jarvis_core::personality_migration::maybe_migrate_personality(&jarvis_home, &config_toml)
            .await
    {
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&jarvis_home).await?;
    ...
    let log_dir = jarvis_core::config::log_dir(&config)?;
    ...
    // Jarvis CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("Jarvis-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for Jarvis crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("jarvis_core=info,jarvis_tui=info,jarvis_rmcp_client=info")
        })
    };
    ...
    let feedback = jarvis_feedback::CodexFeedback::new();
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        jarvis_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"), None, true)
    })) {
        Ok(Ok(otel)) => otel,
        Ok(Err(e)) => {
            #[allow(clippy::print_stderr)]
...
async fn run_ratatui_app(
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: jarvis_core::protocol::TokenUsage::default(),
                        thread_id: None,
                        thread_name: None,
    ...
    let auth_manager = AuthManager::shared(
        initial_config.jarvis_home.clone(),
        false,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `Jarvis {action}` without an ID to choose from existing sessions."
            )),
        })
    };
    ...
    let session_selection = if use_fork {
        if let Some(id_str) = cli.fork_session_id.as_deref() {
            ...
            let path = if is_uuid {
                find_thread_path_by_id_str(&config.jarvis_home, id_str).await?
            } else {
                find_thread_path_by_name_str(&config.jarvis_home, id_str).await?
            };
            match path {
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        match JarvisAuth::from_auth_storage(&jarvis_home, config.cli_auth_credentials_store_mode) {
...
mod tests {
    ...
    use jarvis_core::config::ConfigBuilder;
    use jarvis_core::config::ConfigOverrides;
    use jarvis_core::config::ProjectConfig;
    use jarvis_core::protocol::AskForApproval;
    use jarvis_protocol::protocol::RolloutItem;
    use jarvis_protocol::protocol::RolloutLine;
    ...
    async fn build_config(temp_dir: &TempDir) -> std::io::Result<Config> {
        ConfigBuilder::default()
            .jarvis_home(temp_dir.path().to_path_buf())
            .build()
            .await
    }
