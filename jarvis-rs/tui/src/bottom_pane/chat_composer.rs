...
//!
//! For the detailed burst state machine, see `Jarvis-rs/tui/src/bottom_pane/paste_burst.rs`.
...
use jarvis_common::fuzzy_match::fuzzy_match;
use jarvis_protocol::custom_prompts::CustomPrompt;
use jarvis_protocol::custom_prompts::PROMPTS_CMD_PREFIX;
...
use jarvis_chatgpt::connectors::AppInfo;
    ...
    fn mention_items(&self) -> Vec<MentionItem> {
            ...
            && let Some(snapshot) = self.connectors_snapshot.as_ref()
        {
            for connector in &snapshot.connectors {
                ...
                let slug = jarvis_core::connectors::connector_mention_slug(connector);
...
mod tests {
    ...
    fn footer_hint_row_is_separated_from_composer() {
        ...
        let composer = ChatComposer::new(
            ...
            sender,
            false,
            "Ask Jarvis to do anything".to_string(),
            false,
        );
...
程序员 与 Unicode 同行";
    ...
    fn pasting_filepath_attaches_image() {
        ...
        let tmp_path: PathBuf = tmp.path().join("jarvis_tui_test_paste_image.png");
