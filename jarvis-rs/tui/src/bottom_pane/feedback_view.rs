...
pub(crate) struct FeedbackNoteView {
    ...
    snapshot: jarvis_feedback::CodexLogSnapshot,
            ...
            Ok(()) => {
                ...
                let mut lines = vec![Line::from(match issue_url.as_ref() {
                    Some(_) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        format!("{prefix} Please report this in #Jarvis-feedback:")
                    }
                ...
                match issue_url {
                    Some(url) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        lines.extend([
                            ...
                            Line::from(vec![
                                "    ".into(),
                                format!("go/Jarvis-feedback/{thread_id}").bold(),
                            ]),
                        ]);
                    }
                    Some(url) => {
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "Jarvis-logs.log".into()]).into(),
    ];
    ...
    super::SelectionViewParams {
        ...
        items: vec![
            super::SelectionItem {
                name: "Yes".to_string(),
                description: Some(
                    "Share the current Jarvis session logs with the team for troubleshooting."
                        .to_string(),
                ),
...
mod tests {
    ...
    fn make_view(category: FeedbackCategory) -> FeedbackNoteView {
        ...
        let snapshot = jarvis_feedback::CodexFeedback::new().snapshot(None);
    ...
    fn issue_url_available_for_bug_bad_result_and_other() {
        ...
        let expected_slack_url = "http://go/Jarvis-feedback-internal".to_string();
