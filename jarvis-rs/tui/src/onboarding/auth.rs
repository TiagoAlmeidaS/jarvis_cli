...
use jarvis_core::auth::read_openai_api_key_from_env;
...
pub(crate) enum SignInState {
    PickMode,
    GoogleContinueInBrowser(ContinueInBrowserState),
        ...
        match key_event.code {
            ...
            KeyCode::Enter => {
                ...
                match sign_in_state {
                    ...
                    SignInState::GoogleSuccessMessage => {
                        *self.sign_in_state.write().unwrap() = SignInState::GoogleSuccess;
                    }
                    _ => {}
                }
            }
            KeyCode::Esc => {
                ...
                match &*sign_in_state {
                    SignInState::GoogleContinueInBrowser(_) => {
...
impl AuthModeWidget {
    fn is_api_login_allowed(&self) -> bool {
        !matches!(self.forced_login_method, Some(ForcedLoginMethod::Google))
    }

    fn is_google_login_allowed(&self) -> bool {
        !matches!(self.forced_login_method, Some(ForcedLoginMethod::Api))
    }

    fn displayed_sign_in_options(&self) -> Vec<SignInOption> {
        let mut options = Vec::new();
        if self.is_google_login_allowed() {
            options.push(SignInOption::Google);
        }
    ...
    fn disallow_api_login(&mut self) {
        self.highlighted_mode = SignInOption::Google;
    ...
    fn render_pick_mode(&self, area: Rect, buf: &mut Buffer) {
        let mut lines: Vec<Line> = vec![
            Line::from(vec![
                "  ".into(),
                "Sign in with Google to use Jarvis".into(),
            ]),
            "".into(),
        ];
        ...
        // Removed chatgpt_description as it's no longer needed
        ...
        if !self.is_api_login_allowed() {
            lines.push(
                "  API key login is disabled by this workspace. Sign in with Google to continue."
                    .dim()
                    .into(),
            );
            lines.push("".into());
    ...
    fn render_continue_in_browser(&self, area: Rect, buf: &mut Buffer) {
        ...
        if let SignInState::GoogleContinueInBrowser(state) = &*sign_in_state
    ...
    fn render_google_success_message(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            "✓ Signed in with your Google account".fg(Color::Green).into(),
            "".into(),
            "  Before you start:".into(),
            "".into(),
            "  Decide how much autonomy you want to grant Jarvis".into(),
            Line::from(vec![
                "  For more details see the ".into(),
                "\u{1b}]8;;https://github.com/openai/Jarvis\u{7}Jarvis docs\u{1b}]8;;\u{7}".underlined(),
            ])
            .dim(),
            "".into(),
            "  Jarvis can make mistakes".into(),
    ...
    fn render_google_success(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
    ...
    fn render_api_key_configured(&self, area: Rect, buf: &mut Buffer) {
        let lines = vec![
            ...
            "".into(),
            "  Jarvis will use usage-based billing with your API key.".into(),
        ];
    ...
    /// Kicks off the Google OAuth flow and keeps the UI state consistent with the attempt.
    fn start_google_login(&mut self) {
        self.error = None;
        let opts = ServerOptions::new(
            self.jarvis_home.clone(),
                ...
                tokio::spawn(async move {
                    let auth_url = child.auth_url.clone();
                    {
                        *sign_in_state.write().unwrap() =
                            SignInState::GoogleContinueInBrowser(ContinueInBrowserState {
                                auth_url,
                    ...
                    match r {
                        Ok(()) => {
                            ...
                            *sign_in_state.write().unwrap() = SignInState::GoogleSuccessMessage;
                            request_frame.schedule_frame();
                        }
                        _ => {
...
impl StepStateProvider for AuthModeWidget {
    fn get_step_state(&self) -> StepState {
        ...
        match &*sign_in_state {
            ...
            | SignInState::GoogleContinueInBrowser(_)
            | SignInState::GoogleSuccessMessage => StepState::InProgress,
            SignInState::GoogleSuccess | SignInState::ApiKeyConfigured => StepState::Complete,
        }
    }
}
...
impl WidgetRef for AuthModeWidget {
    fn render_ref(&self, area: Rect, buf: &mut Buffer) {
        ...
        match &*sign_in_state {
            ...
            SignInState::GoogleSuccessMessage => {
                self.render_google_success_message(area, buf);
            }
            SignInState::GoogleSuccess => {
                self.render_google_success(area, buf);
            }
            SignInState::ApiKeyEntry(state) => {
...
mod tests {
    ...
    fn widget_forced_google() -> (AuthModeWidget, TempDir) {
        let jarvis_home = TempDir::new().unwrap();
        let jarvis_home_path = jarvis_home.path().to_path_buf();
        let widget = AuthModeWidget {
            request_frame: FrameRequester::test_dummy(),
            highlighted_mode: SignInOption::Google,
            error: None,
            sign_in_state: Arc::new(RwLock::new(SignInState::PickMode)),
            jarvis_home: jarvis_home_path.clone(),
            ...
            forced_login_method: Some(ForcedLoginMethod::Google),
    ...
    #[test]
    fn saving_api_key_is_blocked_when_google_forced() {
        let (mut widget, _tmp) = widget_forced_google();
