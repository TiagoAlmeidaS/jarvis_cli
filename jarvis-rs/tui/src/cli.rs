use clap::Parser;
use clap::ValueHint;
use jarvis_common::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `Jarvis resume` subcommand.
    // These are not exposed as user flags on the base `Jarvis` command.
    #[clap(skip)]
    ...
    /// top-level `Jarvis resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `Jarvis fork` subcommand.
    // These are not exposed as user flags on the base `Jarvis` command.
    #[clap(skip)]
    ...
    /// top-level `Jarvis fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    pub sandbox_mode: Option<jarvis_common::SandboxModeCliArg>,
