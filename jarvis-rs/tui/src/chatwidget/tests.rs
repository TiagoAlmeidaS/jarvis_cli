...
use jarvis_core::config::ConstraintError;
use jarvis_core::config_loader::RequirementSource;
...
async fn test_config() -> Config {
    ...
    let jarvis_home = std::env::temp_dir();
...
async fn resumed_initial_messages_render_history() {
    ...
    let configured = jarvis_core::protocol::SessionConfiguredEvent {
...
async fn interrupted_turn_restores_queued_messages_with_images_and_elements() {
    ...
    chat.handle_codex_event(Event {
        id: "interrupt".into(),
        msg: EventMsg::TurnAborted(jarvis_core::protocol::TurnAbortedEvent {
...
async fn make_chatwidget_manual(
    ...
    let mut bottom = BottomPane::new(BottomPaneParams {
        ...
        enhanced_keys_supported: false,
        placeholder_text: "Ask Jarvis to do anything".to_string(),
    ...
    let auth_manager = AuthManager::from_auth_for_testing(JarvisAuth::from_api_key("test"));
    let jarvis_home = cfg.jarvis_home.clone();
    let models_manager = Arc::new(ModelsManager::new(jarvis_home, auth_manager.clone()));
...
fn set_chatgpt_auth(chat: &mut ChatWidget) {
    chat.auth_manager =
        AuthManager::from_auth_for_testing(JarvisAuth::create_dummy_chatgpt_auth_for_testing());
    chat.models_manager = Arc::new(ModelsManager::new(
        chat.config.jarvis_home.clone(),
...
async fn rate_limit_switch_prompt_shows_once_per_session() {
    let auth = JarvisAuth::create_dummy_chatgpt_auth_for_testing();
...
fn begin_exec_with_source(
    ...
    let parsed_cmd: Vec<ParsedCommand> = jarvis_core::parse_command::parse_command(&command);
...
async fn exec_end_without_begin_uses_event_command() {
    ...
    let parsed_cmd = jarvis_core::parse_command::parse_command(&command);
    let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
    chat.handle_codex_event(Event {
...
async fn slash_init_skips_when_project_doc_exists() {
    ...
    match op_rx.try_recv() {
        ...
        other => panic!("expected no Jarvis op to be sent, got {other:?}"),
    }
...
async fn collaboration_modes_defaults_to_code_on_startup() {
    let jarvis_home = tempdir().expect("tempdir");
...
async fn set_model_updates_active_collaboration_mask() {
    ...
    chat.set_model("gpt-5.1-Jarvis-mini");

    assert_eq!(chat.current_model(), "gpt-5.1-Jarvis-mini");
    assert_eq!(chat.active_collaboration_mode_kind(), ModeKind::Plan);
}
...
async fn slash_rollout_displays_current_path() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(None).await;
    let rollout_path = PathBuf::from("/tmp/Jarvis-test-rollout.jsonl");
...
async fn review_commit_picker_shows_subjects_without_timestamps() {
    ...
    let entries = vec![
        jarvis_core::git_info::CommitLogEntry {
...
async fn model_selection_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5-Jarvis")).await;
    chat.thread_id = Some(ThreadId::new());
...
async fn model_cap_error_does_not_switch_models() {
    ...
    chat.handle_codex_event(Event {
        ...
        msg: EventMsg::Error(ErrorEvent {
            ...
            jarvis_error_info: Some(JarvisErrorInfo::ModelCap {
...
async fn model_reasoning_selection_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.1-Jarvis-max")).await;

    set_chatgpt_auth(&mut chat);
    chat.set_reasoning_effort(Some(ReasoningEffortConfig::High));

    let preset = get_available_model(&chat, "gpt-5.1-Jarvis-max");
...
//
// Synthesizes a Jarvis ExecApprovalRequest event to trigger the approval modal
...
async fn apply_patch_approval_sends_op_with_submission_id() {
    ...
    while let Ok(app_ev) = rx.try_recv() {
        if let AppEvent::CodexOp(Op::PatchApproval { id, decision }) = app_ev {
            ...
            assert_matches!(decision, jarvis_core::protocol::ReviewDecision::Approved);
            found = true;
            break;
        }
    }
...
async fn apply_patch_full_flow_integration_like() {
    ...
    // 3) App forwards to widget.submit_op, which pushes onto jarvis_op_tx
    chat.submit_op(op);
    let forwarded = op_rx
        .try_recv()
        .expect("expected op forwarded to Jarvis channel");
    match forwarded {
