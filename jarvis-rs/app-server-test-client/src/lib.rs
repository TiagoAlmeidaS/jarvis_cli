...
use jarvis_app_server_protocol::ClientInfo;
use jarvis_app_server_protocol::ClientRequest;
use jarvis_app_server_protocol::CommandExecutionApprovalDecision;
use jarvis_app_server_protocol::CommandExecutionRequestApprovalParams;
use jarvis_app_server_protocol::CommandExecutionRequestApprovalResponse;
use jarvis_app_server_protocol::DynamicToolSpec;
use jarvis_app_server_protocol::FileChangeApprovalDecision;
use jarvis_app_server_protocol::FileChangeRequestApprovalParams;
use jarvis_app_server_protocol::FileChangeRequestApprovalResponse;
use jarvis_app_server_protocol::GetAccountRateLimitsResponse;
use jarvis_app_server_protocol::InitializeCapabilities;
use jarvis_app_server_protocol::InitializeParams;
use jarvis_app_server_protocol::InitializeResponse;
use jarvis_app_server_protocol::InputItem;
use jarvis_app_server_protocol::JSONRPCMessage;
use jarvis_app_server_protocol::JSONRPCNotification;
use jarvis_app_server_protocol::JSONRPCRequest;
use jarvis_app_server_protocol::JSONRPCResponse;
...
use jarvis_app_server_protocol::SandboxPolicy;
...
use jarvis_app_server_protocol::ServerRequest;
...
/// Minimal launcher that initializes the Jarvis app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Jarvis", version, about = "Bootstrap Jarvis app-server", long_about = None)]
struct Cli {
    /// Path to the `Jarvis` CLI binary.
    #[arg(long, env = "jarvis_BIN", default_value = "Jarvis")]
    jarvis_bin: PathBuf,

    /// Forwarded to the `Jarvis` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Send a user message through the Jarvis app-server.
    SendMessage {
        /// User message to send to Jarvis.
        user_message: String,
    },
    ...
    TestLogin,
    /// Fetch the current account rate limits from the Jarvis app-server.
    GetAccountRateLimits,
    /// List the available models from the Jarvis app-server.
...
pub fn run() -> Result<()> {
    ...
    match command {
        CliCommand::SendMessage { user_message } => {
            ...
            send_message(&jarvis_bin, &config_overrides, user_message)
        }
        CliCommand::SendMessageV2 { user_message } => {
            send_message_v2(&jarvis_bin, &config_overrides, user_message, &dynamic_tools)
        }
        CliCommand::TriggerCmdApproval { user_message } => {
            trigger_cmd_approval(&jarvis_bin, &config_overrides, user_message, &dynamic_tools)
        }
        CliCommand::TriggerPatchApproval { user_message } => {
            trigger_patch_approval(&jarvis_bin, &config_overrides, user_message, &dynamic_tools)
        }
        CliCommand::NoTriggerCmdApproval => {
            no_trigger_cmd_approval(&jarvis_bin, &config_overrides, &dynamic_tools)
        }
        ...
        CliCommand::TestLogin => {
            ...
            test_login(&jarvis_bin, &config_overrides)
        }
        CliCommand::GetAccountRateLimits => {
            ensure_dynamic_tools_unused(&dynamic_tools, "get-account-rate-limits")?;
            get_account_rate_limits(&jarvis_bin, &config_overrides)
        }
        CliCommand::ModelList => {
            ensure_dynamic_tools_unused(&dynamic_tools, "model-list")?;
            model_list(&jarvis_bin, &config_overrides)
        }
    }
}
...
fn test_login(jarvis_bin: &Path, config_overrides: &[String]) -> Result<()> {
    let mut client = CodexClient::spawn(jarvis_bin, config_overrides)?;
...
fn get_account_rate_limits(jarvis_bin: &Path, config_overrides: &[String]) -> Result<()> {
    let mut client = CodexClient::spawn(jarvis_bin, config_overrides)?;
...
fn model_list(jarvis_bin: &Path, config_overrides: &[String]) -> Result<()> {
    let mut client = CodexClient::spawn(jarvis_bin, config_overrides)?;
...
impl CodexClient {
    fn spawn(jarvis_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let jarvis_bin_display = jarvis_bin.display();
        let mut cmd = Command::new(jarvis_bin);
        for override_kv in config_overrides {
            cmd.arg("--config").arg(override_kv);
        }
        let mut jarvis_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{jarvis_bin_display}` app-server"))?;

        let stdin = jarvis_app_server
            .stdin
            .take()
            .context("Jarvis app-server stdin unavailable")?;
        let stdout = jarvis_app_server
            .stdout
            .take()
            .context("Jarvis app-server stdout unavailable")?;

        Ok(Self {
            child: jarvis_app_server,
            stdin: Some(stdin),
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "Jarvis-toy-app-server".to_string(),
                    title: Some("Jarvis Toy App Server".to_string()),
    ...
    fn stream_conversation(&mut self, conversation_id: &ThreadId) -> Result<()> {
        loop {
            ...
            if !notification.method.starts_with("Jarvis/event/") {
                continue;
            }
    ...
    fn write_request(&mut self, request: &ClientRequest) -> Result<()> {
        ...
        if let Some(stdin) = self.stdin.as_mut() {
            ...
            stdin
                .flush()
                .context("failed to flush request to Jarvis app-server")?;
        } else {
            bail!("Jarvis app-server stdin closed");
        }
    ...
    fn read_jsonrpc_message(&mut self) -> Result<JSONRPCMessage> {
        loop {
            ...
            let bytes = self
                ...
                .context("failed to read from Jarvis app-server")?;

            if bytes == 0 {
                bail!("Jarvis app-server closed stdout");
            }
    ...
    fn write_jsonrpc_message(&mut self, message: JSONRPCMessage) -> Result<()> {
        ...
        if let Some(stdin) = self.stdin.as_mut() {
            ...
            stdin
                .flush()
                .context("failed to flush response to Jarvis app-server")?;
            return Ok(());
        }
...
impl Drop for CodexClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = self.child.try_wait() {
            println!("[Jarvis app-server exited: {status}]");
            return;
        }
