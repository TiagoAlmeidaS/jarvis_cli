...
use jarvis_backend_client as backend;
use jarvis_backend_client::CodeTaskDetailsResponseExt;
...
mod api {
    ...
    impl<'a> Tasks<'a> {
        ...
        pub(crate) async fn messages(&self, id: TaskId) -> Result<Vec<String>> {
            ...
            let url = match details_path(self.base_url, &id.0) {
                Some(url) => url,
                None => format!("{}/api/Jarvis/tasks/{}", self.base_url, id.0),
            };
        ...
        pub(crate) async fn create(
            ...
            if let Ok(diff) = std::env::var("jarvis_STARTING_DIFF")
                && !diff.is_empty()
            {
                input_items.push(serde_json::json!({
    ...
    impl<'a> Apply<'a> {
        ...
        pub(crate) async fn run(
            ...
            let req = jarvis_git::ApplyGitRequest {
                cwd: std::env::current_dir().unwrap_or_else(|_| std::env::temp_dir()),
                diff: diff.clone(),
                revert: false,
                preflight,
            };
            let r = jarvis_git::apply_git_patch(&req)
