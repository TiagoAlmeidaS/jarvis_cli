...
/// While we may add an `apply-patch` subcommand to the `Jarvis` CLI multitool
/// at some point, we must ensure that the smaller `Jarvis-exec` CLI can still
...
fn test_standalone_exec_cli_can_use_apply_patch() -> anyhow::Result<()> {
    ...
    Command::new(jarvis_utils_cargo_bin::cargo_bin("Jarvis-exec")?)
        .arg(jarvis_APPLY_PATCH_ARG1)
        .arg(
            r#"*** Begin Patch
