...
use jarvis_core::git_info::get_git_repo_root;
...
struct ThreadEventEnvelope {
    ...
    thread: Arc<jarvis_core::JarvisThread>,
    event: Event,
}
...
pub async fn run_main(cli: Cli, jarvis_linux_sandbox_exe: Option<PathBuf>) -> anyhow::Result<()> {
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        jarvis_core::otel_init::build_provider(&config, env!("CARGO_PKG_VERSION"), None, false)
    })) {
        Ok(Ok(otel)) => otel,
        Ok(Err(e)) => {
    ...
    // Print the effective configuration and initial request so users can see what Jarvis
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);

    info!("Jarvis initialized with event: {session_configured:?}");
...

    {
        ...
        tokio::spawn(async move {
            if tokio::signal::ctrl_c().await.is_ok() {
                ...
                // Immediately notify Jarvis to abort any in-flight task.
...
async fn resolve_resume_path(
    ...
    if args.last {
        ...
        match jarvis_core::RolloutRecorder::find_latest_thread_path(
            &config.jarvis_home,
            1,
            None,
            jarvis_core::ThreadSortKey::UpdatedAt,
            &[],
        ...
        if Uuid::parse_str(id_str).is_ok() {
            let path = find_thread_path_by_id_str(&config.jarvis_home, id_str).await?;
            Ok(path)
        } else {
            let path = find_thread_path_by_name_str(&config.jarvis_home, id_str).await?;
            Ok(path)
        }
    } else {
        Ok(None)
