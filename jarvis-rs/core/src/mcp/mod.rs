...
use jarvis_protocol::mcp::Resource;
use jarvis_protocol::mcp::ResourceTemplate;
use jarvis_protocol::mcp::Tool;
use jarvis_protocol::protocol::McpListToolsResponseEvent;
...
pub(crate) const jarvis_APPS_MCP_SERVER_NAME: &str = "jarvis_apps";
const jarvis_CONNECTORS_TOKEN_ENV_VAR: &str = "jarvis_CONNECTORS_TOKEN";

fn jarvis_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(jarvis_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(jarvis_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(jarvis_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn jarvis_apps_mcp_bearer_token(auth: Option<&JarvisAuth>) -> Option<String> {
...
fn jarvis_apps_mcp_http_headers(auth: Option<&JarvisAuth>) -> Option<HashMap<String, String>> {
    let mut headers = HashMap::new();
    if let Some(token) = jarvis_apps_mcp_bearer_token(auth) {
        headers.insert("Authorization".to_string(), format!("Bearer {token}"));
    }
    if let Some(account_id) = auth.and_then(JarvisAuth::get_account_id) {
...
fn jarvis_apps_mcp_url(base_url: &str) -> String {
    ...
    } else if base_url.contains("/api/Jarvis") {
        format!("{base_url}/apps")
    } else {
        format!("{base_url}/api/Jarvis/apps")
    }
}

fn jarvis_apps_mcp_server_config(config: &Config, auth: Option<&JarvisAuth>) -> McpServerConfig {
    let bearer_token_env_var = jarvis_apps_mcp_bearer_token_env_var();
    let http_headers = if bearer_token_env_var.is_some() {
        None
    } else {
        jarvis_apps_mcp_http_headers(auth)
    };
    let url = jarvis_apps_mcp_url(&config.chatgpt_base_url);
...
pub(crate) fn with_codex_apps_mcp(
    ...
    if connectors_enabled {
        servers.insert(
            jarvis_APPS_MCP_SERVER_NAME.to_string(),
            jarvis_apps_mcp_server_config(config, auth),
        );
    } else {
        servers.remove(jarvis_APPS_MCP_SERVER_NAME);
    }
    servers
}
