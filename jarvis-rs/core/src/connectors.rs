...
pub use jarvis_app_server_protocol::AppInfo;
...
pub async fn list_accessible_connectors_from_mcp_tools(
    ...
    if let Some(cfg) = mcp_servers.get(jarvis_APPS_MCP_SERVER_NAME) {
        let timeout = cfg.startup_timeout_sec.unwrap_or(DEFAULT_STARTUP_TIMEOUT);
        mcp_connection_manager
            .wait_for_server_ready(jarvis_APPS_MCP_SERVER_NAME, timeout)
            .await;
    }
