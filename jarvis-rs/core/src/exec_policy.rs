...
use jarvis_execpolicy::AmendError;
use jarvis_execpolicy::Decision;
use jarvis_execpolicy::Error as ExecPolicyRuleError;
use jarvis_execpolicy::Evaluation;
use jarvis_execpolicy::Policy;
use jarvis_execpolicy::PolicyParser;
use jarvis_execpolicy::RuleMatch;
use jarvis_execpolicy::blocking_append_allow_prefix_rule;
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: jarvis_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(jarvis_home);
...
fn default_policy_path(jarvis_home: &Path) -> PathBuf {
    jarvis_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
/// - Note: we only use this amendment when the command fails to run in sandbox and Jarvis prompts the user to run outside the sandbox
...
"#;
    ...
    async fn append_execpolicy_amendment_updates_policy_and_file() {
        ...
        manager
            .append_amendment_and_update(jarvis_home.path(), &ExecPolicyAmendment::from(prefix))
            .await
        ...
        let contents = fs::read_to_string(default_policy_path(jarvis_home.path()))
...
"#
    ...
    async fn append_execpolicy_amendment_rejects_empty_prefix() {
        ...
        let result = manager
            .append_amendment_and_update(jarvis_home.path(), &ExecPolicyAmendment::from(vec![]))
            .await;
