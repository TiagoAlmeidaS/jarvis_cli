...
async fn make_config_for_test(
    ...
    tokio::fs::write(
        jarvis_home.join(CONFIG_TOML_FILE),
...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .jarvis_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = jarvis_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
...
async fn returns_empty_when_all_layers_missing() {
    ...
    let user_layer = layers
        .get_user_layer()
        .expect("expected a user layer even when jarvis_home/config.toml does not exist");
    assert_eq!(
        &ConfigLayerEntry {
...
async fn load_config_layers_includes_cloud_requirements() -> anyhow::Result<()> {
    let tmp = tempdir()?;
    let jarvis_home = tmp.path().join("home");
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".jarvis")).await?;
    tokio::fs::create_dir_all(project_root.join(".jarvis")).await?;
    ...
    .await?;
    tokio::fs::write(
        nested.join(".jarvis").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    let jarvis_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&jarvis_home).await?;
    make_config_for_test(&jarvis_home, &project_root, TrustLevel::Trusted, None).await?;
...
"#;
    ...
    .await?;
    tokio::fs::write(
        nested.join(".jarvis").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let jarvis_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&jarvis_home).await?;
    tokio::fs::write(jarvis_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn jarvis_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let jarvis_home = home_dir.join(".jarvis");
    tokio::fs::create_dir_all(&jarvis_home).await?;
    tokio::fs::write(jarvis_home.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
#[tokio::test]
async fn jarvis_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".jarvis");
    let nested_dot_codex = nested.join(".jarvis");
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let jarvis_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&jarvis_home_untrusted).await?;
    make_config_for_test(
        &jarvis_home_untrusted,
        &project_root,
        TrustLevel::Untrusted,
        None,
    )
    .await?;
    let untrusted_config_path = jarvis_home_untrusted.join(CONFIG_TOML_FILE);
    ...
    let jarvis_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&jarvis_home_unknown).await?;
    tokio::fs::write(
        jarvis_home_unknown.join(CONFIG_TOML_FILE),
        "foo = \"user\"\n",
    )
    .await?;
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".jarvis")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".jarvis").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let jarvis_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&jarvis_home).await?;
        let config_path = jarvis_home.join(CONFIG_TOML_FILE);

        if let Some(trust_level) = trust_level {
            make_config_for_test(&jarvis_home, &project_root, trust_level, None).await?;
...
mod requirements_exec_policy_tests {
    ...
    use jarvis_execpolicy::Decision;
    use jarvis_execpolicy::Evaluation;
    use jarvis_execpolicy::RuleMatch;
    use jarvis_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
