...
const DEFAULT_REQUIREMENTS_TOML_FILE_UNIX: &str = "/etc/Jarvis/requirements.toml";

/// On Unix systems, load default settings from this file path, if present.
/// Note that /etc/Jarvis/ is treated as a "config folder," so subfolders such
/// as skills/ and rules/ will also be honored.
pub const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/Jarvis/config.toml";
...
/// - system    `/etc/Jarvis/requirements.toml`
///
/// For backwards compatibility, we also load from
/// `/etc/Jarvis/managed_config.toml` and map it to
/// `/etc/Jarvis/requirements.toml`.
///
/// Configuration is built up from multiple layers in the following order:
///
/// - admin:    managed preferences (*)
/// - system    `/etc/Jarvis/config.toml`
/// - user      `${jarvis_home}/config.toml`
/// - cwd       `${PWD}/config.toml` (loaded but disabled when the directory is untrusted)
/// - tree      parent directories up to root looking for `./.jarvis/config.toml` (loaded but disabled when untrusted)
/// - repo      `$(git rev-parse --show-toplevel)/.jarvis/config.toml` (loaded but disabled when untrusted)
/// - runtime   e.g., --config flags, model selector in UI
///
/// (*) Only available on macOS via managed device profiles.
///
/// See https://developers.openai.com/Jarvis/security for details.
///
...
pub async fn load_config_layers_state(
    ...
    // Honor /etc/Jarvis/requirements.toml.
    if cfg!(unix) {
    ...
    // Add a layer for $jarvis_home/config.toml if it exists. Note if the file
    // exists, but is malformed, then this error should be propagated to the
    // user.
    let user_file = AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, jarvis_home)?;
...
/// If available, apply requirements from `/etc/Jarvis/requirements.toml` to
...
async fn load_project_layers(
    ...
    let jarvis_home_abs = AbsolutePathBuf::from_absolute_path(jarvis_home)?;
    let jarvis_home_normalized =
        normalize_path(jarvis_home_abs.as_path()).unwrap_or_else(|_| jarvis_home_abs.to_path_buf());
    let mut dirs = cwd
        .as_path()
        .ancestors()
        .scan(false, |done, a| {
    ...
    for dir in dirs {
        ...
        if dot_codex_abs == jarvis_home_abs || dot_codex_normalized == jarvis_home_normalized {
            continue;
        }
...
/// from a file like `/etc/Jarvis/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for Jarvis to function correctly.
