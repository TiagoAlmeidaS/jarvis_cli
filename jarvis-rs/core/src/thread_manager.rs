...
/// Represents a newly created Jarvis thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<JarvisThread>,
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<JarvisThread>>>>,
...
impl ThreadManager {
    pub fn new(
        ...
        Self {
            state: Arc::new(ThreadManagerState {
                ...
                skills_manager: Arc::new(SkillsManager::new(jarvis_home)),
                auth_manager,
                session_source,
                #[cfg(any(test, feature = "test-support"))]
    ...
    #[cfg(any(test, feature = "test-support"))]
    /// Construct with a dummy AuthManager containing the provided JarvisAuth.
    /// Used for integration tests: should not be used by ordinary business logic.
    pub fn with_models_provider(auth: JarvisAuth, provider: ModelProviderInfo) -> Self {
        let temp_dir = tempfile::tempdir().unwrap_or_else(|err| panic!("temp Jarvis home: {err}"));
        let jarvis_home = temp_dir.path().to_path_buf();
        let mut manager = Self::with_models_provider_and_home(auth, provider, jarvis_home);
        manager._test_codex_home_guard = Some(temp_dir);
        manager
    }

    #[cfg(any(test, feature = "test-support"))]
    /// Construct with a dummy AuthManager containing the provided JarvisAuth and Jarvis home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> CodexResult<Arc<JarvisThread>> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<jarvis_protocol::dynamic_tools::DynamicToolSpec>,
    ...
    /// as `Arc<JarvisThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<JarvisThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(&self, thread_id: ThreadId) -> CodexResult<Arc<JarvisThread>> {
        let threads = self.threads.read().await;
        threads
            .get(&thread_id)
            .cloned()
            .ok_or_else(|| JarvisErr::ThreadNotFound(thread_id))
    }
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<JarvisThread>> {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        .await?;
        self.finalize_thread_spawn(Jarvis, thread_id).await
    }
    ...
    async fn finalize_thread_spawn(
        ...
        let event = Jarvis.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(JarvisErr::SessionConfiguredNotFirstEvent);
            }
        };

        let thread = Arc::new(JarvisThread::new(
            Jarvis,
