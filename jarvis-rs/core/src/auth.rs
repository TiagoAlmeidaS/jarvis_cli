...
use jarvis_app_server_protocol::AuthMode as ApiAuthMode;
...
use jarvis_client::JarvisHttpClient;
use jarvis_protocol::account::PlanType as AccountPlanType;
...
impl OAuthProvider {
    pub fn client_id(&self) -> String {
        match self {
            Self::Google => std::env::var("GOOGLE_CLIENT_ID")
...
pub const REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "jarvis_REFRESH_TOKEN_URL_OVERRIDE";
...
impl JarvisAuth {
    fn from_auth_dot_json(
        ...
        if auth_mode == ApiAuthMode::ApiKey {
            ...
            return Ok(JarvisAuth::from_api_key_with_client(api_key, client));
        }
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(jarvis_home.to_path_buf(), storage_mode);
    ...
    pub fn from_auth_storage(
        ...
        load_auth(jarvis_home, false, auth_credentials_store_mode)
    }
    ...
    fn from_api_key_with_client(api_key: &str, _client: JarvisHttpClient) -> Self {
...
impl ChatgptAuth {
    ...
    fn client(&self) -> &JarvisHttpClient {
        &self.state.client
    }
}

pub const OPENAI_API_KEY_ENV_VAR: &str = "OPENAI_API_KEY";
pub const jarvis_API_KEY_ENV_VAR: &str = "jarvis_API_KEY";
...
pub fn read_codex_api_key_from_env() -> Option<String> {
    env::var(jarvis_API_KEY_ENV_VAR)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

/// Delete the auth.json file inside `jarvis_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    jarvis_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(jarvis_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}

/// Check if the user has legacy OpenAI/ChatGPT authentication configured.
/// Returns true if migration is needed.
pub fn check_legacy_openai_auth(
    jarvis_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let auth = load_auth_dot_json(jarvis_home, auth_credentials_store_mode)?;
...
pub fn login_with_api_key(
    ...
    save_auth(jarvis_home, &auth_dot_json, auth_credentials_store_mode)
}
...
pub fn enforce_login_restrictions(config: &Config) -> std::io::Result<()> {
    ...
    if let Some(required_method) = config.forced_login_method {
        let method_violation = match (required_method, auth.internal_auth_mode()) {
            ...
            (ForcedLoginMethod::Google, AuthMode::Chatgpt) => None,
            (ForcedLoginMethod::Api, AuthMode::Chatgpt) => Some(
                "API key login is required, but Google OAuth is currently being used. Logging out."
                    .to_string(),
            ),
            (ForcedLoginMethod::Google, AuthMode::ApiKey) => Some(
                "Google login is required, but an API key is currently being used. Logging out."
                    .to_string(),
            ),
        };
...
fn logout_with_message(
    ...
    let removal_result = logout_all_stores(jarvis_home, auth_credentials_store_mode);
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(jarvis_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(jarvis_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(jarvis_home, auth_credentials_store_mode)?;
...
fn load_auth(
    ...
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<JarvisAuth>> {
    let build_auth = |auth_dot_json: AuthDotJson, storage_mode| {
        let client = crate::default_client::create_client();
        JarvisAuth::from_auth_dot_json(jarvis_home, auth_dot_json, storage_mode, client)
    };

    // API key via env var takes precedence over any other auth method.
    if enable_codex_api_key_env && let Some(api_key) = read_codex_api_key_from_env() {
        let client = crate::default_client::create_client();
        return Ok(Some(JarvisAuth::from_api_key_with_client(
            api_key.as_str(),
            client,
        )));
    }
...
impl Debug for CachedAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CachedAuth")
            .field(
                "auth_mode",
                &self.auth.as_ref().map(JarvisAuth::api_auth_mode),
            )
            .field(
                "external_refresher",
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(JarvisAuth::get_account_id);
        let mode = if cached_auth
            .as_ref()
            .is_some_and(JarvisAuth::is_external_chatgpt_tokens)
        {
...
/// hands out cloned `JarvisAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific JarvisAuth, for testing only.
    pub fn from_auth_for_testing(auth: JarvisAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            jarvis_home: PathBuf::from("non-existent"),
    ...
    #[cfg(any(test, feature = "test-support"))]
    /// Create an AuthManager with a specific JarvisAuth and Jarvis home, for testing only.
    pub fn from_auth_for_testing_with_home(auth: JarvisAuth, jarvis_home: PathBuf) -> Arc<Self> {
        let cached = CachedAuth {
            auth: Some(auth),
            external_refresher: None,
        };
    ...
    fn reload_if_account_id_matches(&self, expected_account_id: Option<&str>) -> ReloadOutcome {
        ...
        let new_auth = self.load_auth_from_storage();
        let new_account_id = new_auth.as_ref().and_then(JarvisAuth::get_account_id);
    ...
    fn auths_equal(a: Option<&JarvisAuth>, b: Option<&JarvisAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    fn load_auth_from_storage(&self) -> Option<JarvisAuth> {
        load_auth(
            &self.jarvis_home,
    ...
    fn set_cached_auth(&self, new_auth: Option<JarvisAuth>) -> bool {
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        match auth {
            JarvisAuth::ChatgptAuthTokens(_) => {
                self.refresh_external_auth(ExternalAuthRefreshReason::Unauthorized)
                    .await
            }
            JarvisAuth::Chatgpt(chatgpt_auth) => {
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = logout_all_stores(&self.jarvis_home, self.auth_credentials_store_mode)?;
    ...
    pub fn get_auth_mode(&self) -> Option<ApiAuthMode> {
        self.auth_cached().as_ref().map(JarvisAuth::api_auth_mode)
    }

    pub fn get_internal_auth_mode(&self) -> Option<AuthMode> {
        self.auth_cached()
            .as_ref()
            .map(JarvisAuth::internal_auth_mode)
    }

    async fn refresh_if_stale(&self, auth: &JarvisAuth) -> Result<bool, RefreshTokenError> {
        let chatgpt_auth = match auth {
            JarvisAuth::Chatgpt(chatgpt_auth) => chatgpt_auth,
            _ => return Ok(false),
        };
    ...
    async fn refresh_external_auth(
        ...
        let previous_account_id = self
            .auth_cached()
            .as_ref()
            .and_then(JarvisAuth::get_account_id);
...
mod tests {
    ...
    fn missing_auth_json_returns_none() {
        let dir = tempdir().unwrap();
        let auth = JarvisAuth::from_auth_storage(dir.path(), AuthCredentialsStoreMode::File)
            .expect("call should succeed");
        assert_eq!(auth, None);
    }

    #[tokio::test]
    #[serial(jarvis_api_key)]
    async fn pro_account_with_no_api_key_uses_chatgpt_auth() {
        ...
        let auth = super::load_auth(jarvis_home.path(), false, AuthCredentialsStoreMode::File)
            .unwrap()
            .unwrap();
    ...
    fn write_auth_file(params: AuthFileParams, jarvis_home: &Path) -> std::io::Result<String> {
        let auth_file = get_auth_file(jarvis_home);
    ...
    async fn enforce_login_restrictions_logs_out_for_method_mismatch() {
        ...
        assert!(err.to_string().contains("Google login is required"));
        assert!(
            !jarvis_home.path().join("auth.json").exists(),
    ...
    #[tokio::test]
    #[serial(jarvis_api_key)]
    async fn enforce_login_restrictions_blocks_env_api_key_when_google_required() {
        let _guard = EnvVarGuard::set(jarvis_API_KEY_ENV_VAR, "sk-env");
        let jarvis_home = tempdir().unwrap();

        let config = build_config(jarvis_home.path(), Some(ForcedLoginMethod::Google), None).await;

        let err = super::enforce_login_restrictions(&config)
            .expect_err("environment API key should not satisfy forced Google login");
        assert!(
            err.to_string()
                .contains("Google login is required, but an API key is currently being used.")
        );
    }
