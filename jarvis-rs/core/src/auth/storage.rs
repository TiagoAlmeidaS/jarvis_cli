...
use jarvis_keyring_store::DefaultKeyringStore;
use jarvis_keyring_store::KeyringStore;
use once_cell::sync::Lazy;

/// Determine where Jarvis should store CLI auth credentials.
#[derive(Debug, Default, Copy, Clone, PartialEq, Eq, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "lowercase")]
pub enum AuthCredentialsStoreMode {
    #[default]
    /// Persist credentials in jarvis_home/auth.json.
    File,
    /// Persist credentials in the keyring. Fail if unavailable.
    Keyring,
    /// Use keyring when available; otherwise, fall back to a file in jarvis_home.
    Auto,
    /// Store credentials in memory only for the current process.
    Ephemeral,
}

/// Expected structure for $jarvis_home/auth.json.
...
pub(super) fn get_auth_file(jarvis_home: &Path) -> PathBuf {
    jarvis_home.join("auth.json")
}

pub(super) fn delete_file_if_exists(jarvis_home: &Path) -> std::io::Result<bool> {
...
impl FileAuthStorage {
    pub(super) fn new(jarvis_home: PathBuf) -> Self {
        Self { jarvis_home }
    }

    /// Attempt to read and parse the `auth.json` file in the given `jarvis_home` directory.
...
impl AuthStorageBackend for FileAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let auth_file = get_auth_file(&self.jarvis_home);
    ...
    fn delete(&self) -> std::io::Result<bool> {
        delete_file_if_exists(&self.jarvis_home)
    }
}

const KEYRING_SERVICE: &str = "Jarvis Auth";

// turns jarvis_home path into a stable, short key string
fn compute_store_key(jarvis_home: &Path) -> std::io::Result<String> {
    let canonical = jarvis_home
        .canonicalize()
        .unwrap_or_else(|_| jarvis_home.to_path_buf());
...
impl KeyringAuthStorage {
    fn new(jarvis_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            jarvis_home,
            keyring_store,
        }
...
impl AuthStorageBackend for KeyringAuthStorage {
    fn load(&self) -> std::io::Result<Option<AuthDotJson>> {
        let key = compute_store_key(&self.jarvis_home)?;
    ...
    fn save(&self, auth: &AuthDotJson) -> std::io::Result<()> {
        ...
        if let Err(err) = delete_file_if_exists(&self.jarvis_home) {
    ...
    fn delete(&self) -> std::io::Result<bool> {
        ...
        let file_removed = delete_file_if_exists(&self.jarvis_home)?;
...
impl AutoAuthStorage {
    fn new(jarvis_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            keyring_storage: Arc::new(KeyringAuthStorage::new(jarvis_home.clone(), keyring_store)),
            file_storage: Arc::new(FileAuthStorage::new(jarvis_home)),
        }
    }
}
...
// A global in-memory store for mapping jarvis_home -> AuthDotJson.
...
impl EphemeralAuthStorage {
    fn new(jarvis_home: PathBuf) -> Self {
        Self { jarvis_home }
    }
...
pub(super) fn create_auth_storage(
    ...
    create_auth_storage_with_keyring_store(jarvis_home, mode, keyring_store)
}
...
fn create_auth_storage_with_keyring_store(
    ...
    match mode {
        AuthCredentialsStoreMode::File => Arc::new(FileAuthStorage::new(jarvis_home)),
...
mod tests {
    ...
    use jarvis_keyring_store::tests::MockKeyringStore;
    use keyring::Error as KeyringError;

    #[tokio::test]
    async fn file_storage_load_returns_auth_dot_json() -> anyhow::Result<()> {
        let jarvis_home = tempdir()?;
        let storage = FileAuthStorage::new(jarvis_home.path().to_path_buf());
    ...
    async fn file_storage_save_persists_auth_dot_json() -> anyhow::Result<()> {
        ...
        let file = get_auth_file(jarvis_home.path());
        storage
            .save(&auth_dot_json)
            .context("failed to save auth file")?;
    ...
    fn keyring_auth_storage_load_returns_deserialized_auth() -> anyhow::Result<()> {
        ...
        seed_keyring_with_auth(
            &mock_keyring,
            || compute_store_key(jarvis_home.path()),
            &expected,
        )?;
    ...
    fn keyring_auth_storage_compute_store_key_for_home_directory() -> anyhow::Result<()> {
        let jarvis_home = PathBuf::from("~/.jarvis");

        let key = compute_store_key(jarvis_home.as_path())?;
    ...
    fn keyring_auth_storage_save_persists_and_removes_fallback_file() -> anyhow::Result<()> {
        ...
        let auth_file = get_auth_file(jarvis_home.path());
        std::fs::write(&auth_file, "stale")?;
        ...
        let key = compute_store_key(jarvis_home.path())?;
