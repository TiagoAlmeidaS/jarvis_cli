...
use jarvis_protocol::models::VIEW_IMAGE_TOOL_NAME;
...
impl ToolsConfig {
    pub fn new(params: &ToolsConfigParams) -> Self {
        ...
        let shell_type = if !features.enabled(Feature::ShellTool) {
            ...
            if jarvis_utils_pty::conpty_supported() {
...
fn create_github_create_issue_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "owner".to_string(),
            JsonSchema::String {
        ...
        (
            "body".to_string(),
            JsonSchema::String {
                description: Some("Issue body/description".to_string()),
            },
        ),
        (
            "labels".to_string(),
            JsonSchema::Array {
                items: Box::new(JsonSchema::String { description: None }),
                description: Some("Optional labels to apply to the issue".to_string()),
            },
        ),
        (
            "assignees".to_string(),
            JsonSchema::Array {
                items: Box::new(JsonSchema::String { description: None }),
                description: Some("Optional GitHub usernames to assign the issue to".to_string()),
            },
        ),
    ]);
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Create a new GitHub issue in a repository. Requires a GitHub PAT token to be configured.".to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
            required: Some(vec!["owner".to_string(), "repo".to_string(), "title".to_string()]),
            additional_properties: Some(false.into()),
        },
    })
}

fn create_github_comment_pr_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "owner".to_string(),
            JsonSchema::String {
        ...
        (
            "comment".to_string(),
            JsonSchema::String {
                description: Some("Comment body (supports Markdown)".to_string()),
            },
        ),
    ]);
...
fn create_github_list_repos_tool() -> ToolSpec {
    let properties = BTreeMap::from([(
        "username".to_string(),
        JsonSchema::String {
            description: Some("Optional GitHub username or organization. If omitted, lists repositories for the authenticated user.".to_string()),
        },
    )]);

    ToolSpec::Function(ResponsesApiTool {
        name: "github_list_repos".to_string(),
        description: "List GitHub repositories for a user or organization. Requires a GitHub PAT token to be configured.".to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
            required: None,
...
fn create_github_clone_repo_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "owner".to_string(),
            JsonSchema::String {
    ...
    ToolSpec::Function(ResponsesApiTool {
        name: "github_clone_repo".to_string(),
        description: "Get the clone URL for a GitHub repository. Requires a GitHub PAT token to be configured.".to_string(),
        strict: false,
        parameters: JsonSchema::Object {
            properties,
...
fn create_github_list_issues_tool() -> ToolSpec {
    let properties = BTreeMap::from([
        (
            "owner".to_string(),
            JsonSchema::String {
        ...
        (
            ...
            JsonSchema::Array {
                items: Box::new(JsonSchema::String { description: None }),
                description: Some("Optional list of label names to filter by".to_string()),
            },
        ),
    ]);
...
pub(crate) fn build_specs(
    ...
    let github_handler = Arc::new(GitHubHandler);
    builder.push_spec_with_parallel_support(create_github_create_issue_tool(), true);
    builder.push_spec_with_parallel_support(create_github_comment_pr_tool(), true);
...
mod tests {
    ...
    fn test_full_toolset_specs_for_gpt5_codex_unified_exec_web_search() {
        ...
        let model_info = ModelsManager::construct_model_info_offline("gpt-5-Jarvis", &config);
        ...
        for spec in [
            ...
            create_github_create_issue_tool(),
            create_github_comment_pr_tool(),
    ...
    fn test_test_model_info_includes_sync_tool() {
        let config = test_config();
        let model_info = ModelsManager::construct_model_info_offline("test-gpt-5-Jarvis", &config);
        let features = Features::with_defaults();
        let tools_config = ToolsConfig::new(&ToolsConfigParams {
            model_info: &model_info,
            features: &features,
