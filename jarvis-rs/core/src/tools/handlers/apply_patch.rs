...
use jarvis_apply_patch::ApplyPatchFileChange;
...
impl ToolHandler for ApplyPatchHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        ...
        match jarvis_apply_patch::maybe_parse_apply_patch_verified(&command, &cwd) {
            jarvis_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
                match apply_patch::apply_patch(turn.as_ref(), changes).await {
                    ...
                    InternalApplyPatchInvocation::DelegateToExec(apply) => {
                        ...
                        let req = ApplyPatchRequest {
                            ...
                            timeout_ms: None,
                            jarvis_exe: turn.jarvis_linux_sandbox_exe.clone(),
                        };
            ...
            jarvis_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            jarvis_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            jarvis_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match jarvis_apply_patch::maybe_parse_apply_patch_verified(command, cwd) {
        ...
        jarvis_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
...
mod tests {
    use super::*;
    use jarvis_apply_patch::MaybeApplyPatchVerified;
...
*** End Patch"#;
        ...
        let action = match jarvis_apply_patch::maybe_parse_apply_patch_verified(&argv, cwd) {
