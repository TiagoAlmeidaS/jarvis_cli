...
impl ToolCallRuntime {
    ...
    pub(crate) fn handle_tool_call(
    ...
    ) -> impl std::future::Future<Output = Result<ResponseInputItem, JarvisErr>> {
        ...
        async move {
            match handle.await {
                ...
                Ok(Err(FunctionCallError::Fatal(message))) => Err(JarvisErr::Fatal(message)),
                Ok(Err(other)) => Err(JarvisErr::Fatal(other.to_string())),
                Err(err) => Err(JarvisErr::Fatal(format!(
