...
use jarvis_protocol::openai_models::TruncationMode;
...
pub(crate) fn find_model_info_for_slug(slug: &str) -> ModelInfo {
    ...
    } else if slug.starts_with("Jarvis-mini-latest") {
        model_info!(
            slug,
            base_instructions: BASE_INSTRUCTIONS_WITH_APPLY_PATCH.to_string(),
    ...
    } else if slug.starts_with("exp-Jarvis") || slug.starts_with("Jarvis-1p") {
        model_info!(
            slug,
    ...
    } else if slug.starts_with("gpt-5.2-Jarvis") || slug.starts_with("bengalfox") {
        model_info!(
            slug,
            base_instructions: GPT_5_2_CODEX_INSTRUCTIONS.to_string(),
            apply_patch_tool_type: Some(ApplyPatchToolType::Freeform),
    ...
    } else if slug.starts_with("gpt-5.1-Jarvis-max") {
        model_info!(
            slug,
    ...
    } else if (slug.starts_with("gpt-5-Jarvis")
        || slug.starts_with("gpt-5.1-Jarvis")
        || slug.starts_with("Jarvis-"))
        && !slug.contains("-mini")
    {
        model_info!(
            slug,
        ...
        || slug.starts_with("Jarvis-")
    {
        ...
        warn!("Unknown model {slug} is used. This will degrade the performance of Jarvis.");
        model_info!(
            slug,
            context_window: None,
