...
impl ModelsManager {
    ...
    ///
    /// Uses `jarvis_home` to store cached model metadata and initializes with built-in presets.
    pub fn new(jarvis_home: PathBuf, auth_manager: Arc<AuthManager>) -> Self {
        let cache_path = jarvis_home.join(MODEL_CACHE_FILE);
    ...
    async fn fetch_and_update_models(&self) -> CoreResult<()> {
        let _timer =
            jarvis_otel::start_global_timer("Jarvis.remote_models.fetch_update.duration_ms", &[]);
    ...
    async fn try_load_cache(&self) -> bool {
        let _timer =
            jarvis_otel::start_global_timer("Jarvis.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::models_manager::client_version_to_whole();
...
mod tests {
    ...
    async fn refresh_available_models_sorts_by_priority() {
        ...
        let jarvis_home = tempdir().expect("temp dir");
        ...
        let manager =
            ModelsManager::with_provider(jarvis_home.path().to_path_buf(), auth_manager, provider);
