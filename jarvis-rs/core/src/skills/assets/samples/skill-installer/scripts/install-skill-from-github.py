#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $JARVIS_HOME/skills."""
...
def _jarvis_home() -> str:
    return os.environ.get("JARVIS_HOME", os.path.expanduser("~/.jarvis"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "jarvis")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "jarvis-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_jarvis_home(), "skills")
