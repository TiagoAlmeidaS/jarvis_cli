...
const SYSTEM_SKILLS_MARKER_FILENAME: &str = ".jarvis-system-skills.marker";
const SYSTEM_SKILLS_MARKER_SALT: &str = "v1";

/// Returns the on-disk cache location for embedded system skills.
///
/// This is typically located at `jarvis_home/skills/.system`.
pub(crate) fn system_cache_root_dir(jarvis_home: &Path) -> PathBuf {
    AbsolutePathBuf::try_from(jarvis_home)
        .and_then(|jarvis_home| system_cache_root_dir_abs(&jarvis_home))
...
fn system_cache_root_dir_abs(jarvis_home: &AbsolutePathBuf) -> std::io::Result<AbsolutePathBuf> {
    jarvis_home
        .join(SKILLS_DIR_NAME)?
        .join(SYSTEM_SKILLS_DIR_NAME)
}

/// Installs embedded system skills into `jarvis_home/skills/.system`.
///
...
/// install is skipped.
pub(crate) fn install_system_skills(jarvis_home: &Path) -> Result<(), SystemSkillsError> {
    let jarvis_home = AbsolutePathBuf::try_from(jarvis_home)
        .map_err(|source| SystemSkillsError::io("normalize Jarvis home dir", source))?;
    let skills_root_dir = jarvis_home
        .join(SKILLS_DIR_NAME)
    ...
    let dest_system = system_cache_root_dir_abs(&jarvis_home)
