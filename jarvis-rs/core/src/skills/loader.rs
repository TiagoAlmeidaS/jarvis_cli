        ...
        config_layer_stack.get_layers(ConfigLayerStackOrdering::HighestPrecedenceFirst, true)
    {
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // Deprecated user skills location (`$jarvis_home/skills`), kept for backward
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
                ...
                // Embedded system skills are cached under `$jarvis_home/skills/.system` and are a
            ...
            ConfigLayerSource::System { .. } => {
                // The system config layer lives under `/etc/Jarvis/` on Unix, so treat
                // `/etc/Jarvis/skills` as admin-scoped skills.
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
...
fn discover_skills_under_root(root: &Path, scope: SkillScope, outcome: &mut SkillLoadOutcome) {
    ...
    // Follow symlinked directories for user, admin, and repo skills. System skills are written by Jarvis itself.
...
mod tests {
    ...
    const REPO_ROOT_CONFIG_DIR_NAME: &str = ".jarvis";

    async fn make_config(jarvis_home: &TempDir) -> Config {
        make_config_for_cwd(jarvis_home, jarvis_home.path().to_path_buf()).await
    }

    async fn make_config_for_cwd(jarvis_home: &TempDir, cwd: PathBuf) -> Config {
        let trust_root = cwd
            .ancestors()
    ...
    -> anyhow::Result<()> {
        ...
        let system_folder = tmp.path().join("etc/Jarvis");
        let home_folder = tmp.path().join("home");
        let user_folder = home_folder.join("Jarvis");
    ...
    fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
        ...
        let dot_codex = project_root.join(".jarvis");
    ...
    fn write_skill(jarvis_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
        write_skill_at(&jarvis_home.path().join("skills"), dir, name, description)
    }
    ...
    fn write_system_skill(
        ...
        write_skill_at(
            &jarvis_home.path().join("skills/.system"),
            dir,
            name,
            description,
        )
    ...
    async fn loads_skill_dependencies_metadata_from_yaml() {
        let jarvis_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&jarvis_home, "demo", "dep-skill", "from json");
...
"#,
        ...
        let cfg = make_config(&jarvis_home).await;
    ...
    async fn loads_skill_interface_metadata_from_yaml() {
        let jarvis_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&jarvis_home, "demo", "ui-skill", "from json");
        let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    async fn loads_skills_via_symlinked_subdir_for_user_scope() {
        ...
        fs::create_dir_all(jarvis_home.path().join("skills")).unwrap();
        symlink_dir(shared.path(), &jarvis_home.path().join("skills/shared"));
    ...
    async fn does_not_loop_on_symlink_cycle_for_user_scope() {
        ...
        // Create a cycle:
        //   $jarvis_home/skills/cycle/loop -> $jarvis_home/skills/cycle
        let cycle_dir = jarvis_home.path().join("skills/cycle");
    ...
    async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
        ...
        let cfg = make_config_for_cwd(&jarvis_home, repo_dir.path().to_path_buf()).await;
        let outcome = load_skills(&cfg);
    ...
    async fn system_scope_ignores_symlinked_subdir() {
        ...
        let system_root = jarvis_home.path().join("skills/.system");
    ...
    async fn loads_valid_skill() {
        let jarvis_home = tempfile::tempdir().expect("tempdir");
        let skill_path = write_skill(&jarvis_home, "demo", "demo-skill", "does things\ncarefully");
        let cfg = make_config(&jarvis_home).await;
    ...
    async fn skips_hidden_and_invalid() {
        let jarvis_home = tempfile::tempdir().expect("tempdir");
        let hidden_dir = jarvis_home.path().join("skills/.hidden");
        ...
        let invalid_dir = jarvis_home.path().join("skills/invalid");
    ...
    async fn enforces_length_limits() {
        ...
        write_skill(&jarvis_home, "max-len", "max-len", &max_desc);
        let cfg = make_config(&jarvis_home).await;
        ...
        write_skill(&jarvis_home, "too-long", "too-long", &too_long_desc);
        let outcome = load_skills(&cfg);
        assert_eq!(outcome.skills.len(), 1);
        assert_eq!(outcome.errors.len(), 1);
        assert!(
    ...
    async fn loads_skills_from_all_codex_dirs_under_project_root() {
        ...
        let cfg = make_config_for_cwd(&jarvis_home, nested_dir).await;
    ...
    async fn loads_skills_from_codex_dir_when_not_git_repo() {
        ...
        let cfg = make_config_for_cwd(&jarvis_home, work_dir.path().to_path_buf()).await;
    ...
    async fn keeps_duplicate_names_from_repo_and_user() {
        ...
        let user_skill_path = write_skill(&jarvis_home, "user", "dupe-skill", "from user");
    ...
    async fn repo_skills_search_does_not_escape_repo_root() {
        ...
        let cfg = make_config_for_cwd(&jarvis_home, repo_dir).await;
    ...
    async fn loads_skills_when_cwd_is_file_in_repo() {
        ...
        let cfg = make_config_for_cwd(&jarvis_home, file_path).await;
    ...
    async fn loads_skills_from_system_cache_when_present() {
        ...
        let skill_path = write_system_skill(&jarvis_home, "system", "system-skill", "from system");
