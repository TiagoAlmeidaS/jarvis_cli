...
impl SkillsManager {
    pub fn new(jarvis_home: PathBuf) -> Self {
        if let Err(err) = install_system_skills(&jarvis_home) {
...
mod tests {
    ...
    fn write_user_skill(jarvis_home: &TempDir, dir: &str, name: &str, description: &str) {
        let skill_dir = jarvis_home.path().join("skills").join(dir);
    ...
    async fn skills_for_config_seeds_cache_by_cwd() {
        ...
        let skills_manager = SkillsManager::new(jarvis_home.path().to_path_buf());

        write_user_skill(&jarvis_home, "a", "skill-a", "from a");
        ...
        write_user_skill(&jarvis_home, "b", "skill-b", "from b");
