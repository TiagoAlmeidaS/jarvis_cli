...
use jarvis_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: jarvis_protocol::config_types::WindowsSandboxLevel,
...
pub async fn process_exec_tool_call(
    ...
    let sandbox_type = match &sandbox_policy {
        ...
        _ => get_platform_sandbox(
            windows_sandbox_level != jarvis_protocol::config_types::WindowsSandboxLevel::Disabled,
        )
    ...
    let exec_env = manager
        .transform(
            ...
            sandbox_type,
            sandbox_cwd,
            jarvis_linux_sandbox_exe.as_ref(),
            windows_sandbox_level,
        )
        .map_err(JarvisErr::from)?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    windows_sandbox_level: jarvis_protocol::config_types::WindowsSandboxLevel,
    err: &str,
) {
    ...
    let level = if matches!(
        windows_sandbox_level,
        jarvis_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    };
    if let Some(metrics) = jarvis_otel::metrics::global() {
        let _ = metrics.counter(
            "Jarvis.windows_sandbox.createprocessasuserw_failed",
            1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_jarvis_home;
    use jarvis_protocol::config_types::WindowsSandboxLevel;
    use jarvis_windows_sandbox::run_windows_sandbox_capture;
    use jarvis_windows_sandbox::run_windows_sandbox_capture_elevated;
    ...
    let policy_str = serde_json::to_string(sandbox_policy).map_err(|err| {
        JarvisErr::Io(io::Error::other(format!(
            "failed to serialize Windows sandbox policy: {err}"
        )))
    })?;
    let sandbox_cwd = cwd.clone();
    let jarvis_home = find_jarvis_home().map_err(|err| {
        JarvisErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve jarvis_home: {err}"
        )))
    })?;
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(JarvisErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, JarvisErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(JarvisErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(JarvisErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(JarvisErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
                }));
            }
...
pub(crate) mod errors {
    ...
    impl From<SandboxTransformError> for JarvisErr {
        fn from(err: SandboxTransformError) -> Self {
            match err {
                ...
                SandboxTransformError::SeatbeltUnavailable => JarvisErr::UnsupportedOperation(
...
mod tests {
    ...
    async fn process_exec_tool_call_respects_cancellation_token() -> Result<()> {
        ...
        let output = match result {
            Err(JarvisErr::Sandbox(SandboxErr::Timeout { output })) => output,
