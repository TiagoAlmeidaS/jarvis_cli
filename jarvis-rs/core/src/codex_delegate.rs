...
use jarvis_protocol::protocol::ApplyPatchApprovalRequestEvent;
use jarvis_protocol::protocol::Event;
use jarvis_protocol::protocol::EventMsg;
use jarvis_protocol::protocol::ExecApprovalRequestEvent;
use jarvis_protocol::protocol::Op;
use jarvis_protocol::protocol::RequestUserInputEvent;
use jarvis_protocol::protocol::SessionSource;
use jarvis_protocol::protocol::SubAgentSource;
use jarvis_protocol::protocol::Submission;
...
/// Start an interactive sub-Jarvis thread and return IO channels.
///
...
pub(crate) async fn run_codex_thread_interactive(
...
) -> Result<Jarvis, JarvisErr> {
    let (tx_sub, rx_sub) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);
    let (tx_ops, rx_ops) = async_channel::bounded(SUBMISSION_CHANNEL_CAPACITY);

    let CodexSpawnOk { Jarvis, .. } = Jarvis::spawn(
        config,
        auth_manager,
        models_manager,
    ...
    let jarvis_for_events = Arc::clone(&Jarvis);
    ...
    let jarvis_for_ops = Arc::clone(&Jarvis);
    tokio::spawn(async move {
        forward_ops(jarvis_for_ops, rx_ops, cancel_token_ops).await;
    });
    ...
    Ok(Jarvis {
        ...
        tx_sub: tx_ops,
        rx_event: rx_sub,
        agent_status: Jarvis.agent_status.clone(),
        session: Arc::clone(&Jarvis.session),
    })
}
...
async fn forward_events(
    ...
    loop {
        tokio::select! {
            _ = &mut cancelled => {
                shutdown_delegate(&Jarvis).await;
                break;
            }
            event = Jarvis.next_event() => {
...
async fn shutdown_delegate(Jarvis: &Jarvis) {
    let _ = Jarvis.submit(Op::Interrupt).await;
    let _ = Jarvis.submit(Op::Shutdown {}).await;
...
async fn handle_exec_approval(
    ...
    let _ = Jarvis.submit(Op::ExecApproval { id, decision }).await;
}
...
async fn handle_patch_approval(
    ...
    .await;
    let _ = Jarvis.submit(Op::PatchApproval { id, decision }).await;
}
...
async fn handle_request_user_input(
    ...
    .await;
    let _ = Jarvis.submit(Op::UserInputAnswer { id, response }).await;
}
...
async fn await_approval_with_cancel<F>(
    ...
    sub_id: &str,
    cancel_token: &CancellationToken,
) -> jarvis_protocol::protocol::ReviewDecision
where
    F: core::future::Future<Output = jarvis_protocol::protocol::ReviewDecision>,
{
    tokio::select! {
        biased;
        _ = cancel_token.cancelled() => {
            parent_session
                .notify_approval(sub_id, jarvis_protocol::protocol::ReviewDecision::Abort)
                .await;
            jarvis_protocol::protocol::ReviewDecision::Abort
        }
        decision = fut => {
            decision
        }
...
mod tests {
    ...
    use jarvis_protocol::protocol::TurnAbortedEvent;
    ...
    async fn forward_events_cancelled_while_send_blocked_shuts_down_delegate() {
        ...
        let (session, ctx, _rx_evt) = crate::Jarvis::make_session_and_context_with_rx().await;
        let Jarvis = Arc::new(Jarvis {
            next_id: AtomicU64::new(0),
            tx_sub,
            rx_event: rx_events,
