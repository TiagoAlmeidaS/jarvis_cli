...
use jarvis_otel::config::OtelExporter;
use jarvis_otel::config::OtelHttpProtocol;
use jarvis_otel::config::OtelSettings;
use jarvis_otel::config::OtelTlsConfig as OtelTlsSettings;
use jarvis_otel::otel_provider::OtelProvider;
use std::error::Error;
...
/// Filter predicate for exporting only Jarvis-owned events via OTEL.
/// Keeps events that originated from jarvis_otel module
pub fn jarvis_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("jarvis_otel")
}
