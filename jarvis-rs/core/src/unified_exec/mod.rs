...
mod tests {
    ...
    async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
        ...
        write_stdin(
            &session,
            process_id,
            "export jarvis_INTERACTIVE_SHELL_VAR=Jarvis\n",
            2_500,
        )
        .await?;
        ...
        let out_2 = write_stdin(
            &session,
            process_id,
            "echo $jarvis_INTERACTIVE_SHELL_VAR\n",
            2_500,
        )
        .await?;
        assert!(
            out_2.output.contains("Jarvis"),
    ...
    async fn multi_unified_exec_sessions() -> anyhow::Result<()> {
        ...
        let out_2 =
            exec_command(&session, &turn, "echo $jarvis_INTERACTIVE_SHELL_VAR", 2_500).await?;
    ...
    async fn unified_exec_timeouts() -> anyhow::Result<()> {
        ...
        write_stdin(
            &session,
            process_id,
            format!("export jarvis_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
            2_500,
        )
        .await?;
        ...
        let out_2 = write_stdin(
            &session,
            process_id,
            "sleep 5 && echo $jarvis_INTERACTIVE_SHELL_VAR\n",
            10,
        )
        .await?;
        assert!(
    ...
    async fn requests_with_large_timeout_are_capped() -> anyhow::Result<()> {
        ...
        let result = exec_command(&session, &turn, "echo Jarvis", 120_000).await?;
    ...
    async fn completed_commands_do_not_persist_sessions() -> anyhow::Result<()> {
        let (session, turn) = test_session_and_turn().await;
        let result = exec_command(&session, &turn, "echo Jarvis", 2_500).await?;
