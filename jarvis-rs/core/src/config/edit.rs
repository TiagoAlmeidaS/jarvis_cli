...
pub async fn apply(
    ...
    let jarvis_home = jarvis_home.to_path_buf();
    let profile = profile.map(ToOwned::to_owned);
    task::spawn_blocking(move || apply_blocking(&jarvis_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(jarvis_home: &Path) -> Self {
        Self {
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.jarvis_home, self.profile.as_deref(), &self.edits)
    }
...
mod tests {
    ...
    fn blocking_set_model_top_level() {
        ...
        let jarvis_home = tmp.path();

        apply_blocking(
            jarvis_home,
            None,
            &[ConfigEdit::SetModel {
                model: Some("gpt-5.1-Jarvis".to_string()),
        ...
        let expected = r#"model = "gpt-5.1-Jarvis"
...
"#,
        ...
        let raw = std::fs::read_to_string(jarvis_home.join(CONFIG_TOML_FILE)).expect("read config");
    ...
    fn blocking_set_model_writes_through_symlink_chain() {
        ...
        let link_path = jarvis_home.join("config-link.toml");
        let config_path = jarvis_home.join(CONFIG_TOML_FILE);
...
"#;
    ...
    fn blocking_set_model_replaces_symlink_on_cycle() {
        let tmp = tempdir().expect("tmpdir");
        let jarvis_home = tmp.path();
        let link_a = jarvis_home.join("a.toml");
        let link_b = jarvis_home.join("b.toml");
        let config_path = jarvis_home.join(CONFIG_TOML_FILE);
...
"#;
        std::fs::write(jarvis_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
"#,
        ...
        apply_blocking(
            ...
            None,
            &[ConfigEdit::SetNoticeHideModelMigrationPrompt(
                "hide_gpt-5.1-Jarvis-max_migration_prompt".to_string(),
                true,
            )],
        )
        .expect("persist");
        ...
        let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-Jarvis-max_migration_prompt" = true
"#;
        assert_eq!(contents, expected);
    }
...
"#,
        ...
        apply_blocking(jarvis_home, None, &[ConfigEdit::ReplaceMcpServers(servers)])
            .expect("persist");
...
"#;
    ...
    fn blocking_clear_path_noop_when_missing() {
        ...
        assert!(
            !jarvis_home.join(CONFIG_TOML_FILE).exists(),
...
"#;
        ...
        let updated_expected = r#"model = "gpt-5.1-Jarvis"
model_reasoning_effort = "high"
"#;
        ConfigEditsBuilder::new(jarvis_home)
            .set_model(Some("gpt-5.1-Jarvis"), Some(ReasoningEffort::High))
