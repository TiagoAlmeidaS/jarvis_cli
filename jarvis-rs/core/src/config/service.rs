...
use jarvis_app_server_protocol::Config as ApiConfig;
use jarvis_app_server_protocol::ConfigBatchWriteParams;
use jarvis_app_server_protocol::ConfigLayerMetadata;
...
use jarvis_app_server_protocol::OverriddenMetadata;
...
impl ConfigService {
    ...
    pub fn new_with_defaults(jarvis_home: PathBuf) -> Self {
        Self {
            jarvis_home,
        ...
        let layers = match params.cwd.as_deref() {
            Some(cwd) => {
                ...
                crate::config::ConfigBuilder::default()
                    .jarvis_home(self.jarvis_home.clone())
    ...
    pub async fn load_user_saved_config(
        &self,
    ) -> Result<jarvis_app_server_protocol::UserSavedConfig, ConfigServiceError> {
        let layers = self
            .load_thread_agnostic_config()
            .await
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, &self.jarvis_home)
        ...
        if !config_edits.is_empty() {
            ConfigEditsBuilder::new(&self.jarvis_home)
    ...
    /// include any in-repo .jarvis/ folders because there is no cwd/project root
...
"#;
    ...
    async fn write_value_preserves_comments_and_order() -> Result<()> {
        ...
        let original = r#"# Jarvis user configuration
model = "gpt-5"
...
"#;
        ...
        let expected = r#"# Jarvis user configuration
model = "gpt-5"
approval_policy = "on-request"
