...
use jarvis_app_server_protocol::Tools;
use jarvis_app_server_protocol::UserSavedConfig;
use jarvis_protocol::config_types::AltScreenMode;
...
pub use jarvis_git::GhostSnapshotConfig;
...
pub(crate) fn test_config() -> Config {
    let jarvis_home = tempdir().expect("create temp dir");
    Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides::default(),
        jarvis_home.path().to_path_buf(),
    )
...
pub struct Config {
    ...
    /// Optional external notifier command. When set, Jarvis will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Jarvis
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.jarvis/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Jarvis"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Jarvis '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Jarvis home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Jarvis can reach out to for tool calls.
    pub mcp_servers: Constrained<HashMap<String, McpServerConfig>>,

    /// Preferred store for MCP OAuth credentials.
    /// keyring: Use an OS-specific keyring service.
    ///          Credentials stored in the keyring will only be readable by Jarvis unless the user explicitly grants access via OS-level keyring access.
    ///          https://github.com/openai/Jarvis/blob/main/Jarvis-rs/rmcp-client/src/oauth.rs#L2
    /// file: jarvis_home/.credentials.json
    ///       This file will be readable to Jarvis and other applications running as the same user.
    /// auto (default): keyring if available, otherwise file.
    pub mcp_oauth_credentials_store_mode: OAuthCredentialsStoreMode,

    /// Optional fixed port to use for the local HTTP callback server used during MCP OAuth login.
    ///
    /// When unset, Jarvis will bind to an ephemeral port chosen by the OS.
    ...
    /// Directory containing all Jarvis state (defaults to `~/.jarvis` but can be
    /// overridden by the `jarvis_home` environment variable).
    pub jarvis_home: PathBuf,
    ...
    /// Path to the `Jarvis-linux-sandbox` executable. This must be set if
    /// [`crate::exec::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `Jarvis-linux-sandbox`.
    pub jarvis_linux_sandbox_exe: Option<PathBuf>,
    ...
    /// When `true`, checks for Jarvis updates on startup and surfaces update prompts.
    /// Set to `false` only if your Jarvis updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Jarvis product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Jarvis product surfaces.
    /// Defaults to `true`.
...
impl ConfigBuilder {
    pub fn jarvis_home(mut self, jarvis_home: PathBuf) -> Self {
        self.jarvis_home = Some(jarvis_home);
        self
    }
    ...
    pub async fn build(self) -> std::io::Result<Config> {
        ...
        let jarvis_home = jarvis_home.map_or_else(find_jarvis_home, std::io::Result::Ok)?;
...
impl Config {
    ...
    pub fn load_default_with_cli_overrides(
        ...
        let config_toml = deserialize_config_toml_with_base(merged, &jarvis_home)?;
    ...
    /// ignores user settings. For example, the `Jarvis exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd` and `jarvis_linux_sandbox_exe`.
...
pub async fn load_config_as_toml_with_cli_overrides(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, jarvis_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .jarvis/ folders.
...
/// Patch `jarvis_home/config.toml` project state to set trust level.
/// Use with caution.
...
pub fn set_default_oss_provider(jarvis_home: &Path, provider: &str) -> std::io::Result<()> {
...
/// Base config deserialized from ~/.jarvis/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by Jarvis will likely degrade model performance.
    ...
    /// keyring: Use an OS-specific keyring service.
    ///          https://github.com/openai/Jarvis/blob/main/Jarvis-rs/rmcp-client/src/oauth.rs#L2
    /// file: Use a file in the Jarvis home directory.
    ...
    /// directories for `.jarvis` folders. Defaults to [".git"] when unset.
    #[serde(default)]
        ...
        let mut downgrade_workspace_write_if_unsupported = |policy: &mut SandboxPolicy| {
            if cfg!(target_os = "windows")
                ...
                && windows_sandbox_level
                    == jarvis_protocol::config_types::WindowsSandboxLevel::Disabled
...
impl Config {
    ...
    fn load_from_base_config_with_overrides(
        ...
        Self::load_config_with_layer_stack(cfg, overrides, jarvis_home, config_layer_stack)
    }
    ...
    fn load_config_with_layer_stack(
        ...
        let user_instructions = Self::load_instructions(Some(&jarvis_home));
    ...
    fn load_instructions(jarvis_dir: Option<&Path>) -> Option<String> {
        let base = jarvis_dir?;
...
/// Returns the path to the Jarvis configuration directory, which can be
/// specified by the `jarvis_HOME` environment variable. If not set, defaults to
/// `~/.jarvis`.
///
/// - If `jarvis_HOME` is set, the value must exist and be a directory. The
///   value will be canonicalized and this function will Err otherwise.
/// - If `jarvis_HOME` is not set, this function does not verify that the
///   directory exists.
pub fn find_jarvis_home() -> std::io::Result<PathBuf> {
    jarvis_utils_home_dir::find_jarvis_home()
}

/// Returns the path to the folder where Jarvis logs are stored. Does not verify
/// that the directory exists.
pub fn log_dir(cfg: &Config) -> std::io::Result<PathBuf> {
    let mut p = cfg.jarvis_home.clone();
    p.push("log");
    Ok(p)
}
...
"#,
    ...
    async fn project_profile_overrides_user_profile() -> std::io::Result<()> {
        ...
        std::fs::write(
            jarvis_home.path().join(CONFIG_TOML_FILE),
            format!(
                r#"
profile = "global"
...
"#,
        ...
        let project_config_dir = workspace.path().join(".jarvis");
...
"#,
        ...
        let config = ConfigBuilder::default()
            .jarvis_home(jarvis_home.path().to_path_buf())
            .harness_overrides(ConfigOverrides {
    ...
    async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
        let jarvis_home = TempDir::new()?;
        let managed_path = jarvis_home.path().join("managed_config.toml");
        let config_path = jarvis_home.path().join(CONFIG_TOML_FILE);
        ...
        let cwd = AbsolutePathBuf::try_from(jarvis_home.path())?;
        let config_layer_stack = load_config_layers_state(
            jarvis_home.path(),
            Some(cwd),
            &Vec::new(),
    ...
    async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
        ...
        let servers = load_global_mcp_servers(jarvis_home.path()).await?;
    ...
    async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
        ...
        let loaded = load_global_mcp_servers(jarvis_home.path()).await?;
...
"#,
        ...
        let err = load_global_mcp_servers(jarvis_home.path())
            .await
...
"#
    ...
    async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
        ...
        let cwd_path = PathBuf::from("/tmp/Jarvis-mcp");
        let servers = BTreeMap::from([(
            "docs".to_string(),
            McpServerConfig {
                transport: McpServerTransportConfig::Stdio {
        ...
        let serialized = std::fs::read_to_string(&config_path)?;
        assert!(
            serialized.contains(r#"cwd = "/tmp/Jarvis-mcp""#),
        ...
        match &docs.transport {
            McpServerTransportConfig::Stdio { cwd, .. } => {
                assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/Jarvis-mcp")));
            }
            other => panic!("unexpected transport {other:?}"),
        }
...
"#
    ...
    async fn set_model_updates_defaults() -> anyhow::Result<()> {
        ...
        ConfigEditsBuilder::new(jarvis_home.path())
            .set_model(Some("gpt-5.1-Jarvis"), Some(ReasoningEffort::High))
            .apply()
            .await?;

        let serialized =
            tokio::fs::read_to_string(jarvis_home.path().join(CONFIG_TOML_FILE)).await?;
        let parsed: ConfigToml = toml::from_str(&serialized)?;

        assert_eq!(parsed.model.as_deref(), Some("gpt-5.1-Jarvis"));
...
"#,
    ...
    async fn set_model_updates_profile() -> anyhow::Result<()> {
        ...
        ConfigEditsBuilder::new(jarvis_home.path())
            ...
            .set_model(Some("gpt-5.1-Jarvis"), Some(ReasoningEffort::Medium))
            .apply()
            .await?;
        ...
        assert_eq!(profile.model.as_deref(), Some("gpt-5.1-Jarvis"));
        assert_eq!(
...
"#,
    ...
    impl PrecedenceTestFixture {
        ...
        fn jarvis_home(&self) -> PathBuf {
            self.jarvis_home.path().to_path_buf()
        }
    }
    ...
    fn loads_compact_prompt_from_file() -> std::io::Result<()> {
        let jarvis_home = TempDir::new()?;
        let workspace = jarvis_home.path().join("workspace");
...
"#;
        ...
        let jarvis_home_temp_dir = TempDir::new().unwrap();
        ...
        Ok(PrecedenceTestFixture {
            cwd: cwd_temp_dir,
            jarvis_home: jarvis_home_temp_dir,
            cfg,
            model_provider_map,
            openai_provider,
    ...
    fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
        ...
        assert_eq!(
            Config {
                ...
                jarvis_home: fixture.jarvis_home(),
                config_layer_stack: Default::default(),
                history: History::default(),
                ephemeral: false,
                file_opener: UriBasedFileOpener::VsCode,
                jarvis_linux_sandbox_exe: None,
...
"#;
    ...
    fn test_set_default_oss_provider() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let jarvis_home = temp_dir.path();
        let config_path = jarvis_home.join(CONFIG_TOML_FILE);

        // Test setting valid provider on empty config
        set_default_oss_provider(jarvis_home, OLLAMA_OSS_PROVIDER_ID)?;
        let content = std::fs::read_to_string(&config_path)?;
        assert!(content.contains("oss_provider = \"ollama\""));

        // Test updating existing config
        std::fs::write(&config_path, "model = \"gpt-4\"\n")?;
        set_default_oss_provider(jarvis_home, LMSTUDIO_OSS_PROVIDER_ID)?;
        let content = std::fs::read_to_string(&config_path)?;
        ...
        let result = set_default_oss_provider(jarvis_home, "invalid_provider");
    ...
    fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
        ...
        let result = set_default_oss_provider(jarvis_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
        assert!(result.is_err());
        let error = result.unwrap_err();
        assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
        assert!(
...
"#,
        ...
        let err = ConfigBuilder::default()
            .jarvis_home(jarvis_home.path().to_path_buf())
            .fallback_cwd(Some(jarvis_home.path().to_path_buf()))
