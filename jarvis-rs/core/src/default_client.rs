...
use crate::spawn::jarvis_SANDBOX_ENV_VAR;
use jarvis_client::JarvisHttpClient;
pub use jarvis_client::JarvisRequestBuilder;
...
/// See https://github.com/openai/Jarvis/pull/3388/files for an example of what that would look like.
...
/// Parenthesis will be added by Jarvis. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "jarvis_cli_rs";
pub const jarvis_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str = "jarvis_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-Jarvis-residency";
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(jarvis_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(jarvis_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_originator(originator_value: &str) -> bool {
    originator_value == DEFAULT_ORIGINATOR
        || originator_value == "jarvis_vscode"
        || originator_value.starts_with("Jarvis ")
}
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Jarvis user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Jarvis user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Jarvis originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> JarvisHttpClient {
    let inner = build_reqwest_client();
    JarvisHttpClient::new(inner)
}
...
fn is_sandboxed() -> bool {
    std::env::var(jarvis_SANDBOX_ENV_VAR).as_deref() == Ok("seatbelt")
}
...
mod tests {
    ...
    fn is_first_party_originator_matches_known_values() {
        ...
        assert_eq!(is_first_party_originator("jarvis_vscode"), true);
        assert_eq!(is_first_party_originator("Jarvis Something Else"), true);
        assert_eq!(is_first_party_originator("jarvis_cli"), false);
    ...
    async fn test_create_client_sets_default_headers() {
        ...
        // User-Agent matches the computed Jarvis UA for that originator
    ...
    fn test_invalid_suffix_is_sanitized() {
        let prefix = "jarvis_cli_rs/0.0.0";
        let suffix = "bad\rsuffix";

        assert_eq!(
            sanitize_user_agent(format!("{prefix} ({suffix})"), prefix),
            "jarvis_cli_rs/0.0.0 (bad_suffix)"
        );
    }
