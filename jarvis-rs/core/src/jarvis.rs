...
use jarvis_protocol::dynamic_tools::DynamicToolResponse;
use jarvis_protocol::dynamic_tools::DynamicToolSpec;
use jarvis_protocol::items::PlanItem;
use jarvis_protocol::items::TurnItem;
use jarvis_protocol::items::UserMessageItem;
use jarvis_protocol::mcp::CallToolResult;
use jarvis_protocol::models::BaseInstructions;
use jarvis_protocol::models::format_allow_prefixes;
use jarvis_protocol::openai_models::ModelInfo;
use jarvis_protocol::protocol::FileChange;
use jarvis_protocol::protocol::HasLegacyEvent;
use jarvis_protocol::protocol::ItemCompletedEvent;
use jarvis_protocol::protocol::ItemStartedEvent;
use jarvis_protocol::protocol::RawResponseItemEvent;
...
use jarvis_protocol::protocol::TurnAbortReason;
use jarvis_protocol::protocol::TurnContextItem;
use jarvis_protocol::protocol::TurnStartedEvent;
use jarvis_protocol::request_user_input::RequestUserInputArgs;
...
/// The high-level interface to the Jarvis system.
...
/// Wrapper returned by [`Jarvis::spawn`] containing the spawned [`Jarvis`],
...
pub const INITIAL_SUBMIT_ID: &str = "";
pub const SUBMISSION_CHANNEL_CAPACITY: usize = 64;

impl Jarvis {
    /// Spawn a new [`Jarvis`] and initialize the session.
    ...
    pub(crate) async fn spawn(
        ...
        let exec_policy = ExecPolicyManager::load(&config.features, &config.config_layer_stack)
            .await
            .map_err(|err| JarvisErr::Fatal(format!("failed to load rules: {err}")))?;
        ...
        let session_configuration = SessionConfiguration {
            ...
            jarvis_home: config.jarvis_home.clone(),
            thread_name: None,
        ...
        // Generate a unique ID for the lifetime of this Jarvis session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.jarvis_home)
        })?;
    ...
    /// Use sparingly: prefer `submit()` so Jarvis is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, sub: Submission) -> CodexResult<()> {
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| JarvisErr::InternalAgentDied)?;
        Ok(())
    }
...
pub struct TurnContext {
    ...
    pub(crate) jarvis_linux_sandbox_exe: Option<PathBuf>,
...
pub(crate) struct SessionConfiguration {
    ...
    cwd: PathBuf,
    /// Directory containing all Jarvis state for this session.
    jarvis_home: PathBuf,
...
impl SessionConfiguration {
    pub(crate) fn jarvis_home(&self) -> &PathBuf {
        &self.jarvis_home
    }
...
impl Session {
    ...
    pub(crate) async fn jarvis_home(&self) -> PathBuf {
        let state = self.state.lock().await;
        state.session_configuration.jarvis_home().clone()
    }
    ...
    fn make_turn_context(
        ...
        TurnContext {
            ...
            jarvis_linux_sandbox_exe: per_turn_config.jarvis_linux_sandbox_exe.clone(),
    ...
    async fn new(
        ...
        let otel_manager = OtelManager::new(
            ...
            session_configuration.collaboration_mode.model(),
            session_configuration.collaboration_mode.model(),
            auth.and_then(JarvisAuth::get_account_id),
            auth.and_then(JarvisAuth::get_account_email),
            auth.map(JarvisAuth::api_auth_mode),
        ...
        let sandbox_state = SandboxState {
            sandbox_policy: session_configuration.sandbox_policy.get().clone(),
            jarvis_linux_sandbox_exe: config.jarvis_linux_sandbox_exe.clone(),
                ...
                if let Some(prev) = rollout_items.iter().rev().find_map(|it| {
                    ...
                    if prev != curr {
                        ...
                        self.send_event(
                         ...
                         Consider switching back to `{prev}` as it may affect Jarvis performance."
                                ),
                            }),
                        )
                            .await;
                ...
                Err(err) => {
                    ...
                    self.send_event_raw(Event {
                        ...
                        msg: EventMsg::Error(ErrorEvent {
                            ...
                            jarvis_error_info: Some(JarvisErrorInfo::BadRequest),
                        }),
                    })
                    .await;
                    return Err(err);
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&jarvis_home, amendment)
            .await?;
    ...
    pub(crate) async fn record_model_warning(&self, message: impl Into<String>, ctx: &TurnContext) {
        self.services
            .otel_manager
            .counter("Jarvis.model_warning", 1, &[]);
    ...
    pub(crate) async fn notify_stream_error(
        ...
        turn_context: &TurnContext,
        message: impl Into<String>,
        jarvis_error: JarvisErr,
    ) {
        let additional_details = jarvis_error.to_string();
        let jarvis_error_info = JarvisErrorInfo::ResponseStreamDisconnected {
            http_status_code: jarvis_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            jarvis_error_info: Some(jarvis_error_info),
    ...
    async fn refresh_mcp_servers_inner(
        ...
        let sandbox_state = SandboxState {
            ...
            jarvis_linux_sandbox_exe: turn_context.jarvis_linux_sandbox_exe.clone(),
...
mod handlers {
    ...
    use jarvis_protocol::protocol::ThreadNameUpdatedEvent;
    use jarvis_protocol::protocol::ThreadRolledBackEvent;
    use jarvis_protocol::protocol::TurnAbortReason;
    use jarvis_protocol::protocol::WarningEvent;
    use jarvis_protocol::request_user_input::RequestUserInputResponse;
    ...
    use jarvis_protocol::config_types::Settings;
    use jarvis_protocol::dynamic_tools::DynamicToolResponse;
    use jarvis_protocol::mcp::RequestId as ProtocolRequestId;
    use jarvis_protocol::user_input::UserInput;
    ...
    pub async fn resolve_elicitation(
        ...
        decision: jarvis_protocol::approvals::ElicitationAction,
    ) {
        let action = match decision {
            jarvis_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
            jarvis_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
            jarvis_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
        };
            ...
            let event = Event {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        offset,
                        log_id,
                        entry: entry_opt.map(|e| jarvis_protocol::message_history::HistoryEntry {
            ...
            Err(err) => {
                let event = Event {
                    ...
                    msg: EventMsg::Error(ErrorEvent {
                        ...
                        jarvis_error_info: Some(JarvisErrorInfo::Other),
                    }),
                };
                sess.send_event_raw(event).await;
            }
        ...
        if num_turns == 0 {
            sess.send_event_raw(Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    jarvis_error_info: Some(JarvisErrorInfo::ThreadRollbackFailed),
                }),
            })
            .await;
            return;
    ...
    ///
    /// This appends the name to `jarvis_home/sessions_index.jsonl` via `session_index::append_thread_name` for the
    ...
    pub async fn set_thread_name(sess: &Arc<Session>, sub_id: String, name: String) {
        ...
        let jarvis_home = sess.jarvis_home().await;
        if let Err(e) =
            session_index::append_thread_name(&jarvis_home, sess.conversation_id, &name).await
        {
            let event = Event {
                id: sub_id,
                msg: EventMsg::Error(ErrorEvent {
    ...
    pub async fn resolve(
        ...
        if !config.features.enabled(Feature::AutonomousIssueResolver) {
            let turn_context = sess.new_default_turn_with_sub_id(sub_id.clone()).await;
            let event = Event {
                id: sub_id,
                msg: EventMsg::Error(ErrorEvent {
                    message: "Autonomous issue resolver is disabled. Enable it with `[features].autonomous_issue_resolver = true` in config.toml or `--enable autonomous_issue_resolver`."
                        .to_string(),
                    jarvis_error_info: Some(JarvisErrorInfo::Other),
                }),
            };
        ...
        super::do_resolve_issue(sess, config, sub_id, issue_resolver_request).await;
    }
    ...
    pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
        ...
        sess.services
            ...
            .await;
        info!("Shutting down Jarvis instance");
        ...
        sess.services.otel_manager.counter(
            "Jarvis.conversation.turn.count",
...
pub(crate) async fn spawn_issue_resolver_thread(
    sess: Arc<Session>,
    config: Arc<Config>,
    _parent_turn_context: Arc<TurnContext>,
    sub_id: String,
    resolved: crate::issue_resolver_prompts::ResolvedIssueResolver,
) {
    use crate::issue_resolver::IssueResolverParams;
    use crate::issue_resolver::IssueResolverTask;

    let turn_context = sess.new_default_turn_with_sub_id(sub_id.clone()).await;
    sess.refresh_mcp_servers_if_requested(&turn_context).await;

    let params = IssueResolverParams {
        github_pat: resolved.github_pat.clone(),
        owner: resolved.owner.clone(),
        repo: resolved.repo.clone(),
        issue_number: resolved.issue_number.unwrap_or(0),
    };

    let task = IssueResolverTask::new(params);

    let prompt = resolved.prompt.clone();
    let owner = resolved.owner.clone();
    let repo = resolved.repo.clone();
    let issue_number = resolved.issue_number;
    ...
    sess.spawn_task(turn_context.clone(), input, task).await;

    sess.send_event(
        &turn_context,
        EventMsg::EnteredIssueResolverMode(jarvis_protocol::protocol::IssueResolverRequest {
            owner,
            repo,
            issue_number,
        }),
...
/// Internal implementation of issue resolution (used by resolve handler).
async fn do_resolve_issue(
    sess: &Arc<Session>,
    config: &Arc<Config>,
    sub_id: String,
    resolve_request: jarvis_protocol::protocol::IssueResolverRequest,
) {
...
pub async fn spawn_review_thread(
    sess: Arc<Session>,
    config: Arc<Config>,
    ...
    let review_turn_context = TurnContext {
        ...
        final_output_json_schema: None,
        jarvis_linux_sandbox_exe: parent_turn_context.jarvis_linux_sandbox_exe.clone(),
        tool_call_gate: Arc::new(ReadinessFlag::new()),
...
pub async fn run_turn(
    ...
    // Although from the perspective of Jarvis.rs, TurnDiffTracker has the lifecycle of a Task which contains
    ...
    loop {
        ...
        .await
        {
            ...
            Err(JarvisErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(JarvisErr::InvalidImageRequest()) => {
                let mut state = sess.state.lock().await;
                error_or_panic(
...
fn filter_codex_apps_mcp_tools(
    ...
    mcp_tools.retain(|_, tool| {
        if tool.server_name != jarvis_APPS_MCP_SERVER_NAME {
            return true;
        }
        let Some(connector_id) = jarvis_apps_connector_id(tool) else {
            return false;
        };
...
fn jarvis_apps_connector_id(tool: &crate::mcp_connection_manager::ToolInfo) -> Option<&str> {
...
async fn run_sampling_request(
    ...
    loop {
        ...
        .await
        {
            ...
            Err(JarvisErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(JarvisErr::ContextWindowExceeded);
            }
            Err(JarvisErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, rate_limits).await;
                }
                return Err(JarvisErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                JarvisErr::Stream(_, requested_delay) => {
...
fn agent_message_text(item: &jarvis_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            jarvis_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: jarvis_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(jarvis_protocol::items::AgentMessageItem {
...
async fn try_run_sampling_request(
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(jarvis_async_utils::CancelErr::Cancelled) => break Err(JarvisErr::TurnAborted),
        };

        let event = match event {
            Some(res) => res?,
            None => {
                break Err(JarvisErr::Stream(
...
pub fn get_last_assistant_message_from_turn(responses: &[ResponseItem]) -> Option<String> {
...
mod tests {
    ...
    use crate::turn_diff_tracker::TurnDiffTracker;
    use jarvis_app_server_protocol::AppInfo;
    ...
    use jarvis_protocol::mcp::CallToolResult as McpCallToolResult;
    ...
    async fn thread_rollback_fails_when_turn_in_progress() {
        ...
        *sess.active_turn.lock().await = Some(crate::state::ActiveTurn::default());
        ...
        assert_eq!(
            error_event.jarvis_error_info,
            Some(JarvisErrorInfo::ThreadRollbackFailed)
        );
    ...
    async fn set_rate_limits_retains_previous_credits() {
        let jarvis_home = tempfile::tempdir().expect("create temp dir");
        let config = build_test_config(jarvis_home.path()).await;
        let config = Arc::new(config);
        ...
        let initial = RateLimitSnapshot {
            ...
            plan_type: Some(jarvis_protocol::account::PlanType::Plus),
        };
    ...
    async fn set_rate_limits_updates_plan_type_when_present() {
        ...
        let update = RateLimitSnapshot {
            ...
            secondary: None,
            credits: None,
            plan_type: Some(jarvis_protocol::account::PlanType::Pro),
        };
        state.set_rate_limits(update.clone());
        ...
        loop {
            ...
            match evt.msg {
                EventMsg::Error(payload)
                    if payload.jarvis_error_info == Some(JarvisErrorInfo::ThreadRollbackFailed) =>
                {
                    return payload;
                }
                _ => continue,
    ...
    async fn build_test_config(jarvis_home: &Path) -> Config {
        ConfigBuilder::default()
            .jarvis_home(jarvis_home.to_path_buf())
            .build()
            .await
    ...
    #[cfg(test)]
    pub async fn make_session_and_context() -> (Session, TurnContext) {
        ...
        let conversation_id = ThreadId::default();
        let auth_manager =
            AuthManager::from_auth_for_testing(JarvisAuth::from_api_key("Test API Key"));
        ...
        let skills_manager = Arc::new(SkillsManager::new(config.jarvis_home.clone()));
    ...
    #[cfg(test)]
    pub async fn make_session_and_context_with_rx() -> (
        Arc<Session>,
        Arc<TurnContext>,
