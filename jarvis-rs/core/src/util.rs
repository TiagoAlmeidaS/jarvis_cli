...
/// `jarvis_feedback::CodexFeedback::metadata_layer()` is installed, these fields are captured and
...
///
/// ```rust
/// jarvis_core::feedback_tags!(model = "gpt-5", cached = true);
/// jarvis_core::feedback_tags!(provider = provider_id, request_id = request_id);
/// ```
#[macro_export]
...
mod tests {
  ...
  "error": {
    ...
    fn resume_command_prefers_name_over_id() {
        ...
        assert_eq!(command, Some("Jarvis resume my-thread".to_string()));
    }
    ...
    fn resume_command_quotes_thread_name_when_needed() {
        ...
        assert_eq!(
            command,
            Some("Jarvis resume -- -starts-with-dash".to_string())
        );

        let command = resume_command(Some("two words"), None);
        assert_eq!(command, Some("Jarvis resume 'two words'".to_string()));

        let command = resume_command(Some("quote'case"), None);
        assert_eq!(command, Some("Jarvis resume \"quote'case\"".to_string()));
    }
}
