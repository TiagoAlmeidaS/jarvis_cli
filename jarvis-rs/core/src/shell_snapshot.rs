...
impl ShellSnapshot {
    pub fn start_snapshotting(
        ...
        tokio::spawn(
            async move {
                let timer = otel_manager.start_timer("Jarvis.shell_snapshot.duration_ms", &[]);
                let snapshot =
                    ShellSnapshot::try_new(&jarvis_home, snapshot_session_id, &snapshot_shell)
                        .await
                        .map(Arc::new);
                let success = if snapshot.is_some() { "true" } else { "false" };
                let _ = timer.map(|timer| timer.record(&[("success", success)]));
                otel_manager.counter("Jarvis.shell_snapshot", 1, &[("success", success)]);
    ...
    async fn try_new(jarvis_home: &Path, session_id: ThreadId, shell: &Shell) -> Option<Self> {
        // File to store the snapshot
        let extension = match shell.shell_type {
            ShellType::PowerShell => "ps1",
            _ => "sh",
        };
        let path = jarvis_home
            .join(SNAPSHOT_DIR)
        ...
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&jarvis_home, cleanup_session_id).await {
...
async fn run_script_with_timeout(
    ...
    unsafe {
        handler.pre_exec(|| {
            jarvis_utils_pty::process_group::detach_from_tty()?;
            Ok(())
        });
    }
...
pub async fn cleanup_stale_snapshots(jarvis_home: &Path, active_session_id: ThreadId) -> Result<()> {
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path = find_thread_path_by_id_str(jarvis_home, session_id).await?;
...
mod tests {
    ...
    async fn write_rollout_stub(jarvis_home: &Path, session_id: ThreadId) -> Result<PathBuf> {
        let dir = jarvis_home
            .join("sessions")
            .join("2025")
            .join("01")
    ...
    async fn cleanup_stale_snapshots_removes_stale_rollouts() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(jarvis_home, stale_session).await?;
    ...
    async fn cleanup_stale_snapshots_skips_active_session() -> Result<()> {
        ...
        let rollout_path = write_rollout_stub(jarvis_home, active_session).await?;
        fs::write(&active_snapshot, "active").await?;

        set_file_mtime(&rollout_path, SNAPSHOT_RETENTION + Duration::from_secs(60))?;

        cleanup_stale_snapshots(jarvis_home, active_session).await?;
