...
//!
//! The history is stored at `~/.jarvis/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.jarvis`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.jarvis_home.clone();
...
pub(crate) async fn append_entry(
    ...
    // Resolve `~/.jarvis/history.jsonl` and ensure the parent directory exists.
...
mod tests {
    ...
    async fn append_entry_trims_history_when_beyond_max_bytes() {
        ...
        let history_path = jarvis_home.path().join("history.jsonl");
