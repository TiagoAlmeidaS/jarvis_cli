...
pub type Result<T> = std::result::Result<T, JarvisErr>;
...
pub enum JarvisErr {
    ...
    #[error(
        "Jarvis ran out of room in the model's context window. Start a new thread or clear earlier history before retrying."
    )]
    ...
    #[error(
        "To use Jarvis with your ChatGPT plan, upgrade to Plus: https://chatgpt.com/explore/plus."
    )]
    UsageNotIncluded,
    ...
    #[error("Jarvis-linux-sandbox was required but not provided")]
...
impl From<CancelErr> for JarvisErr {
    fn from(_: CancelErr) -> Self {
        JarvisErr::TurnAborted
    }
}
...
impl JarvisErr {
    pub fn is_retryable(&self) -> bool {
        match self {
            JarvisErr::TurnAborted
            | JarvisErr::Interrupted
            | JarvisErr::EnvVar(_)
            | JarvisErr::Fatal(_)
            | JarvisErr::UsageNotIncluded
            | JarvisErr::QuotaExceeded
            | JarvisErr::InvalidImageRequest()
            | JarvisErr::InvalidRequest(_)
            | JarvisErr::RefreshTokenFailed(_)
            | JarvisErr::UnsupportedOperation(_)
            | JarvisErr::Sandbox(_)
            | JarvisErr::LandlockSandboxExecutableNotProvided
            | JarvisErr::RetryLimit(_)
            | JarvisErr::ContextWindowExceeded
            | JarvisErr::ThreadNotFound(_)
            | JarvisErr::AgentLimitReached { .. }
            | JarvisErr::Spawn
            | JarvisErr::SessionConfiguredNotFirstEvent
            | JarvisErr::UsageLimitReached(_)
            | JarvisErr::ModelCap(_) => false,
            JarvisErr::Stream(..)
            | JarvisErr::Timeout
            | JarvisErr::UnexpectedStatus(_)
            | JarvisErr::ResponseStreamFailed(_)
            | JarvisErr::ConnectionFailed(_)
            | JarvisErr::InternalServerError
            | JarvisErr::InternalAgentDied
            | JarvisErr::Io(_)
            | JarvisErr::Json(_)
            | JarvisErr::TokioJoin(_) => true,
            #[cfg(target_os = "linux")]
            JarvisErr::LandlockRuleset(_) | JarvisErr::LandlockPathFd(_) => false,
        }
    }
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        let message = match self.plan_type.as_ref() {
            Some(PlanType::Known(KnownPlan::Plus)) => format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/Jarvis/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
            ...
            Some(PlanType::Known(KnownPlan::Free)) | Some(PlanType::Known(KnownPlan::Go)) => {
                format!(
                    "You've hit your usage limit. Upgrade to Plus to continue using Jarvis (https://chatgpt.com/explore/plus),{}",
                    retry_suffix_after_or(self.resets_at.as_ref())
                )
            }
            Some(PlanType::Known(KnownPlan::Pro)) => format!(
                "You've hit your usage limit. Visit https://chatgpt.com/Jarvis/settings/usage to purchase more credits{}",
                retry_suffix_after_or(self.resets_at.as_ref())
            ),
...
impl JarvisErr {
    /// Minimal shim so that existing `e.downcast_ref::<JarvisErr>()` checks continue to compile
    ...
    pub fn to_codex_protocol_error(&self) -> JarvisErrorInfo {
        match self {
            JarvisErr::ContextWindowExceeded => JarvisErrorInfo::ContextWindowExceeded,
            JarvisErr::UsageLimitReached(_)
            | JarvisErr::QuotaExceeded
            | JarvisErr::UsageNotIncluded => JarvisErrorInfo::UsageLimitExceeded,
            JarvisErr::ModelCap(err) => JarvisErrorInfo::ModelCap {
                model: err.model.clone(),
                reset_after_seconds: err.reset_after_seconds,
            },
            JarvisErr::RetryLimit(_) => JarvisErrorInfo::ResponseTooManyFailedAttempts {
                http_status_code: self.http_status_code_value(),
            },
            JarvisErr::ConnectionFailed(_) => JarvisErrorInfo::HttpConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            JarvisErr::ResponseStreamFailed(_) => JarvisErrorInfo::ResponseStreamConnectionFailed {
                http_status_code: self.http_status_code_value(),
            },
            JarvisErr::RefreshTokenFailed(_) => JarvisErrorInfo::Unauthorized,
            JarvisErr::SessionConfiguredNotFirstEvent
            | JarvisErr::InternalServerError
            | JarvisErr::InternalAgentDied => JarvisErrorInfo::InternalServerError,
            JarvisErr::UnsupportedOperation(_)
            | JarvisErr::ThreadNotFound(_)
            | JarvisErr::AgentLimitReached { .. } => JarvisErrorInfo::BadRequest,
            JarvisErr::Sandbox(_) => JarvisErrorInfo::SandboxError,
            _ => JarvisErrorInfo::Other,
        }
    }
    ...
    pub fn to_error_event(&self, message_prefix: Option<String>) -> ErrorEvent {
        ...
        ErrorEvent {
            message,
            jarvis_error_info: Some(self.to_codex_protocol_error()),
        }
    }

    pub fn http_status_code_value(&self) -> Option<u16> {
        let http_status_code = match self {
            JarvisErr::RetryLimit(err) => Some(err.status),
            JarvisErr::UnexpectedStatus(err) => Some(err.status),
            JarvisErr::ConnectionFailed(err) => err.source.status(),
            JarvisErr::ResponseStreamFailed(err) => err.source.status(),
            _ => None,
        };
...
pub fn get_error_message_ui(e: &JarvisErr) -> String {
    let message = match e {
        JarvisErr::Sandbox(SandboxErr::Denied { output }) => {
        ...
        JarvisErr::Sandbox(SandboxErr::Timeout { output }) => {
            format!(
...
mod tests {
    ...
    fn usage_limit_reached_error_formats_plus_plan() {
        ...
        assert_eq!(
            err.to_string(),
            "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/Jarvis/settings/usage to purchase more credits or try again later."
        );
    }
    ...
    fn model_cap_error_maps_to_protocol() {
        let err = JarvisErr::ModelCap(ModelCapError {
            model: "boomslang".to_string(),
    ...
    fn sandbox_denied_uses_aggregated_output_when_stderr_empty() {
        ...
        let err = JarvisErr::Sandbox(SandboxErr::Denied {
    ...
    fn to_error_event_handles_response_stream_failed() {
        ...
        let err = JarvisErr::ResponseStreamFailed(ResponseStreamFailed {
            source,
        ...
        assert_eq!(
            event.jarvis_error_info,
            Some(JarvisErrorInfo::ResponseStreamConnectionFailed {
    ...
    fn usage_limit_reached_error_formats_free_plan() {
        ...
        assert_eq!(
            err.to_string(),
            "You've hit your usage limit. Upgrade to Plus to continue using Jarvis (https://chatgpt.com/explore/plus), or try again later."
        );
    }
    ...
    fn usage_limit_reached_error_formats_pro_plan_with_reset() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Visit https://chatgpt.com/Jarvis/settings/usage to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
    ...
    fn unexpected_status_prefers_error_message_when_present() {
        let err = UnexpectedResponseError {
            ...
            body: r#"{"error":{"message":"Workspace is not authorized in this region."},"status":401}"#
                .to_string(),
            url: Some("https://chatgpt.com/backend-api/Jarvis/responses".to_string()),
            cf_ray: None,
            request_id: Some("req-123".to_string()),
        };
        ...
        assert_eq!(
            err.to_string(),
            format!(
                "unexpected status {status}: Workspace is not authorized in this region., url: https://chatgpt.com/backend-api/Jarvis/responses, request id: req-123"
            )
        );
    }
    ...
    fn unexpected_status_includes_cf_ray_and_request_id() {
        ...
        assert_eq!(
            err.to_string(),
            format!(
                "unexpected status {status}: plain text error, url: https://chatgpt.com/backend-api/Jarvis/responses, cf-ray: 9c81f9f18f2fa49d-LHR, request id: req-xyz"
            )
        );
    }
    ...
    fn usage_limit_reached_includes_hours_and_minutes() {
        ...
        with_now_override(base, move || {
            ...
            let expected = format!(
                "You've hit your usage limit. Upgrade to Pro (https://chatgpt.com/explore/pro), visit https://chatgpt.com/Jarvis/settings/usage to purchase more credits or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
    ...
    fn usage_limit_reached_with_promo_message() {
        ...
        with_now_override(base, move || {
            ...
            let err = UsageLimitReachedError {
                ...
                rate_limits: Some(rate_limit_snapshot()),
                promo_message: Some(
                    "To continue using Jarvis, start a free trial of <PLAN> today".to_string(),
                ),
            };
            let expected = format!(
                "You've hit your usage limit. To continue using Jarvis, start a free trial of <PLAN> today, or try again at {expected_time}."
            );
            assert_eq!(err.to_string(), expected);
        });
    }
