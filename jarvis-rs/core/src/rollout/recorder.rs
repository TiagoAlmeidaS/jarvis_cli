//! Persist Jarvis session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use jarvis_protocol::protocol::ResumedHistory;
...
type ThreadMetadataBuilder = (); // Placeholder when state feature is disabled
...
///
/// ```ignore
/// $ jq -C . ~/.jarvis/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.jarvis/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Jarvis home directory.
    ...
    async fn list_threads_with_db_fallback(
        ...
        #[cfg(feature = "state")]
        {
            let state_db_ctx = state_db::open_if_present(jarvis_home, default_provider).await;
        ...
        if archived {
            let root = jarvis_home.join(ARCHIVED_SESSIONS_SUBDIR);
...
fn create_log_file(config: &Config, conversation_id: ThreadId) -> std::io::Result<LogFileInfo> {
    // Resolve ~/.jarvis/sessions/YYYY/MM/DD and create it if missing.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.jarvis_home.clone();
...
#[cfg(feature = "state")]
impl From<jarvis_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: jarvis_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
            .map(|item| ThreadItem {
