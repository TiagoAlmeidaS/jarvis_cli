...
use crate::state_db;
use jarvis_file_search as file_search;
...
#[cfg(feature = "state")]
impl From<jarvis_state::Anchor> for Cursor {
    fn from(anchor: jarvis_state::Anchor) -> Self {
...
pub(crate) async fn get_threads(
    ...
    let root = jarvis_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.jarvis/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
            ...
            && lines_scanned < head_limit + USER_EVENT_SCAN_LIMIT)
    {
        ...
        match rollout_line.item {
            ...
            RolloutItem::ResponseItem(item) => {
                ...
                if let jarvis_protocol::models::ResponseItem::Message { role, content, .. } = &item
                    && role == "user"
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    #[cfg(feature = "state")]
    {
        let state_db_ctx = state_db::open_if_present(jarvis_home, "").await;
    ...
    let mut root = jarvis_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
        return Ok(None);
    }
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(jarvis_home, SESSIONS_SUBDIR, id_str).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(jarvis_home, ARCHIVED_SESSIONS_SUBDIR, id_str).await
}
