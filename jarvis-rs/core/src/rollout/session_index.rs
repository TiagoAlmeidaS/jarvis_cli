...
pub async fn append_thread_name(
    ...
    append_session_index_entry(jarvis_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(jarvis_home);
...
pub async fn find_thread_path_by_name_str(
    ...
    let Some(thread_id) = find_thread_id_by_name(jarvis_home, name).await? else {
        return Ok(None);
    };
    super::list::find_thread_path_by_id_str(jarvis_home, &thread_id.to_string()).await
}

fn session_index_path(jarvis_home: &Path) -> PathBuf {
    jarvis_home.join(SESSION_INDEX_FILE)
}
