...
#[cfg(feature = "state")]
use jarvis_state::BackfillStats;
#[cfg(feature = "state")]
use jarvis_state::DB_ERROR_METRIC;
#[cfg(feature = "state")]
use jarvis_state::DB_METRIC_BACKFILL;
#[cfg(feature = "state")]
use jarvis_state::DB_METRIC_BACKFILL_DURATION_MS;
#[cfg(feature = "state")]
use jarvis_state::ExtractionOutcome;
#[cfg(feature = "state")]
use jarvis_state::ThreadMetadataBuilder;
#[cfg(feature = "state")]
use jarvis_state::apply_rollout_item;
use std::cmp::Reverse;
use std::path::Path;
...
pub(crate) async fn backfill_sessions(
    runtime: &jarvis_state::StateRuntime,
    config: &Config,
    otel: Option<&OtelManager>,
) {
    let timer = otel.and_then(|otel| otel.start_timer(DB_METRIC_BACKFILL_DURATION_MS, &[]).ok());
    let sessions_root = config.jarvis_home.join(rollout::SESSIONS_SUBDIR);
    let archived_root = config.jarvis_home.join(rollout::ARCHIVED_SESSIONS_SUBDIR);
...
mod tests {
    ...
    use chrono::Utc;
    use jarvis_protocol::ThreadId;
    use jarvis_protocol::protocol::CompactedItem;
    use jarvis_protocol::protocol::RolloutItem;
