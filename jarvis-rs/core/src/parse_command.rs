...
/// The easiest way to iterate is to add unit tests and have Jarvis fix the implementation.
...
/// Tests are at the top to encourage using TDD + Jarvis to fix the implementation.
mod tests {
    use super::*;
    ...
    fn supports_grep_recursive_current_dir() {
        assert_parsed(
            &vec_str(&["grep", "-R", "jarvis_SANDBOX_ENV_VAR", "-n", "."]),
            vec![ParsedCommand::Search {
                cmd: "grep -R jarvis_SANDBOX_ENV_VAR -n .".to_string(),
                query: Some("jarvis_SANDBOX_ENV_VAR".to_string()),
                path: Some(".".to_string()),
            }],
        );
    }
    ...
    fn supports_grep_recursive_specific_file() {
        assert_parsed(
            &vec_str(&[
                "grep",
                "-R",
                "jarvis_SANDBOX_ENV_VAR",
                "-n",
                "core/src/spawn.rs",
            ]),
            vec![ParsedCommand::Search {
                cmd: "grep -R jarvis_SANDBOX_ENV_VAR -n core/src/spawn.rs".to_string(),
                query: Some("jarvis_SANDBOX_ENV_VAR".to_string()),
    ...
    fn supports_cd_and_rg_files() {
        assert_parsed(
            &shlex_split_safe("cd Jarvis-rs && rg --files"),
            vec![ParsedCommand::ListFiles {
                cmd: "rg --files".to_string(),
                path: None,
            }],
    ...
    fn supports_single_string_script_with_cd_and_pipe() {
        ...
        assert_parsed(
            &vec_str(&["bash", "-lc", inner]),
            vec![ParsedCommand::Search {
                cmd: "rg -n jarvis_api Jarvis-rs -S".to_string(),
                query: Some("jarvis_api".to_string()),
                path: Some("Jarvis-rs".to_string()),
            }],
        );
    }
