...
use jarvis_protocol::protocol::McpStartupCompleteEvent;
use jarvis_protocol::protocol::McpStartupFailure;
use jarvis_protocol::protocol::McpStartupStatus;
use jarvis_protocol::protocol::McpStartupUpdateEvent;
...
pub const MCP_SANDBOX_STATE_CAPABILITY: &str = "Jarvis/sandbox-state";

/// Custom MCP request to push sandbox state updates.
/// When used, the `params` field of the notification is [`SandboxState`].
pub const MCP_SANDBOX_STATE_METHOD: &str = "Jarvis/sandbox-state/update";
...
fn normalize_codex_apps_tool_title(
    ...
    if server_name != jarvis_APPS_MCP_SERVER_NAME {
...
async fn start_server_task(
    ...
    let params = InitializeRequestParam {
        ...
        client_info: Implementation {
            name: "Jarvis-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("Jarvis".into()),
            icons: None,
            website_url: None,
        },
...
fn mcp_init_error_display(
    ...
    {
        format!(
            "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = jarvis_GITHUB_PERSONAL_ACCESS_TOKEN"
        )
    } else if is_mcp_client_auth_required_error(err) {
        format!(
            "The {server_name} MCP server is not logged in. Run `Jarvis mcp login {server_name}`."
        )
...
mod tests {
    use super::*;
    use jarvis_protocol::protocol::McpAuthStatus;
