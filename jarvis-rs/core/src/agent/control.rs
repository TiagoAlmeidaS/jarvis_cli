...
/// is used for every sub-agent spawned by Jarvis. By doing so, we make sure the guards are
/// scoped to a user session.
#[derive(Clone, Default)]
pub(crate) struct AgentControl {
    /// Weak handle back to the global thread registry/state.
    /// This is `Weak` to avoid reference cycles and shadow persistence of the form
    /// `ThreadManagerState -> JarvisThread -> Session -> SessionServices -> ThreadManagerState`.
...
impl AgentControl {
    ...
    pub(crate) async fn spawn_agent(
        ...
        prompt: String,
        session_source: Option<jarvis_protocol::protocol::SessionSource>,
    ...
    pub(crate) async fn send_prompt(
        ...
        let result = state
            ...
            .await;
        if matches!(result, Err(JarvisErr::InternalAgentDied)) {
    ...
    fn upgrade(&self) -> CodexResult<Arc<ThreadManagerState>> {
        self.manager
            .upgrade()
            .ok_or_else(|| JarvisErr::UnsupportedOperation("thread manager dropped".to_string()))
    }
}
...
mod tests {
    ...
    use jarvis_protocol::protocol::TurnCompleteEvent;
    ...
    impl AgentControlHarness {
        ...
        async fn start_thread(&self) -> (ThreadId, Arc<JarvisThread>) {
            let new_thread = self
                .manager
    ...
    async fn send_prompt_errors_when_thread_missing() {
        ...
        assert_matches!(err, JarvisErr::ThreadNotFound(id) if id == thread_id);
    }
    ...
    async fn spawn_agent_respects_max_threads_limit() {
        ...
        let JarvisErr::AgentLimitReached {
            max_threads: seen_max_threads,
        } = err
        else {
            panic!("expected JarvisErr::AgentLimitReached");
        };
    ...
    async fn spawn_agent_limit_shared_across_clones() {
        ...
        let JarvisErr::AgentLimitReached { max_threads } = err else {
            panic!("expected JarvisErr::AgentLimitReached");
        };
