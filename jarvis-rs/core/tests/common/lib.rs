...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\Jarvis\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.jarvis`.
pub async fn load_default_config_for_test(jarvis_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        jarvis_linux_sandbox_exe: Some(
            jarvis_utils_cargo_bin::cargo_bin("Jarvis-linux-sandbox")
                .expect("should find binary for Jarvis-linux-sandbox"),
        ),
    ...
    F: FnMut(&jarvis_core::protocol::EventMsg) -> bool,
{
    ...
    wait_for_event_with_timeout(Jarvis, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(Jarvis: &JarvisThread, matcher: F) -> T
where
    F: Fn(&jarvis_core::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(Jarvis, |ev| matcher(ev).is_some()).await;
    matcher(&ev).unwrap()
}
    ...
    F: FnMut(&jarvis_core::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), Jarvis.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    jarvis_core::spawn::jarvis_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    jarvis_core::spawn::jarvis_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

pub fn format_with_current_shell(command: &str) -> Vec<String> {
    jarvis_core::shell::default_user_shell().derive_exec_args(command, true)
}
...
pub fn format_with_current_shell_non_login(command: &str) -> Vec<String> {
    jarvis_core::shell::default_user_shell().derive_exec_args(command, false)
}
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    jarvis_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
