...
impl TestCodexBuilder {
    ...
    pub fn with_auth(mut self, auth: JarvisAuth) -> Self {
        self.auth = auth;
        self
    }
    ...
    async fn build_from_config(
        ...
        let new_conversation = match resume_from {
            Some(path) => {
                let auth_manager = jarvis_core::AuthManager::from_auth_for_testing(auth);
                thread_manager
        ...
        Ok(TestCodex {
            ...
            cwd,
            config,
            Jarvis: new_conversation.thread,
    ...
    async fn prepare_config(
        ...
        if let Ok(path) = jarvis_utils_cargo_bin::cargo_bin("Jarvis") {
            config.jarvis_linux_sandbox_exe = Some(path);
        }
...
impl TestCodex {
    ...
    pub fn jarvis_home_path(&self) -> &Path {
    ...
    pub async fn submit_turn_with_policies(
        ...
        wait_for_event(&self.Jarvis, |event| {
...
pub fn test_codex() -> TestCodexBuilder {
    TestCodexBuilder {
        config_mutators: vec![],
        auth: JarvisAuth::from_api_key("dummy"),
        pre_build_hooks: vec![],
        home: None,
    }
}
