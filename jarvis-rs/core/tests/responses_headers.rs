...
use jarvis_core::WEB_SEARCH_ELIGIBLE_HEADER;
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/openai/Jarvis.git",
    ]);
    ...
    loop {
        ...
        if let Some(header_value) = maybe_header {
            let parsed: serde_json::Value = serde_json::from_str(&header_value)
                .expect("x-Jarvis-turn-metadata should be valid JSON");
    ...
    panic!("x-Jarvis-turn-metadata was never observed within 5s after git setup");
}
