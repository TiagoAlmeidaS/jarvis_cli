...
use jarvis_protocol::models::MessagePhase;
use jarvis_protocol::models::ReasoningItemContent;
use jarvis_protocol::models::ReasoningItemReasoningSummary;
...
/// Writes an `auth.json` into the provided `jarvis_home` with the specified parameters.
...
fn write_auth_json(
    ...
    std::fs::write(
        jarvis_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = jarvis_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![jarvis_protocol::models::ContentItem::InputText {
    ...
    let prior_system = jarvis_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![jarvis_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = jarvis_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Jarvis to resume from our file
    let jarvis_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_codex()
        .with_home(jarvis_home.clone())
        .with_config(|config| {
    ...
    Jarvis
        ...
        .await
        .unwrap();
    wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    let mut builder = test_codex().with_auth(JarvisAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_codex()
        .with_auth(JarvisAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    model_provider.base_url = Some(format!("{}/api/Jarvis", server.uri()));
    let mut builder = test_codex()
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/Jarvis/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let mut config = load_default_config_for_test(&jarvis_home).await;
    config.model_provider = model_provider;

    let auth_manager =
        match JarvisAuth::from_auth_storage(jarvis_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => jarvis_core::AuthManager::from_auth_for_testing(auth),
            Ok(None) => panic!("No JarvisAuth found in jarvis_home"),
            Err(e) => panic!("Failed to load JarvisAuth: {e}"),
        };
    ...
    let NewThread { thread: Jarvis, .. } = thread_manager
        .start_thread(config)
        .await
        .expect("create new conversation");
...
async fn skills_append_to_instructions() {
    ...
    let jarvis_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = jarvis_home.path().join("skills/demo");
    ...
    let jarvis_home_path = jarvis_home.path().to_path_buf();
    let mut builder = test_codex()
        .with_home(jarvis_home.clone())
        .with_auth(JarvisAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = jarvis_home_path;
        });
    let Jarvis = builder
        .build(&server)
        .await
    ...
    let _codex_home_guard = jarvis_home;
}
...
-> anyhow::Result<()> {
    ...
    .await;
    let TestCodex { Jarvis, .. } = test_codex().with_model("gpt-5.1").build(&server).await?;
...
async fn azure_responses_request_includes_store_and_reasoning_ids() {
    ...
    let auth_manager = AuthManager::from_auth_for_testing(JarvisAuth::from_api_key("Test API Key"));
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-Jarvis-primary-used-percent", "12.5")
        .insert_header("x-Jarvis-secondary-used-percent", "40.0")
        .insert_header("x-Jarvis-primary-window-minutes", "10")
        .insert_header("x-Jarvis-secondary-window-minutes", "60")
        .insert_header("x-Jarvis-primary-reset-at", "1704069000")
        .insert_header("x-Jarvis-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_codex()
        .with_auth(JarvisAuth::from_api_key("test"))
        .with_config(move |config| {
    ...
    let first_token_event =
        wait_for_event(&Jarvis, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is gpt-5.1-Jarvis-max in tests → 95% usable context window
    ...
    wait_for_event(&Jarvis, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-Jarvis-primary-used-percent", "100.0")
        .insert_header("x-Jarvis-secondary-used-percent", "87.5")
        .insert_header("x-Jarvis-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-Jarvis-primary-window-minutes", "15")
        .insert_header("x-Jarvis-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let token_event = wait_for_event(&Jarvis, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&Jarvis, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&Jarvis, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = JarvisErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_codex_auth() -> JarvisAuth {
    JarvisAuth::create_dummy_chatgpt_auth_for_testing()
}
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Jarvis sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
