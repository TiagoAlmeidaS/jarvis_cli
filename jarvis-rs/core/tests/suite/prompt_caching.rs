...
use jarvis_core::shell::Shell;
use jarvis_core::shell::default_user_shell;
...
async fn prompt_tools_are_consistent_across_requests() -> anyhow::Result<()> {
    ...
    let TestCodex {
        ...
        .with_config(|config| {
            ...
            config.model = Some("gpt-5.1-Jarvis-max".to_string());
...
async fn jarvis_mini_latest_tools() -> anyhow::Result<()> {
    skip_if_no_network!(Ok(()));
    use pretty_assertions::assert_eq;
    ...
    let TestCodex { Jarvis, .. } = test_codex()
        .with_config(|config| {
            ...
            config.features.enable(Feature::CollaborationModes);
            config.model = Some("Jarvis-mini-latest".to_string());
        })
        .build(&server)
        .await?;
...
-> anyhow::Result<()> {
    ...
    let TestCodex { Jarvis, config, .. } = test_codex()
        .with_config(|config| {
            config.user_instructions = Some("be consistent and helpful".to_string());
            config.features.enable(Feature::CollaborationModes);
        })
