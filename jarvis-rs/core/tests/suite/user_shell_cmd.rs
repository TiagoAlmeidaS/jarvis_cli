...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    Jarvis
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let _ = wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;
    Jarvis.submit(Op::Interrupt).await.unwrap();

    // Expect a TurnAborted(Interrupted) notification.
    let msg = wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:jarvis_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
