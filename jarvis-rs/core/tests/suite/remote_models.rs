...
const REMOTE_MODEL_SLUG: &str = "Jarvis-test";
...
async fn remote_models_remote_model_uses_unified_exec() -> Result<()> {
    ...
    let begin_event = wait_for_event_match(&Jarvis, |msg| match msg {
...
async fn remote_models_truncation_policy_without_override_preserves_remote() -> Result<()> {
    ...
    let slug = "Jarvis-test-truncation-policy";
...
async fn remote_models_truncation_policy_with_tool_output_override() -> Result<()> {
    ...
    let slug = "Jarvis-test-truncation-override";
    let remote_model = test_remote_model_with_policy(
        slug,
        ModelVisibility::List,
        1,
...
async fn remote_models_preserve_builtin_presets() -> Result<()> {
    ...
    let manager = ModelsManager::with_provider(
        ...
        jarvis_core::auth::AuthManager::from_auth_for_testing(auth),
        provider,
    );
    ...
    assert!(
        available
            .iter()
            .any(|model| model.model == "gpt-5.1-Jarvis-max"),
...
async fn remote_models_hide_picker_only_models() -> Result<()> {
    ...
    let server = MockServer::start().await;
    let remote_model = test_remote_model("Jarvis-auto-balanced", ModelVisibility::Hide, 0);
    mount_models_once(
        &server,
        ModelsResponse {
            models: vec![remote_model],
    ...
    let selected = manager
        ...
        .await;
    assert_eq!(selected, "gpt-5.2-Jarvis");
    ...
    let hidden = available
        .iter()
        .find(|model| model.model == "Jarvis-auto-balanced")
