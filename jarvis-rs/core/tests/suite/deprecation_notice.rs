...
use jarvis_core::config_loader::ConfigLayerEntry;
use jarvis_core::config_loader::ConfigLayerStack;
use jarvis_core::config_loader::ConfigRequirements;
...
async fn emits_deprecation_notice_for_legacy_feature_flag() -> anyhow::Result<()> {
    ...
    let TestCodex { Jarvis, .. } = builder.build(&server).await?;

    let notice = wait_for_event_match(&Jarvis, |event| match event {
    ...
    assert_eq!(
        details.as_deref(),
        Some(
            "Enable it with `--enable unified_exec` or `[features].unified_exec` in config.toml. See https://github.com/openai/Jarvis/blob/main/docs/config.md#feature-flags for details."
        ),
    );
