...
use jarvis_core::protocol::ReviewCodeLocation;
use jarvis_core::protocol::ReviewFinding;
use jarvis_core::protocol::ReviewLineRange;
use jarvis_core::protocol::ReviewOutputEvent;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Jarvis sandbox network restrictions.
    skip_if_no_network!();
    ...
    let Jarvis = new_conversation_for_server(&server, jarvis_home.clone(), |_| {}).await;
    ...
    let _entered = wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    // Also verify that a user message with the header and a formatted finding
    // was recorded back in the parent session's rollout.
    let path = Jarvis.rollout_path().expect("rollout path");
...
async fn review_filters_agent_message_related_events() {
    ...
    wait_for_event(&Jarvis, |event| match event {
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let Jarvis = new_conversation_for_server(&server, jarvis_home.clone(), |cfg| {
    ...
    // Wait for completion
    let _entered = wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&Jarvis, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = jarvis_home.path().join("resume.jsonl");
    {
        ...
        let user = jarvis_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = jarvis_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let Jarvis =
        resume_conversation_for_server(&server, jarvis_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let Jarvis = new_conversation_for_server(&server, jarvis_home.clone(), move |config| {
