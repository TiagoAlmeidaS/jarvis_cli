...
fn repo_root() -> std::path::PathBuf {
    ...
    jarvis_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn responses_mode_stream_cli() {
    ...
    let bin = jarvis_utils_cargo_bin::cargo_bin("Jarvis").unwrap();
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Jarvis to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// 2. Configures Jarvis to read from this fixture via jarvis_RS_SSE_FIXTURE env var
...
async fn responses_api_stream_cli() {
    ...
    cmd.env("jarvis_home", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("jarvis_RS_SSE_FIXTURE", fixture)
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the Jarvis CLI and invoke `exec`, which is what records a session.
    let bin = jarvis_utils_cargo_bin::cargo_bin("Jarvis").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
    ...
    let bin2 = jarvis_utils_cargo_bin::cargo_bin("Jarvis").unwrap();
    ...
    cmd2.arg("exec")
        ...
        .arg("resume")
        .arg("--last");
    cmd2.env("jarvis_home", home.path())
        .env("OPENAI_API_KEY", "dummy")
        .env("jarvis_RS_SSE_FIXTURE", &fixture)
        .env("OPENAI_BASE_URL", "http://unused.local");

    let output2 = cmd2.output().unwrap();
    assert!(output2.status.success(), "resume Jarvis-cli run failed");
...
async fn integration_git_info_unit_test() {
    ...
    let git_info = jarvis_core::git_info::collect_git_info(&git_repo).await;
