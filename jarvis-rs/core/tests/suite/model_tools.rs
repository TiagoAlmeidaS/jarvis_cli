...
async fn collect_tool_identifiers_for_model(model: &str) -> Vec<String> {
    ...
    let test = builder
        .build(&server)
        .await
        .expect("create test Jarvis conversation");
...
async fn model_selects_expected_tools() {
    ...
    let jarvis_tools = collect_tool_identifiers_for_model("Jarvis-mini-latest").await;
    assert_eq!(
        jarvis_tools,
        ...
        "Jarvis-mini-latest should expose the local shell tool",
    );

    let gpt5_codex_tools = collect_tool_identifiers_for_model("gpt-5-Jarvis").await;
    assert_eq!(
        gpt5_codex_tools,
        expected_default_tools(
            "shell_command",
        ...
        "gpt-5-Jarvis should expose the apply_patch tool",
    );

    let gpt51_codex_tools = collect_tool_identifiers_for_model("gpt-5.1-Jarvis").await;
    assert_eq!(
        gpt51_codex_tools,
        expected_default_tools(
            "shell_command",
        ...
        "gpt-5.1-Jarvis should expose the apply_patch tool",
    );
