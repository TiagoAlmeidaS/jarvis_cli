...
use jarvis_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(jarvis_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = jarvis_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(jarvis_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(jarvis_home, "sessions", id)
}

async fn upsert_thread_metadata(jarvis_home: &Path, thread_id: ThreadId, rollout_path: PathBuf) {
    let runtime = StateRuntime::init(jarvis_home.to_path_buf(), "test-provider".to_string(), None)
        .await
        .unwrap();
    ...
    builder.cwd = jarvis_home.to_path_buf();
...
async fn find_handles_gitignore_covering_codex_home_directory() {
    ...
    let jarvis_home = repo.path().join(".jarvis");
    std::fs::create_dir_all(&jarvis_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".jarvis/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&jarvis_home, id);

    let found = find_thread_path_by_id_str(&jarvis_home, &id.to_string())
        .await
        .unwrap();
...
async fn find_locates_rollout_file_written_by_recorder() -> std::io::Result<()> {
    ...
    let config = ConfigBuilder::default()
        .jarvis_home(home.path().to_path_buf())
        .build()
        .await?;
