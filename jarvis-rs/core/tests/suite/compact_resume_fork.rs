...
//! request payload that Jarvis would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(jarvis_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn compact_resume_and_fork_preserve_model_history_view() {
    ...
    let expected_model = "gpt-5.1-Jarvis";
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<JarvisThread>) {
...
async fn compact_conversation(conversation: &Arc<JarvisThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
async fn fetch_conversation_path(conversation: &Arc<JarvisThread>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager = jarvis_core::AuthManager::from_auth_for_testing(
        jarvis_core::JarvisAuth::from_api_key("dummy"),
    );
    manager
