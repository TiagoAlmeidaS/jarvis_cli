...
async fn wait_for_snapshot(jarvis_home: &Path) -> Result<PathBuf> {
    let snapshot_dir = jarvis_home.join("shell_snapshots");
...
async fn run_snapshot_command(command: &str) -> Result<SnapshotRun> {
    ...
    let end = wait_for_event_match(&Jarvis, |ev| match ev {
...
async fn linux_unified_exec_uses_shell_snapshot() -> Result<()> {
    ...
    assert!(run.snapshot_path.starts_with(&run.jarvis_home));
