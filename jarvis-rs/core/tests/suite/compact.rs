...
use jarvis_core::compact::SUMMARY_PREFIX;
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Jarvis.
    ...
    // Shut down Jarvis to flush rollout entries before inspecting the file.
    Jarvis.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let first = wait_for_event_match(&Jarvis, |ev| match ev {
    ...
    let last = wait_for_event_match(&Jarvis, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn manual_compact_emits_context_compaction_items() {
    ...
    Jarvis
        ...
        .await
        .unwrap();
    wait_for_event(&Jarvis, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    ...
    {
        let event = Jarvis.next_event().await.unwrap();
        match event.msg {
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        jarvis_protocol::models::ResponseItem::Message {
            id: None,
        ...
        jarvis_protocol::models::ResponseItem::Compaction {
...
async fn manual_compact_retries_after_context_window_error() {
    ...
    let EventMsg::BackgroundEvent(event) =
        wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::BackgroundEvent(_))).await
    else {
...
async fn auto_compact_counts_encrypted_reasoning_before_last_user() {
    ...
    let Jarvis = test_codex()
        .with_auth(JarvisAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            set_test_compact_prompt(config);
