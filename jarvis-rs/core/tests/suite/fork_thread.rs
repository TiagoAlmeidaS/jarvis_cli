...
async fn fork_thread_twice_drops_to_first_message() {
    ...
    let base_path = Jarvis.rollout_path().expect("rollout path");
    ...
    let fork1_path = jarvis_fork1.rollout_path().expect("rollout path");
    ...
    let fork2_path = jarvis_fork2.rollout_path().expect("rollout path");
