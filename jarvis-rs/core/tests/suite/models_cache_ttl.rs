...
const REMOTE_MODEL: &str = "Jarvis-test-ttl";
const VERSIONED_MODEL: &str = "Jarvis-test-versioned";
const MISSING_VERSION_MODEL: &str = "Jarvis-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "Jarvis-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let mut builder = test_codex().with_auth(JarvisAuth::create_dummy_chatgpt_auth_for_testing());
    ...
    let cache_path = config.jarvis_home.join(CACHE_FILE);
    ...
    let _ = wait_for_event(&Jarvis, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    ...
    builder = builder
        .with_pre_build_hook(move |home| {
            let cache = ModelsCache {
                fetched_at: Utc::now(),
                etag: None,
                client_version: Some(jarvis_core::models_manager::client_version_to_whole()),
...
async fn refreshes_when_cache_version_differs() -> Result<()> {
    ...
    builder = builder
        .with_pre_build_hook(move |home| {
            let client_version = jarvis_core::models_manager::client_version_to_whole();
            let cache = ModelsCache {
                fetched_at: Utc::now(),
                etag: None,
