...
use jarvis_core::exec::ExecToolCallOutput;
use jarvis_core::exec::SandboxType;
...
use jarvis_core::get_platform_sandbox;

fn skip_test() -> bool {
    if std::env::var(jarvis_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
        eprintln!("{jarvis_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
        return true;
    }
