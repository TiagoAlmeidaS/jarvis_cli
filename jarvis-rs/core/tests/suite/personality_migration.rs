...
use jarvis_core::SESSIONS_SUBDIR;
use jarvis_core::config::ConfigToml;
use jarvis_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use jarvis_core::personality_migration::PersonalityMigrationStatus;
use jarvis_core::personality_migration::maybe_migrate_personality;
...
async fn read_config_toml(jarvis_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(jarvis_home.join("config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(jarvis_home: &Path) -> io::Result<()> {
...
async fn write_archived_session_with_user_event(jarvis_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = jarvis_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}
