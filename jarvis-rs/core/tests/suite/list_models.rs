...
async fn list_models_returns_api_key_models() -> Result<()> {
    ...
    let config = load_default_config_for_test(&jarvis_home).await;
    let manager = ThreadManager::with_models_provider(
        JarvisAuth::from_api_key("sk-test"),
        ...
        migration_markdown: Some(
            indoc! {r#"
                **Jarvis just got an upgrade. Introducing {model_to}.**
                ...
                Jarvis is now powered by {model_to}, our latest frontier agentic coding model. It is smarter and faster than its predecessors and capable of long-running project-scale work. Learn more about {model_to} at https://openai.com/index/introducing-gpt-5-2-Jarvis
