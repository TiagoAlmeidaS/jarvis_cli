...
use jarvis_protocol::openai_models::ModelInstructionsVariables;
use jarvis_protocol::openai_models::ModelMessages;
use jarvis_protocol::openai_models::ModelVisibility;
...
async fn personality_does_not_mutate_base_instructions_without_template() {
    let jarvis_home = TempDir::new().expect("create temp dir");
...
async fn base_instructions_override_disables_personality_template() {
    ...
    let model_info = ModelsManager::construct_model_info_offline("gpt-5.2-Jarvis", &config);
...
async fn user_turn_personality_some_adds_update_message() -> anyhow::Result<()> {
    ...
    .await;
    let mut builder = test_codex()
        .with_model("exp-Jarvis-personality")
        .with_config(|config| {
            config.features.disable(Feature::RemoteModels);
            config.features.enable(Feature::Personality);
        });
...
async fn ignores_remote_personality_if_remote_models_disabled() -> anyhow::Result<()> {
    ...
    let remote_slug = "gpt-5.2-Jarvis";
    ...
    let mut builder = test_codex()
        .with_auth(jarvis_core::JarvisAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config.features.disable(Feature::RemoteModels);
            config.features.enable(Feature::Personality);
    ...
    assert!(
        instructions_text.contains("You are Jarvis, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals."),
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let remote_slug = "Jarvis-remote-default-personality";
...
async fn user_turn_personality_remote_model_template_includes_update_message() -> anyhow::Result<()>
{
    ...
    let remote_slug = "Jarvis-remote-personality";
    ...
    let mut builder = test_codex()
        ...
        .with_config(|config| {
            config.features.enable(Feature::RemoteModels);
            config.features.enable(Feature::Personality);
            config.model = Some("gpt-5.2-Jarvis".to_string());
        });
    let test = builder.build(&server).await?;
...
async fn wait_for_model_available(
    ...
    slug: &str,
    config: &jarvis_core::config::Config,
) {
