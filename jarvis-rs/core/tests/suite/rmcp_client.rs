...
/// This test writes to a fallback credentials file in jarvis_home.
/// Ideally, we wouldn't need to serialize the test but it's much more cumbersome to wire jarvis_home through the code.
#[serial(jarvis_home)]
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn streamable_http_with_oauth_round_trip() -> anyhow::Result<()> {
    ...
    let _guard = EnvVarGuard::set("jarvis_home", temp_home.path().as_os_str());
