...
async fn backfill_scans_existing_rollouts() -> Result<()> {
    ...
    let mut builder = test_codex()
        .with_pre_build_hook(move |jarvis_home| {
            let rollout_path = jarvis_home.join(&rollout_rel_path_for_hook);
    ...
    let db_path = jarvis_state::state_db_path(test.config.jarvis_home.as_path());
    let rollout_path = test.config.jarvis_home.join(&rollout_rel_path);
...
async fn tool_call_logs_include_thread_id() -> Result<()> {
    ...
    let mut found = None;
    for _ in 0..80 {
        let query = jarvis_state::LogQuery {
            descending: true,
            limit: Some(20),
            ..Default::default()
        };
