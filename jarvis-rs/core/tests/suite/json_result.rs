...
async fn jarvis_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    jarvis_returns_json_result("gpt-5.1".to_string()).await
}

#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn jarvis_returns_json_result_for_gpt5_codex() -> anyhow::Result<()> {
    jarvis_returns_json_result("gpt-5.1-Jarvis".to_string()).await
}

async fn jarvis_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("jarvis_output_schema".into()))
    ...
    let TestCodex { Jarvis, cwd, .. } = test_codex().build(&server).await?;
    ...
    let message = wait_for_event(&Jarvis, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
