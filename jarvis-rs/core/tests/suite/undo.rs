...
fn init_git_repo(path: &Path) -> Result<()> {
    ...
    git(path, &["config", "user.name", "Jarvis Tests"])?;
    git(path, &["config", "user.email", "Jarvis-tests@example.com"])?;

    // Create README.txt
    let readme_path = path.join("README.txt");
    fs::write(&readme_path, "Test repository initialized by Jarvis.\n")?;
...
async fn invoke_undo(Jarvis: &Arc<JarvisThread>) -> Result<UndoCompletedEvent> {
    Jarvis.submit(Op::Undo).await?;
    let event = wait_for_event_match(Jarvis, |msg| match msg {
...
async fn expect_successful_undo(Jarvis: &Arc<JarvisThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(Jarvis).await?;
    assert!(
        event.success,
...
async fn expect_failed_undo(Jarvis: &Arc<JarvisThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(Jarvis).await?;
    assert!(
        !event.success,
