...
const LINUX_SANDBOX_ARG0: &str = "Jarvis-linux-sandbox";
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if argv1 == jarvis_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                match jarvis_apply_patch::apply_patch(&patch_arg, &mut stdout, &mut stderr) {
                    Ok(()) => 0,
                    Err(_) => 1,
                }
            }
            None => {
                eprintln!("Error: {jarvis_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument.");
                1
            }
        };
    ...
    match prepend_path_entry_for_codex_aliases() {
        ...
        Err(err) => {
            // It is possible that Jarvis will proceed successfully even if
...
/// While we want to deploy the Jarvis CLI as a single executable for simplicity,
...
/// `Jarvis-linux-sandbox` we *directly* execute
/// [`jarvis_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.jarvis/.env` before creating any threads.
/// 2.  Construct a Tokio multi-thread runtime.
/// 3.  Derive the path to the current executable (so children can re-invoke the
///     sandbox) when running on Linux.
/// 4.  Execute the provided async `main_fn` inside that runtime, forwarding any
///     error. Note that `main_fn` receives `jarvis_linux_sandbox_exe:
///     Option<PathBuf>`, as an argument, which is generally needed as part of
///     constructing [`jarvis_core::config::Config`].
///
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    ...
    runtime.block_on(async move {
        let jarvis_linux_sandbox_exe: Option<PathBuf> = if cfg!(target_os = "linux") {
            std::env::current_exe().ok()
        } else {
            None
        };

        main_fn(jarvis_linux_sandbox_exe).await
    })
}

const ILLEGAL_ENV_VAR_PREFIX: &str = "jarvis_";

/// Load env vars from ~/.jarvis/.env.
///
...
/// Helper to set vars from a dotenvy iterator while filtering out `jarvis_` keys.
...
///   with the "secret" --Jarvis-run-as-apply-patch flag.
///
...
pub fn prepend_path_entry_for_codex_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    let jarvis_home = jarvis_core::config::find_jarvis_home()?;
    #[cfg(not(debug_assertions))]
    {
        // Guard against placing helpers in system temp directories outside debug builds.
        let temp_root = std::env::temp_dir();
        if jarvis_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (jarvis_home: {jarvis_home:?})"
                ),
            ));
        }
    }

    std::fs::create_dir_all(&jarvis_home)?;
    // Use a jarvis_home-scoped temp root to avoid cluttering the top-level directory.
    let temp_root = jarvis_home.join("tmp").join("arg0");
