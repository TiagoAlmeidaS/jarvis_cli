...
use jarvis_core::config::types::ShellEnvironmentPolicy;
use jarvis_core::error::JarvisErr;
use jarvis_core::error::SandboxErr;
use jarvis_core::exec::ExecParams;
use jarvis_core::exec::process_exec_tool_call;
use jarvis_core::exec_env::create_env;
use jarvis_core::protocol::SandboxPolicy;
use jarvis_core::protocol_config_types::WindowsSandboxLevel;
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> jarvis_core::exec::ExecToolCallOutput {
    ...
    let jarvis_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_codex-linux-sandbox");
    let jarvis_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
    ...
    let output = match result {
        Ok(output) => output,
        Err(JarvisErr::Sandbox(SandboxErr::Denied { output })) => *output,
        _ => {
