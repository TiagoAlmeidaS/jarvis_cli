...
Rebuild Jarvis-linux-sandbox on Linux with jarvis_BWRAP_ENABLE_FFI=1.\n\
Example:\n\
- cd Jarvis-rs && jarvis_BWRAP_ENABLE_FFI=1 cargo build -p Jarvis-linux-sandbox\n\
If this crate was already built without it, run:\n\
- cargo clean -p Jarvis-linux-sandbox\n\
Notes:\n\
- libcap headers must be available via pkg-config\n\
- bubblewrap sources expected at Jarvis-rs/vendor/bubblewrap (default)"
        );
    }
}
