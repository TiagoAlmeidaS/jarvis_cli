...
//! - sensitive subpaths such as `.git` and `.jarvis` remain read-only even when
...
pub(crate) fn create_bwrap_command_args(
    ...
    let bwrap_path = match bwrap_path {
        ...
        None => which::which("bwrap").map_err(|err| {
            JarvisErr::UnsupportedOperation(format!("bubblewrap (bwrap) not found on PATH: {err}"))
        })?,
    };
...
/// inside a writable root (e.g., `.jarvis -> ./decoy`). In that case we mount
