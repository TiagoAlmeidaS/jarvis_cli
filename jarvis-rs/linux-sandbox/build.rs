...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=jarvis_BWRAP_ENABLE_FFI");
    println!("cargo:rerun-if-env-changed=jarvis_BWRAP_SOURCE_DIR");
    ...
    let enable_ffi = matches!(env::var("jarvis_BWRAP_ENABLE_FFI"), Ok(value) if value == "1");
    if !enable_ffi {
        return;
    }
...
fn try_build_vendored_bwrap() -> Result<(), String> {
    ...
    std::fs::write(
        &config_h,
        r#"#pragma once
#define PACKAGE_STRING "bubblewrap built at Jarvis build-time"
"#,
    )
...
///
/// Priority:
/// 1. `jarvis_BWRAP_SOURCE_DIR` points at an existing bubblewrap checkout.
/// 2. The vendored bubblewrap tree under `Jarvis-rs/vendor/bubblewrap`.
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("jarvis_BWRAP_SOURCE_DIR") {
        let src_dir = PathBuf::from(path);
        if src_dir.exists() {
            return Ok(src_dir);
        }
        return Err(format!(
            "jarvis_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
...
Set jarvis_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under Jarvis-rs/vendor.",
        vendor_dir.display()
    ))
}
