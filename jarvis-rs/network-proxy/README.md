...
`jarvis-network-proxy` is jarvis's local network policy enforcement proxy. It runs:
...
`jarvis-network-proxy` reads from jarvis's merged `config.toml` (via `jarvis-core` config loading).
...
```bash
cargo run -p jarvis-network-proxy --
```
...
`jarvis-network-proxy` can be embedded as a library with a thin API:
...
This section documents the protections implemented by `jarvis-network-proxy`, and the boundaries of
