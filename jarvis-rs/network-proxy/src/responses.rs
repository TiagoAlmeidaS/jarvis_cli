...
pub fn blocked_message(reason: &str) -> &'static str {
    match reason {
        REASON_NOT_ALLOWED => "Jarvis blocked this request: domain not in allowlist.",
        REASON_NOT_ALLOWED_LOCAL => {
            "Jarvis blocked this request: local/private addresses not allowed."
        }
        REASON_DENIED => "Jarvis blocked this request: domain denied by policy.",
        REASON_METHOD_NOT_ALLOWED => {
            "Jarvis blocked this request: method not allowed in limited mode."
        }
        _ => "Jarvis blocked this request by network policy.",
    }
}
