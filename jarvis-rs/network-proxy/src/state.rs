...
use jarvis_core::config_loader::ConfigLayerStack;
...
use jarvis_core::config_loader::load_config_layers_state;
...
pub(crate) async fn build_config_state() -> Result<ConfigState> {
    // Load config through `Jarvis-core` so we inherit the same layer ordering and semantics as the
    // rest of Jarvis (system/managed layers, user layers, session flags, etc.).
    ...
    .await
    .context("failed to load Jarvis config")?;

    let cfg_path = jarvis_home.join(CONFIG_TOML_FILE);
...
fn enforce_trusted_constraints(
    layers: &jarvis_core::config_loader::ConfigLayerStack,
...
fn network_proxy_constraints_from_trusted_layers(
    ...
    for layer in layers.get_layers(
        jarvis_core::config_loader::ConfigLayerStackOrdering::LowestPrecedenceFirst,
        false,
    ) {
