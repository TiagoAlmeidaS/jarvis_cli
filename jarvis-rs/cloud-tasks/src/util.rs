...
use jarvis_login::AuthManager;

pub fn set_user_agent_suffix(suffix: &str) {
    if let Ok(mut guard) = jarvis_core::default_client::USER_AGENT_SUFFIX.lock() {
...
pub async fn build_chatgpt_headers() -> HeaderMap {
    ...
    set_user_agent_suffix("jarvis_cloud_tasks_tui");
    let ua = jarvis_core::default_client::get_codex_user_agent();
    let mut headers = HeaderMap::new();
    headers.insert(
        USER_AGENT,
        HeaderValue::from_str(&ua).unwrap_or(HeaderValue::from_static("Jarvis-cli")),
    );
...
pub fn task_url(base_url: &str, task_id: &str) -> String {
    ...
    if let Some(root) = normalized.strip_suffix("/backend-api") {
        return format!("{root}/Jarvis/tasks/{task_id}");
    }
    if let Some(root) = normalized.strip_suffix("/api/Jarvis") {
        return format!("{root}/Jarvis/tasks/{task_id}");
    }
    if normalized.ends_with("/Jarvis") {
        return format!("{normalized}/tasks/{task_id}");
    }
    format!("{normalized}/Jarvis/tasks/{task_id}")
}
