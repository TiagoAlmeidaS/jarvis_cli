...
pub enum Command {
    /// Submit a new Jarvis Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Jarvis Cloud task.
    Status(StatusCommand),
    /// List Jarvis Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Jarvis Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Jarvis Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Jarvis Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `Jarvis cloud` to browse).
    ...
    /// Git branch to run in Jarvis Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Jarvis Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Jarvis Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Jarvis Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
