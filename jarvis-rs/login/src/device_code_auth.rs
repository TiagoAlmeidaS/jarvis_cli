    ...
    if !resp.status().is_success() {
        ...
        if status == StatusCode::NOT_FOUND {
            return Err(io::Error::new(
                ...
                "device code login is not enabled for this Jarvis server. Use the browser login or verify the server URL.",
            ));
        }
...
fn print_device_code_prompt(verification_url: &str, code: &str) {
    ...
    println!(
        "\nWelcome to Jarvis [v{ANSI_GRAY}{version}{ANSI_RESET}]\n{ANSI_GRAY}OpenAI's command-line coding agent{ANSI_RESET}\n\
...
pub async fn request_device_code(opts: &ServerOptions) -> std::io::Result<DeviceCode> {
    ...
    let base_url = opts.oauth_provider.issuer().trim_end_matches('/');
    let api_base_url = format!("{base_url}/api/accounts");
    let client_id = opts.oauth_provider.client_id();
    let uc = request_user_code(&client, &api_base_url, &client_id).await?;

    Ok(DeviceCode {
        verification_url: format!("{base_url}/Jarvis/device"),
