...
use jarvis_core::auth::AuthDotJson;
use jarvis_core::auth::OAuthProvider;
use jarvis_core::auth::save_auth;
...
fn build_authorize_url(
    ...
    let query = vec![
        ...
        ("access_type".to_string(), "offline".to_string()), // For refresh token
        (
...
pub(crate) async fn persist_tokens_async(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        save_auth(&jarvis_home, &auth, auth_credentials_store_mode)
    })
    .await
...
fn compose_success_url(port: u16, issuer: &str, id_token: &str, access_token: &str) -> String {
    ...
    // For Google OAuth, we don't need OpenAI platform URL
    let platform_url = issuer;
