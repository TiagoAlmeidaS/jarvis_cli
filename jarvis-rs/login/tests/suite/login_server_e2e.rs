...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = jarvis_home.clone();

    let opts = ServerOptions {
        jarvis_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: jarvis_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = jarvis_home.join("auth.json");
...
async fn creates_missing_codex_home_dir() -> Result<()> {
    ...
    let tmp = tempdir()?;
    let jarvis_home = tmp.path().join("missing-subdir"); // does not exist
