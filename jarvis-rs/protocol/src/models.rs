...
use jarvis_utils_image::load_and_resize_to_fit;
...
use jarvis_git::GhostCommit;
use jarvis_utils_image::error::ImageProcessingError;
...
fn local_image_error_placeholder(
    ...
    ContentItem::InputText {
        text: format!(
            "Jarvis could not read the local image at `{}`: {}",
            path.display(),
            error
        ),
    }
...
fn unsupported_image_error_placeholder(path: &std::path::Path, mime: &str) -> ContentItem {
    ContentItem::InputText {
        text: format!(
            "Jarvis cannot attach image at `{}`: unsupported image format `{}`.",
            path.display(),
            mime
        ),
    }
...
// (Moved event mapping logic into Jarvis-core to avoid coupling protocol to UI-facing events.)
    ...
    fn includes_request_rule_instructions_when_enabled() {
        ...
        exec_policy
            .add_prefix_rule(
                ...
                jarvis_execpolicy::Decision::Allow,
            )
            .expect("add rule");
        let instructions = DeveloperInstructions::from_permissions_with_network(
            SandboxMode::WorkspaceWrite,
...
<svg xmlns="http://www.w3.org/2000/svg" width="1" height="1"></svg>"#,
        ...
        match item {
            ResponseInputItem::Message { content, .. } => {
                assert_eq!(content.len(), 1);
                let expected = format!(
                    "Jarvis cannot attach image at `{}`: unsupported image format `image/svg+xml`.",
                    svg_path.display()
                );
                match &content[0] {
