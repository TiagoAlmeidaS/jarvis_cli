...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Jarvis:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::jarvis_thread::JarvisThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// This is a local-only operation handled by Jarvis-core; it does not
    /// involve the model.
    SetThreadName { name: String },

    /// Request Jarvis to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,

    /// Request Jarvis to drop the last N user turns from in-memory context.
    ///
...
/// running the command proposed by Jarvis.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.jarvis`, `.git`, notably `.git/hooks`) under
...
impl SandboxPolicy {
    ...
    /// the current working directory and the per-user tmp dir on macOS. Network
            ...
            SandboxPolicy::WorkspaceWrite {
                ...
                roots
                    ...
                    .map(|writable_root| {
                        ...
                        // Make .agents/skills and .jarvis/config.toml and
                        // related files read-only to the agent, by default.
                        for subdir in &[".agents", ".jarvis"] {
                            #[allow(clippy::expect_used)]
                            let top_level_codex =
...
/// Jarvis errors that we expose to clients.
