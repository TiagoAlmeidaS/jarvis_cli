WARNING: this code is mainly generated by jarvis and should not be used in production

# jarvis-debug-client

A tiny interactive client for `jarvis app-server` (protocol v2 only). It prints
all JSON-RPC lines from the server and lets you send new turns as you type.

## Usage

Start the app-server client (it will spawn `jarvis app-server` itself):

```
cargo run -p jarvis-debug-client -- \
  --jarvis-bin jarvis \
...
```
cargo run -p jarvis-debug-client -- --thread-id thr_123
```

### CLI flags

- `--jarvis-bin <path>`: path to the `jarvis` binary (default: `jarvis`).
- `-c, --config key=value`: pass through `--config` overrides to `jarvis`.
