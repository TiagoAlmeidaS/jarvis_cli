...
pub fn sandbox_dir(jarvis_home: &Path) -> PathBuf {
    jarvis_home.join(".sandbox")
}

pub fn sandbox_secrets_dir(jarvis_home: &Path) -> PathBuf {
    jarvis_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(jarvis_home: &Path) -> PathBuf {
    sandbox_dir(jarvis_home).join("setup_marker.json")
}

pub fn sandbox_users_path(jarvis_home: &Path) -> PathBuf {
    sandbox_secrets_dir(jarvis_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| jarvis_home.to_path_buf());
    log_note(
        &format!(
...
fn find_setup_exe() -> PathBuf {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            let candidate = dir.join("Jarvis-windows-sandbox-setup.exe");
            if candidate.exists() {
                return candidate;
            }
        }
    }
    PathBuf::from("Jarvis-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(jarvis_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(jarvis_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(jarvis_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(jarvis_home);
    ...
    run_setup_exe(&payload, needs_elevation, jarvis_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, jarvis_home);
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, jarvis_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to jarvis_home or anything under jarvis_home/.sandbox.
    // These locations contain sandbox control/state and must remain tamper-resistant.
    let jarvis_home_key = canonical_path_key(jarvis_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(jarvis_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(jarvis_home));
