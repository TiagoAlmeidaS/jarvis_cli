...
use anyhow::Context;
use anyhow::Result;
use jarvis_windows_sandbox::allow_null_device;
use jarvis_windows_sandbox::convert_string_sid_to_sid;
use jarvis_windows_sandbox::create_process_as_user;
use jarvis_windows_sandbox::create_readonly_token_with_caps_from;
use jarvis_windows_sandbox::create_workspace_write_token_with_caps_from;
use jarvis_windows_sandbox::get_current_token_for_restriction;
use jarvis_windows_sandbox::hide_current_user_profile_dir;
use jarvis_windows_sandbox::log_note;
use jarvis_windows_sandbox::parse_policy;
use jarvis_windows_sandbox::to_wide;
use jarvis_windows_sandbox::SandboxPolicy;
...
struct RunnerRequest {
    ...
    // Writable location for logs (sandbox user's .jarvis).
    jarvis_home: PathBuf,
    // Real user's jarvis_home for shared data (caps, config).
...
pub fn main() -> Result<()> {
    ...
    let log_dir = Some(req.jarvis_home.as_path());
    hide_current_user_profile_dir(req.jarvis_home.as_path());
    log_note(
        &format!(
