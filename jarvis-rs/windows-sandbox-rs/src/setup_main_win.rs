...
use base64::Engine;
use jarvis_windows_sandbox::canonicalize_path;
use jarvis_windows_sandbox::convert_string_sid_to_sid;
use jarvis_windows_sandbox::ensure_allow_mask_aces_with_inheritance;
use jarvis_windows_sandbox::ensure_allow_write_aces;
use jarvis_windows_sandbox::extract_setup_failure;
use jarvis_windows_sandbox::hide_newly_created_users;
use jarvis_windows_sandbox::is_command_cwd_root;
use jarvis_windows_sandbox::load_or_create_cap_sids;
use jarvis_windows_sandbox::log_note;
use jarvis_windows_sandbox::path_mask_allows;
use jarvis_windows_sandbox::protect_workspace_codex_dir;
use jarvis_windows_sandbox::sandbox_dir;
use jarvis_windows_sandbox::sandbox_secrets_dir;
use jarvis_windows_sandbox::string_from_sid_bytes;
use jarvis_windows_sandbox::to_wide;
use jarvis_windows_sandbox::workspace_cap_sid_for_cwd;
use jarvis_windows_sandbox::write_setup_error_report;
use jarvis_windows_sandbox::SetupErrorCode;
use jarvis_windows_sandbox::SetupErrorReport;
use jarvis_windows_sandbox::SetupFailure;
use jarvis_windows_sandbox::LOG_FILE_NAME;
use jarvis_windows_sandbox::SETUP_VERSION;
...
pub fn main() -> Result<()> {
    ...
    if let Err(e) = &ret {
        ...
        if let Ok(jarvis_home) = std::env::var("jarvis_home") {
            let sbx_dir = sandbox_dir(Path::new(&jarvis_home));
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.jarvis_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.jarvis_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.jarvis_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    let workspace_sid_str = workspace_cap_sid_for_cwd(&payload.jarvis_home, &payload.command_cwd)?;
    ...
    if !refresh_only {
        lock_sandbox_dir(
            &sandbox_dir(&payload.jarvis_home),
            &payload.real_user,
            &sandbox_group_sid,
            GRANT_ACCESS,
            log,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.jarvis_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.jarvis_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            log,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.jarvis_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.jarvis_home).join("sandbox_users.json");
        if legacy_users.exists() {
            let _ = std::fs::remove_file(&legacy_users);
        }
    }

    // Protect the current workspace's `.jarvis` directory from tampering (write/delete) by using a
    // workspace-specific capability SID. If `.jarvis` doesn't exist yet, skip it (it will be picked
    // up on the next refresh).
    match unsafe { protect_workspace_codex_dir(&payload.command_cwd, workspace_psid) } {
        Ok(true) => {
            let cwd_codex = payload.command_cwd.join(".jarvis");
            log_line(
                log,
                &format!(
                    "applied deny ACE to protect workspace .jarvis {}",
                    cwd_codex.display()
                ),
            )?;
        }
