...
pub struct CapSids {
    ...
    /// apply per-workspace denies (e.g. protect `CWD/.jarvis`)
    /// without permanently affecting other workspaces.
    #[serde(default)]
    pub workspace_by_cwd: HashMap<String, String>,
}

pub fn cap_sid_file(jarvis_home: &Path) -> PathBuf {
    jarvis_home.join("cap_sid")
}
...
pub fn load_or_create_cap_sids(jarvis_home: &Path) -> Result<CapSids> {
    let path = cap_sid_file(jarvis_home);
    if path.exists() {
...
pub fn workspace_cap_sid_for_cwd(jarvis_home: &Path, cwd: &Path) -> Result<String> {
    let path = cap_sid_file(jarvis_home);
    let mut caps = load_or_create_cap_sids(jarvis_home)?;
...
mod tests {
    ...
    fn equivalent_cwd_spellings_share_workspace_sid_key() {
        ...
        let jarvis_home = temp.path().join("Jarvis-home");
        std::fs::create_dir_all(&jarvis_home).expect("create Jarvis home");
        ...
        let first_sid =
            workspace_cap_sid_for_cwd(&jarvis_home, canonical.as_path()).expect("first sid");
        let second_sid =
            workspace_cap_sid_for_cwd(&jarvis_home, alt_spelling.as_path()).expect("second sid");

        assert_eq!(first_sid, second_sid);

        let caps = load_or_create_cap_sids(&jarvis_home).expect("load caps");
