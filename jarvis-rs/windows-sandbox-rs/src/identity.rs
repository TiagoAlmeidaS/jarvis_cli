...
pub fn sandbox_setup_is_complete(jarvis_home: &Path) -> bool {
    let marker_ok = matches!(load_marker(jarvis_home), Ok(Some(marker)) if marker.version_matches());
    if !marker_ok {
        return false;
    }
    matches!(load_users(jarvis_home), Ok(Some(users)) if users.version_matches())
}

fn load_marker(jarvis_home: &Path) -> Result<Option<SetupMarker>> {
    let path = setup_marker_path(jarvis_home);
...
fn load_users(jarvis_home: &Path) -> Result<Option<SandboxUsersFile>> {
    let path = sandbox_users_path(jarvis_home);
...
fn select_identity(policy: &SandboxPolicy, jarvis_home: &Path) -> Result<Option<SandboxIdentity>> {
    let _marker = match load_marker(jarvis_home)? {
        Some(m) if m.version_matches() => m,
        _ => return Ok(None),
    };
    let users = match load_users(jarvis_home)? {
...
pub fn require_logon_sandbox_creds(
    ...
    let sandbox_dir = crate::setup::sandbox_dir(jarvis_home);
    ...
    // NOTE: Do not add jarvis_home/.sandbox to `needed_write`; it must remain non-writable by the
    ...
    let mut identity = match load_marker(jarvis_home)? {
        Some(marker) if marker.version_matches() => {
            _existing_marker = Some(marker.clone());
            let selected = select_identity(policy, jarvis_home)?;
            if selected.is_none() {
                setup_reason =
    ...
    if identity.is_none() {
        ...
        identity = select_identity(policy, jarvis_home)?;
    }
    // Always refresh ACLs (non-elevated) for current roots via the setup binary.
    crate::setup::run_setup_refresh(policy, policy_cwd, command_cwd, env_map, jarvis_home)?;
