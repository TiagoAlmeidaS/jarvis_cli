#!/usr/bin/env python3
"""Stage and optionally package the @jarvis/cli npm module."""
...
JARVIS_CLI_ROOT = SCRIPT_DIR.parent
REPO_ROOT = JARVIS_CLI_ROOT.parent
RESPONSES_API_PROXY_NPM_ROOT = REPO_ROOT / "jarvis-rs" / "responses-api-proxy" / "npm"
JARVIS_SDK_ROOT = REPO_ROOT / "sdk" / "typescript"

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "jarvis": ["jarvis", "rg"],
    "jarvis-responses-api-proxy": ["jarvis-responses-api-proxy"],
    "jarvis-sdk": ["jarvis"],
}
WINDOWS_ONLY_COMPONENTS: dict[str, list[str]] = {
    "jarvis": ["jarvis-windows-sandbox-setup", "jarvis-command-runner"],
}
COMPONENT_DEST_DIR: dict[str, str] = {
    "jarvis": "jarvis",
    "jarvis-responses-api-proxy": "jarvis-responses-api-proxy",
    "jarvis-windows-sandbox-setup": "jarvis",
    "jarvis-command-runner": "jarvis",
    "rg": "path",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the Jarvis CLI npm package.")
    parser.add_argument(
        "--package",
        choices=("jarvis", "jarvis-responses-api-proxy", "jarvis-sdk"),
        default="jarvis",
        help="Which npm package to stage (default: jarvis).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "jarvis":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/jarvis.js --version\n"
                    f"    node {staging_dir_str}/bin/jarvis.js --help\n\n"
                )
            elif package == "jarvis-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    node {staging_dir_str}/bin/jarvis-responses-api-proxy.js --help\n\n"
                )
            else:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="jarvis-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    if package == "jarvis":
        ...
        shutil.copy2(JARVIS_CLI_ROOT / "bin" / "jarvis.js", bin_dir / "jarvis.js")
        rg_manifest = JARVIS_CLI_ROOT / "bin" / "rg"
        ...
        package_json_path = JARVIS_CLI_ROOT / "package.json"
    elif package == "jarvis-responses-api-proxy":
        bin_dir = staging_dir / "bin"
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = RESPONSES_API_PROXY_NPM_ROOT / "bin" / "jarvis-responses-api-proxy.js"
        shutil.copy2(launcher_src, bin_dir / "jarvis-responses-api-proxy.js")
    ...
    elif package == "jarvis-sdk":
        package_json_path = JARVIS_SDK_ROOT / "package.json"
        stage_jarvis_sdk_sources(staging_dir)
    else:
...
def stage_jarvis_sdk_sources(staging_dir: Path) -> None:
    package_root = JARVIS_SDK_ROOT
    ...
    if not dist_src.exists():
        raise RuntimeError("jarvis-sdk build did not produce a dist directory.")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="jarvis-npm-pack-") as pack_dir_str:
