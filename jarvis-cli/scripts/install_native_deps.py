#!/usr/bin/env python3
"""Install Jarvis native binaries (Rust CLI plus ripgrep helpers)."""
...
DEFAULT_WORKFLOW_URL = "https://github.com/TiagoAlmeidaS/jarvis_cli/actions/runs/17952349351"  # rust-v0.40.0
...
class BinaryComponent:
    artifact_prefix: str  # matches the artifact filename prefix (e.g. jarvis-<target>.zst)
...
BINARY_COMPONENTS = {
    "jarvis": BinaryComponent(
        artifact_prefix="jarvis",
        dest_dir="jarvis",
        binary_basename="jarvis",
    ),
    "jarvis-responses-api-proxy": BinaryComponent(
        artifact_prefix="jarvis-responses-api-proxy",
        dest_dir="jarvis-responses-api-proxy",
        binary_basename="jarvis-responses-api-proxy",
    ),
    "jarvis-windows-sandbox-setup": BinaryComponent(
        artifact_prefix="jarvis-windows-sandbox-setup",
        dest_dir="jarvis",
        binary_basename="jarvis-windows-sandbox-setup",
        targets=WINDOWS_TARGETS,
    ),
    "jarvis-command-runner": BinaryComponent(
        artifact_prefix="jarvis-command-runner",
        dest_dir="jarvis",
        binary_basename="jarvis-command-runner",
        targets=WINDOWS_TARGETS,
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install native Jarvis binaries.")
    parser.add_argument(
        "--workflow-url",
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " May be repeated. Defaults to jarvis, jarvis-windows-sandbox-setup,"
            " jarvis-command-runner, and rg."
        ),
    )
    parser.add_argument(
        "root",
...
def main() -> int:
    ...
    JARVIS_CLI_ROOT = (args.root or JARVIS_CLI_ROOT).resolve()
    vendor_dir = JARVIS_CLI_ROOT / VENDOR_DIR_NAME
    ...
    with _gha_group(f"Download native artifacts from workflow {workflow_id}"):
        with tempfile.TemporaryDirectory(prefix="jarvis-native-artifacts-") as artifacts_dir_str:
